...
$L_\infty(S,\mu)$ forces $\mu$ to be normal with pseudocompact support.
\medskip
...
\begin{proposition}\label{MorphDecomp} Let $X$ be a Banach $A$-module and $E$ be
    ...
    \begin{enumerate}[label = (\roman*)]
        ...
        \item $\sigma_2(a\cdot x)=0$ for all $x\in X$ and $a\in A$.
...
\begin{proposition}\label{InnerOpenRegMeasCharac} Let $S$ be a locally compact
    ...
    \begin{enumerate}[label = (\roman*)]
        ...
        \item if $\mu$ is inner open regular, then it is residual;

        \item if $\mu$ is finite and inner open regular, then
              $\mu(E)=\mu(\operatorname{int}_S(E))=\mu(\operatorname{cl}_S(E))$
              for $E\in Bor(S)$;

        \item if $\mu$ is inner compact regular and inner open regular,
              then it is normal;
    \end{enumerate}
\end{proposition}
\begin{proof} $(i)$ It is enough to note that the supremum in the definition of
    ...
    $(ii)$. If $E$ is nowhere dense Borel set, 
    then $\operatorname{int}_S(\operatorname{cl}_S(E))=\varnothing$. 
    Clearly, $\operatorname{int}_S(E)
        \subset\operatorname{int}_S(\operatorname{cl}_S(E))$, 
    so $\mu(E)=\mu(\operatorname{int}_S(E))\leq\mu(\varnothing)=0$. Since $E$ 
    is arbitrary, $\mu$ is residual;

    $(iii)$ The first equality was proved in the paragraph $(i)$. Since $\mu$
    is finite for all $E\in Bor(S)$ we have
    $\mu(E)=\mu(S)-\mu(\operatorname{int}_S(S\setminus E))=\mu(S)-\mu(S\setminus
        \operatorname{cl}_S(E))=\mu(\operatorname{cl}_S(E))$. 

    $(iv)$ Any inner compact regular measure has support and is fully supported
...
\begin{proposition}\label{MorphLpC0Prop} Let $S$ be a locally compact Hausdorff 
    space, let $\mu$ be a locally finite inner compact regular Borel measure 
    on $S$, and let $1\leq p<+\infty$. Let $\psi: L_p(S,\mu)_{ess}\to C_0(S)$ be 
    a $C_0(S)$-morphism and let $t\in S$ be a point with $\mu(\{t\})=0$. 
    Then $\psi([f])(t)=0$ for every $[f]\in L_p(S,\mu)$.
\end{proposition}
\begin{proof} By paragraph $(ii)$ of proposition \ref{LpEssC0ModCharac} we 
    have $L^p(S,\mu)_{ess}=L^p(S,\mu)$. Let $q$ be the conjugate exponent 
    of $p$, so that $1/p+1/q=1$ (with $q=+\infty$ when $p=1$). Since $\mu$ is 
    locally finite and inner compact regular, so is its complete locally 
    determined version $\hat{\mu}$ [\cite{FremMeasTh4.1}, proposition 412H (a)]. 
    Therefore, $\hat{\mu}$ is a Radon 
    measure [\cite{FremMeasTh4.1}, proposition 411H (b)], hence strictly 
    localizable [\cite{FremMeasTh4.1}, proposition 416B]. As a consequence, 
    $\mu$ is localizable [\cite{FremMeasTh2}, proposition 212G(a)]. 
    Therefore the map
    $$
        I:L_q(S,\mu)\to L_p(S,\mu)^*,
        [g]\mapsto\Big([f]\mapsto\int_S f(s)g(s)\,d\mu(s)\Big)
    $$
    is an isometric isomorphism [\cite{FremMeasTh2}, 243G(b), 244O(c)]. 
    Define a linear functional 
    $l: L_p(S,\mu)\to\mathbb{C}, [f]\mapsto\psi([f])(t)$. Clearly, it is 
    bounded. Choose $[h]\in L_q(S,\mu)$ such that $I([h])=l$ and 
    $h$ Borel measurable.

    Fix $a\in C_0(S)$. As $\psi$ is a $C_0(S)$-morphism, for 
    every $[f]\in L_p(S,\mu)$ we get
    $$
        I([(a-a(t)\chi_S)h])([f])
        =l([af])-a(t)l([f])
        =\psi([af])(t)-a(t)\psi([f])(t)=0.
    $$
    Since $[f]\in L_p(S, \mu)$ is arbitrary and $I$ is isometric we 
    get $[(a-a(t)\chi_S)h]=0$ in $L_q(S, \mu)$. In other words $[ah]=a(t)[h]$ 
    in $L_q(S, \mu)$ for all $a\in C_0(S)$. Suppose $[h]\neq 0$ and 
    define a Borel set $E=\{x\in S:h(x)\neq 0\}$. Then $\mu(E)>0$. 
    As $\mu(\{t\})=0$ we have $\mu(E\setminus \{t\})>0$, and by inner compact 
    regularity there is a compact set $K\subset E\setminus \{t\}$ 
    with $\mu(K)>0$. In particular $t\notin K$. Since $S$ is Hausdorff and $K$ 
    is compact, there is an open set $V$ with $K\subset V$ and $t\notin V$. 
    By [\cite{DalesBanSpContFunDualSp}, theorem 1.4.25] there 
    exists $\hat{a}\in C_0(S)$ with $\hat{a}|_K=1$ 
    and $\hat{a}|_{S\setminus V}=0$. As $t\in S\setminus V$ we 
    have $\hat{a}(t)=0$. Then at every $s\in K$ 
    holds $\hat{a}(s) h(s)\neq \hat{a}(t)h(s)$, while $\mu(K)>0$. In particular,
    $[\hat{a}h]\neq \hat{a}(t)[h]$ in $L_q(S,\mu)$. Hence $[h]=0$ and $l=0$, 
    that is $\psi([f])(t)=0$ for all $[f]\in L_p(S,\mu)$.
\end{proof}
...
\begin{proposition}\label{LpC0ModNecessCond} Let $S$ be a locally compact
    ...
    $(iii)$ if $p<+\infty$, then $\mu$ is purely atomic.
\end{proposition}
\begin{proof} $(i)$ Let $K\subset S$ be a compact set of positive measure.
    ...
    remains to apply proposition~\ref{InnerOpenRegMeasCharac}~$(i)$.
    ...
    On the other hand, $s\in K\subset S_c^{\mu}$, so $\mu(\{s\})=0$ and by 
    proposition \ref{MorphLpC0Prop} we get $a(s)=\psi([f])(s)=0$. Contradiction, 
    therefore $\mu(K)=0$. As $K\subset S_c^\mu$ is arbitrary
...
\begin{theorem}\label{LInfReProjNecessCond} Let $S$ be a locally compact
    ...
    $C_0(S)$-module then $\mu$ is normal and
    $\operatorname{supp}(\mu)$ is pseudocompact.

\end{theorem}
\begin{proof} From proposition~\ref{LInfRelProjSuppCond} we conclude 
    that $\operatorname{supp}(\mu)$ is pseudocompact. By 
    proposition~\ref{LpC0ModNecessCond}~$(i)$ the measure $\mu$ is inner open 
    regular. By assumption $\mu$ is inner compact regular, so 
    from~\ref{InnerOpenRegMeasCharac}~$(iv)$ we get that $\mu$ is normal.
\end{proof}
