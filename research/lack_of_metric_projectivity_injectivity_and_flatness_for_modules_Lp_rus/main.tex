...
\end{proposition}
\begin{proof} 
    Поскольку $\mathcal{A}$ --- разложение $\Omega$ на атомы, то
    $
    ...
    \end{aligned}
    \]
    Снова из утверждение \ref{GnrlzdMeanProp} для 
    каждого $A\in\mathcal{A}$ получаем
    $
        |m_{A,-p}(\chi_A)|^p \int_{\Omega} |\chi_A(\omega)|^p d\mu(\omega)
        =1.
    $
    Таким образом, для любого $x\in\ell_p(\mathcal{A})$, выполнено
    \[
    \begin{aligned}
        \Vert J_p(x)\Vert
        =\left(
    ...
    \end{aligned}
    \]
    Итак, $I_p$ и $J_p$ --- изометрические отображения. Очевидно, что они 
    линейны. Заметим, что для любого $x\in\ell_p(\mathcal{A})$ 
    ...
    \[
        ...
        =x.
    \]
    Далее, для любого $A\in\mathcal{A}$ верно, что
    $
        J_p(e_A)
    ...
    \[
        ...
        =[f].
    \]
    Таким образом, $J_p$ и $I_p$ взаимно обратные операторы.
\end{proof}
