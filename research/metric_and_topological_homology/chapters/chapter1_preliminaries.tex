...
respective options in order, enclosed and separated like this:
$\langle$~$\ldots$ / $\ldots$~$\rangle$. For example: a real number $x$ is
$\langle$~positive / non-negative~$\rangle$ if $\langle$~$x>0$ / $x\geq
0$~$\rangle$. Sometimes one of the parts might be empty. We use the symbol $:=$ for
...
$\mathbb{N}_n$ denotes the set of the first $n$ natural numbers, $\mathbb{R}_+$
denotes the set of non-negative real numbers, $\mathbb{T}$ denotes the set of
complex numbers of modulus $1$, and finally, $\mathbb{D}$ denotes the set of complex
numbers with modulus less than $1$. For $z\in\mathbb{C}$, the symbol
$\overline{z}$ stands for the complex conjugate number.

For a given map $f:M\to M'$ and a subset $\langle$~$N\subset M$ / $N'\subset M'$
such that $\operatorname{Im}(f)\subset N'$~$\rangle$ by $\langle$~$f|_N$ /
$f|^{N'}$~$\rangle$ we denote the  $\langle$~restriction of $f$ to $N$ /
corestriction of $f$ to $N'$~$\rangle$, that is $\langle$~$f|_N:N\to
...
the notation we shall use. We assume that our reader is familiar with such basics of
category theory as category, functor and morphism. Otherwise, see
\cite[chapter 0]{HelLectAndExOnFuncAn} for a quick introduction or
\cite[chapter 1]{KashivShapCatsAndSheavs} for more details.
...
morphism $\phi:X\to Y$ is called a $\langle$~retraction / coretraction~$\rangle$
...
express the existence of an isomorphism between $X$ and $Y$ as $X\isom{\mathbf{C}} Y$.
...
with a slight modification of the categories between which these functors act.
...
representable by an object $X$ if
...
Constructions of the categorical product and coproduct shall play an important role
...
is representable by an object $X$. As a consequence, we get that a
...
open set that is contained in $E$. We say that $E$ is a neighborhood of a point 
...
which equals $\{U\cap E:U\in\tau \}$. A topological space is called Hausdorff if any two
distinct points have disjoint open neighborhoods. In this book we shall work
with Hausdorff spaces only.

A map $f:X\to Y$ between topological spaces is called continuous if the preimage
...
Let $\mathcal{E}$ be a family of subsets of a topological space $S$. We say that
$\mathcal{E}$ is a cover if its union equals $S$. We say that a cover is open if
...
\begin{enumerate}[label = (\roman*)]
  ...
  \item pseudocompact if any locally finite family of non-empty open sets
  is finite;

  \item locally compact if any point has a compact neighborhood;

  \item extremally disconnected if the closure of any open set is
  open;

  \item Stonean if it is an extremally disconnected Hausdorff compact space.
\end{enumerate}

The property of being a $\langle$~compact / paracompact / locally
...
Hausdorff space $S$ can be regarded as a dense subspace of some compact Hausdorff
...
Stone-Cech compactification. It can be represented as the closure of the image
of the embedding
$j:S\to\prod_{f\in C}[0,1]:s\mapsto \prod_{f\in C}f(s)$, where $C$ is the set of
...
is known that $\beta\mathbb{N}$ is an extremally disconnected Hausdorff compact.

Occasionally we shall apply Urysohn's lemma to locally compact Hausdorff
...
For more details on topological spaces see the comprehensive
...
a set $M$ if $\mathfrak{B}$ doesn't contain the empty set and the intersection of 
...
given set is a partially ordered set. Filters that are maximal with respect to
...
neighborhood $U$ of $x$ it holds that $\phi^{-1}(U)\in\mathfrak{F}$. Directly from the
...
Limits along filters preserve the order structure of $\mathbb{R}$. More precisely: if
...
Limits along filters respect continuous functions. Rigorously this is formulated as
...
The most important feature of filters and the reason for our interest is the
...
A limit of the net $x:N\to X$ is a limit of the function $x$ along the section
filter of the directed set $N$. It is denoted $\lim_\nu x_\nu$. We shall exploit
both notions of the limit.

More on these matters can be found in \cite[section 7]{BourbElemMathGenTopLivIII}.
...
it contains the empty set, complements of all its elements and is closed
...
of a measure space is an  arbitrary set $\Lambda$ with the $\sigma$-algebra of all
subsets and the so-called counting measure
...
spaces is enough for most applications, but we shall encounter more generic
measure spaces.

A measure space $(\Omega,\Sigma,\mu)$ is called strictly localizable if there
...
follows that all atoms of strictly localizable measure spaces are of finite
...
that a purely atomic measure space can be represented as a disjoint union of some
family of atoms. This family is countable if the measure space is $\sigma$-finite.
These facts allow us to say that the structure of a purely atomic measure space is
well understood. The structure of strictly localizable non-atomic measure spaces
is given by Maharam's theorem \cite[332B]{FremMeasTh}. 
...
$(\Omega_1,\Sigma_1,\mu_1)$ and $(\Omega_2,\Sigma_2,\mu_2)$. The formal definition of
the product measure for localizable measure spaces is rather involved
\cite[definition 251F]{FremMeasTh} and we don't give it here.  For our
purposes it is enough to know that the product of two strictly localizable
measure spaces is again strictly localizable \cite[proposition 251N]{FremMeasTh}. By the direct sum of measure spaces $ \{(\Omega_\lambda, \Sigma_\lambda,
\mu_\lambda):\lambda\in\Lambda \}$ we denote the disjoint union of sets 
$\{\Omega_\lambda:\lambda\in\Lambda \}$ with the $\sigma$-algebra defined as 
$\Sigma= \{
  E\subset \Omega: E\cap \Omega_\lambda\in\Sigma_\lambda
  \text{ for all }\lambda\in\Lambda
 \}$ 
and the measure given by the formula 
$\mu(E)=\sum_{\lambda\in\Lambda}\mu_\lambda(E\cap \Omega_\lambda)$. It is clear now
that strictly localizable measure spaces are exactly direct sums of finite
measure spaces.
...
By a measurable function we always mean a complex or real valued function on
a measurable space, with the property that the preimage of every open set is
measurable. We say that two measurable functions are equivalent if the set where
they are different is negligible.
...
Densities provide a convenient source of measures. If $f:\Omega\to[0,+\infty]$
is a measurable function on $(\Omega,\Sigma,\mu)$, then the formula
\[
f\mu:E\mapsto\int_{E}f(\omega)\,d\mu(\omega)
\]
defines a measure $f\mu:\Sigma\to[0,+\infty]$. If $f:\Omega\to\mathbb{C}$ is an
integrable function on $(\Omega,\Sigma,\mu)$, then the same formula defines a
complex measure $f\mu:\Sigma\to\mathbb{C}$.
...
Lebesgue decomposition theorem for this case. For two given $\sigma$-finite
measures $\mu$ and $\nu$ on a measurable space $(\Omega,\Sigma)$ there exists a
measurable function $\rho_{\nu,\mu}:\Omega\to[0,+\infty]$, a $\sigma$-finite
...
It is called the Borel $\sigma$-algebra of $S$ and denoted by $\operatorname{Bor}(S)$. 
Measures and complex measures defined on Borel $\sigma$-algebras are supported 
with the adjective Borel.

The support of a complex Borel measure $\mu$ is the set of all points
$s\in S$ such that $|\mu|(U)>0$ for every open neighborhood $U$ of $s$. We
...
\begin{enumerate}[label = (\roman*)]
  ...
  class $\mathcal{C}$ of Borel sets if for any Borel set $E$ it holds that 
  \[
  ...
  class $\mathcal{C}$ of Borel sets if for any Borel set $E$ it holds that 
  \[
  \mu(E)=\inf \{\mu(U): E\subset U, U\in\mathcal{C} \};
  \]

  \item residual if $\mu(E)=0$ for any Borel nowhere dense set $E$;

  \item normal if it is residual and has full support.
\end{enumerate}

Usually we consider the regularity of measures with respect to classes of open and
compact sets. Now recall a few facts on topological measures. All compact sets 
have finite measure if the measure is locally finite. Any finite inner compact 
...
second and fourth volumes of~\cite{FremMeasTh}.
...
We assume that our reader is familiar with the fundamentals of functional analysis
...
the linear quotient Banach space. Its norm is defined by the equality 
\[
\lVert x+F\rVert=\inf \{\lVert x+y\rVert: y\in F \},
\]
where $x+F\in E/F$.
...
$T:E\to F$ is called:
\begin{enumerate}[label = (\roman*)]
  \item bounded if its norm $\lVert T\rVert:=\sup \{\lVert T(x)\rVert:x\in B_E \}$ 
  is finite;
...
bounded, and any bounded operator is continuous. By $\langle$~$\mathcal{B}(E,F)$ /
...
$\mathcal{K}(E,F)$ are just the usual operator norms. The norm of a nuclear
operator $T$ is defined by the equality
\[
\lVert T\rVert
:=\inf\left \{
  \sum_{n=1}^\infty\lVert S_n\rVert
...
$E^*=\mathcal{B}(E, \mathbb{C})$. It is called the dual space of $E$. Similarly,
...
For a given bounded linear operator $T:E\to F$, its adjoint is a bounded linear 
operator 
\[
T^*:F^*\to E^*,\,f\mapsto (x\mapsto f(T(x))).
\]
Again from the Hahn-Banach 
theorem it follows that $\lVert T^*\rVert=\lVert T\rVert$. One can easily check 
...
$\langle$~$ \{ f\in E^*: f(x)=0 \text{ for all } x\in F \}$ / 
$ \{ x\in E : f(x)=0 \text{ for all } f\in F \}$~$\rangle$. 
Clearly, $E^{\perp} = \{ 0\}$, $0^{\perp}=E^*$ 
and ${}^{\perp}E^*= \{ 0\}$, ${}^{\perp}0= E$. Annihilators are closely related
to quotient spaces. One can show that 
the operators 
\[
i: E^*/F^{\perp}\to F^*,\, f+F^\perp\mapsto f|_F,
\quad\quad 
q:{(E/F)}^*\to F^{\perp},\, f\mapsto (x\mapsto f(x + F))
\] 
are isometric isomorphisms.

A few words on the classification of bounded linear operators. A bounded linear
operator $T:E\to F$ is called:
\begin{enumerate}[label = (\roman*)]
  \item topologically injective if it performs a homeomorphism on its image;

  \item topologically surjective if it is an open map;

  \item coisometric if it maps the open unit ball onto the open unit ball;

  \item strictly coisometric if it maps the closed unit ball onto the closed unit ball; 

  \item $c$-topologically injective, if $\lVert x\rVert\leq c\lVert  T(x)\rVert$ for
  all $x\in E$;
...
\begin{enumerate}[label = (\roman*)]
  ...
  \item if $ T$ is $c$-topologically injective, then $ T^*$ is strictly
  $c$-topologically surjective;

  \item if $ T^*$ is $c$-topologically surjective or strictly $c$-topologically 
  surjective, then $ T$ is $c$-topologically injective;

  \item if $T^*$ is $c$-topologically injective and $E$ is complete, then $T$ 
...
$c$-coretraction~$\rangle$ if there exists a bounded linear operator $S:F\to E$
such that $\langle$~$T S=1_F$ / $S T=1_E$~$\rangle$ 
and $\lVert T\rVert\lVert S\rVert\leq c$. In this case we say 
that $\langle$~$F$ / $E$~$\rangle$ has a
$c$-complemented copy in $\langle$~$E$ / $F$~$\rangle$ via the topologically $\langle$~surjective /
injective~$\rangle$ operator $T$. If the natural embedding of a subspace $F$
...
with $\lVert P\rVert\leq c$. We say that $F$ is weakly $c$-complemented in $E$
...
complemented subspace is the following: consider an arbitrary Banach space $E$, 
then $E^*$ is contractively complemented in $E^{***}$ via the Dixmier 
projection $P=\iota_{E^*}{(\iota_E)}^*$. A canonical example of an uncomplemented 
subspace is $c_0(\mathbb{N})$ 
inside $\ell_\infty(\mathbb{N})$ \cite[theorem 2.5.5]{KalAlbTopicsBanSpTh}.
Nevertheless, $c_0(\mathbb{N})$ is weakly complemented in
$\ell_\infty(\mathbb{N})$ by the Dixmier projection.

Let $E$, $F$ and $G$ be three Banach spaces; then a bilinear operator
$\Phi:E\times F\to G$ is called bounded if its norm 
\[
\lVert \Phi\rVert:=\sup \{\lVert \Phi(x,y)\rVert:x\in B_E, y\in B_F \}
\] 
is finite. 
...
\lVert u\rVert
:=\inf\left \{
  \sum_{i=1}^n \lVert x_i\rVert\lVert y_i\rVert
...
\] 
such that  $(T\projtens S)(x\otimes y)=T(x)\otimes S(y)$ 
for all $x\in E_1$ and $y\in F_1$. 
Even more, $\lVert T\projtens S\rVert=\lVert T\rVert\lVert S\rVert$. The main feature 
of the projective tensor product which makes it so important is the following 
...
\]
In other words, the projective tensor product linearizes bounded bilinear operators.
...
An important source of examples of Banach spaces is $L_p$-spaces, also known as
...
measure space.  For $1\leq p<\infty$, as usual, the symbol $L_p(\Omega,\mu)$
...
measure $\mu$. The norm of such a function is defined by
\[
\lVert f\rVert
:={\left(
...
$1\leq p<+\infty$ \cite[theorems 243G, 244K]{FremMeasTh}. 
One more well known fact is that $L_p$-spaces are reflexive for 
$1<p<+\infty$. Here we exploited the standard 
notation $p^*=+\infty$ if $p=1$, $p^*=p/(p-1)$ if $1<p<+\infty$ and $p^*=1$ if $p=+\infty$.
...
by $C_0(S)$. When endowed with the $\sup$-norm, $C_0(S)$ becomes a Banach space. Any
set $\Lambda$ with the discrete topology may be regarded as a locally compact space
...
the $\sup$-norm or $L_\infty(\Omega,\mu)$ are $C(K)$ spaces for some compact
Hausdorff space $K$ \cite[remark 4.2.8]{KalAlbTopicsBanSpTh}. 
By $M(S)$ we denote the Banach space of
complex finite Borel regular measures on $S$. The norm of a measure $\mu\in M(S)$
is defined by the equality $\lVert\mu\rVert=|\mu|(S)$, where $|\mu|$ is the total
variation measure of the measure $\mu$. By the Riesz-Markov-Kakutani theorem
\cite[section C.18]{ConwACoursInFuncAn} we have
${C_0(S)}^*\isom{\mathbf{Ban}_1}M(S)$. In fact, $M(S)$ is an $L_1$-space, see
the discussion after \cite[proposition 2.14]{DalLauSecondDualOfMeasAlg}. 
...
by $\ell_p(\Lambda)$. For this type of measure space we have one more important
...
$\lVert x\rVert_p
=\lVert{
  (\rVert x_\lambda\lVert)}_{\lambda\in\Lambda}
\rVert_{\ell_p(\Lambda)}$
...
$\left \{
  ...
  :\lVert x\rVert_p<+\infty
\right \}$ 
with the norm $\lVert\cdot\rVert_p$ is denoted by $\bigoplus_p
...
:x\mapsto {(T_\lambda(x_\lambda))}_{\lambda\in\Lambda}
\]
which we shall denote by $\bigoplus_p \{T_\lambda:\lambda\in\Lambda \}$. Its
norm equals $\sup_{\lambda\in\Lambda}\lVert T_\lambda\rVert$.
...
The reason is that any family of Banach spaces admits a $\langle$~coproduct /
product~$\rangle$ in $\mathbf{Ban}_1$ which in fact is their
...
of objects \cite[chapter 2, section 5]{HelLectAndExOnFuncAn}. Obviously, any 
...
$\langle$~an isometric copy / a copy~$\rangle$ of a Banach space $F$ if $F$ is
...
$\lVert T\rVert\lVert S\rVert\leq C+\epsilon$. 
If $E$ is an $\mathscr{L}_{p,C}^g$-space for some $C\geq 1$, we
simply say that $E$ is an $\mathscr{L}_p^g$-space. This definition
\cite[definition 23.1]{DefFloTensNorOpId} is an improvement of the definition
...
\cite[exercise 4.7]{DefFloTensNorOpId}, but the converse is not true. Any
$c$-complemented subspace of an $\mathscr{L}_{p,C}^g$-space is an
$\mathscr{L}_{p,cC}^g$-space \cite[corollary 23.2.1(2)]{DefFloTensNorOpId}. A
Banach space is an $\mathscr{L}_{p,C}^g$-space iff its dual is an
$\mathscr{L}_{p^*,C}^g$-space \cite[corollary 23.2.1(1)]{DefFloTensNorOpId}.
All $C(K)$-spaces are $\mathscr{L}_{\infty, 1}^g$-spaces
\cite[lemma 4.4]{DefFloTensNorOpId}. Note that, for a given locally compact
...
be concerned with $\mathscr{L}_1^g$- and $\mathscr{L}_\infty^g$-spaces.
...
\cite[corollary III.C.14]{WojBanSpForAnalysts}. This property is preserved by
...
\cite[theorem 5.4.4]{KalAlbTopicsBanSpTh}: a Banach space $E$ has the
...
passes to complemented subspaces \cite[proposition 13.44]{FabHabBanSpTh}. This
property behaves badly with reflexive spaces: since the unit ball of a reflexive
space is weakly compact \cite[theorem 2.8.2]{MeggIntroBanSpTh}, then a reflexive
Banach space with the Dunford-Pettis property has a norm compact unit ball and
therefore this space is finite dimensional. 

To introduce the next Banach geometric property, we need definitions of a Banach
lattice and an unconditional Schauder basis. 

A real Riesz space $E$ is a vector space over $\mathbb{R}$ with the structure of
a partially ordered set such that $x\leq y$ implies $x+z\leq y+z$ for every
...
a real Riesz space which is a lattice as a partially ordered set. If $E$ is a real
vector lattice, then for every $x\in E$ we define its absolute value by the equality
$|x|:=x\vee(-x)$. A complex vector lattice $E$ is a vector space over
$\mathbb{C}$ such that there exists a real vector subspace
$\operatorname{Re}(E)$ called the real part of $E$, which is a real vector lattice and
\begin{enumerate}[label = (\roman*)]
...
lattice such that $\lVert x\rVert\leq \lVert y\rVert$ whenever $x, y\in E$ 
...
\cite[section 1]{LaceyIsomThOfClassicBanSp}.
...
the Banach geometric reason for this phenomenon, we need the definition of an
...
an unconditional basis is $C([0,1])$. Even more, this Banach space can not even be a
subspace of a space with an unconditional basis \cite[proposition 3.5.4]{KalAlbTopicsBanSpTh}.  Any unconditional Schauder basis
${(x_\lambda)}_{\lambda\in\Lambda}$ in $E$ satisfies the following property
\cite[proposition 1.6]{DiestAbsSumOps}: there exists a constant $\kappa\geq 1$
such that
\[
\left\lVert \sum_{\lambda\in\Lambda}t_\lambda f_\lambda(x)x_\lambda\right\rVert
\leq
\kappa\left\lVert \sum_{\lambda\in\Lambda}f_\lambda(x)x_\lambda\right\rVert
\]
for all $x\in E$ and $t\in B_{\ell_\infty(\Lambda)}$. The least such constant
$\kappa$ among all unconditional Schauder bases of $E$ is denoted by
$\kappa(E)$. A similar constant could be defined for Banach spaces without
unconditional Schauder bases. The local unconditional constant $\kappa_u(E)$ of
a Banach space $E$ is defined to be the infimum of all scalars $c$ with the
following property: given any finite dimensional subspace $F$ of $E$ there
exists a Banach space $G$ with an unconditional Schauder basis and two bounded
linear operators $S:F\to G$, $T:G\to E$ such that $TS|^{F}=1_F$ and $\lVert
T\rVert\lVert S\rVert\kappa(G)\leq c$. We say that a Banach space $E$ has the local
unconditional structure property (the l.u.st.\ property for short) if
$\kappa_u(E)$ is finite. Clearly, any Banach space $E$ with an unconditional
Schauder basis has the l.u.st.\ property with $\kappa_u(E)\leq\kappa(E)$. In
...
\cite[theorem 17.1]{DiestAbsSumOps}. Directly from the definition it follows
...
\cite[theorem 17.5]{DiestAbsSumOps}: a Banach space $E$ has the l.u.st.
property iff $E^{**}$ is topologically isomorphic to a complemented subspace of
some Banach lattice. As a corollary of this criterion, we get that $E$ has the
l.u.st.\ property iff so does $E^*$ \cite[corollary 17.6]{DiestAbsSumOps}.

The last property we shall discuss is the well known approximation property
...
that $\lVert T(x)-x\rVert<\epsilon$ for all $x\in K$. If $T$ can be 
chosen with $\lVert T\rVert\leq c$, then $E$ is said to have the $c$-bounded
approximation property. The metric approximation property is another name for
the $1$-bounded approximation property. We say that $E$ has the bounded
...
are inherited by complemented subspaces \cite[exercise 5.5]{DefFloTensNorOpId}. All $L_p$-spaces and $C(K)$-spaces have the metric approximation property
\cite[section 5.2(3)]{DefFloTensNorOpId}, but their subspaces may fail the
approximation property \cite[section 5.2(1)]{DefFloTensNorOpId}. Any Banach
space with an unconditional Schauder basis has the approximation property
\cite[example 4.4]{RyanIntroTensNormsBanSp}. If $E^*$ has the approximation
property, then so does $E$ \cite[corollary 5.7.2]{DefFloTensNorOpId}. The
reason why the approximation property is so important is rather simple --- it has a
lot of equivalent reformulations that involve many nice properties of Banach
spaces. For example, the following properties of a Banach space $E$ are equivalent
\cite[sections 5.3, 5.6]{DefFloTensNorOpId}:
\begin{enumerate}[label = (\roman*)]
  \item $E$ has the approximation property;

  \item the natural mapping $\operatorname{Gr}:E^*\projtens E\to\mathcal{N}(E)$ is an isometric
  isomorphism;
...
There is much more to list here, but we confine ourselves to these three
properties.
...
$\cdot:A\times A\to A:(a,b)\mapsto ab$ is of norm at most $1$. A typical
...
very important class of $C^*$-algebras to be discussed below. By a $\langle$~left
...
$a\in A$. The element which is both a left and a right identity is called the
identity and denoted by $e_A$. In general, we do not assume that Banach algebras
are unital, i.e.\ have an identity. Even if a Banach algebra $A$ is unital, we do
not require its identity to be of norm $1$. We use the notation
...
unitization of $A$, i.e. $A_\times=A$ if $A$ has an identity of norm one and
$A_\times=A_+$ otherwise. Whenever a statement holds for both unitizations, we
write $A_\bullet$ to denote either $A_+$ or $A_\times$. Even in the absence of
an identity in the case of Banach
...
in the norm topology. If we replace the norm topology with the weak topology, we get
...
$\operatorname{sp}_A(a)$ of an element $a\in A$ as the set of all complex numbers
...
\cite[corollary 2.1.16]{HelBanLocConvAlg}.
...
the unit ball of $A^*$  \cite[theorem 1.2.6]{HelBanLocConvAlg}. Therefore, we
...
is unital, then its spectrum is compact \cite[theorem 1.2.50]{HelBanLocConvAlg}. Now for a given Banach algebra $A$ with non-empty spectrum we can
construct a contractive homomorphism $\Gamma_A:A\to
C_0(\operatorname{Spec}(A)):a\mapsto(\varkappa\mapsto \varkappa(a))$ called the
Gelfand transform of $A$ \cite[theorem 4.2.11]{HelBanLocConvAlg}. The kernel
of this homomorphism is called the Jacobson radical and denoted by
...
called semisimple. By Shilov's idempotent theorem \cite[section 3.5]{KaniBanAlg} any semisimple Banach algebra with compact spectrum is unital.

Most standard constructions for Banach spaces have their counterparts for
Banach algebras. For example, the $\bigoplus_p$-sum of Banach algebras endowed with
...
are called ${}^*$-homomorphisms. A Banach algebra with an isometric involution is
called a ${}^*$-Banach algebra. An example of such an algebra is the Banach algebra
of bounded linear operators on a Hilbert space with the operation of taking the
...
is a $C^*$-algebra if it satisfies $\lVert a^*a\rVert=\lVert a\rVert^2$ for all
...
\cite[theorem 4.7.13]{HelBanLocConvAlg} states that any commutative
$C^*$-algebra $A$ is isometrically isomorphic as a ${}^*$-algebra to
$C_0(\operatorname{Spec}(A))$. The second theorem \cite[theorem 4.7.57]{HelBanLocConvAlg} gives a description of generic $C^*$-algebras as closed
${}^*$-Banach subalgebras of $\mathcal{B}(H)$ for some Hilbert space $H$. Such
a representation is not unique, but a norm (if it exists) that turns a
...
$C^*$-algebra is isomorphic as a ${}^*$-algebra to a von Neumann algebra, then it is
called a $W^*$-algebra. By Sakai's well known theorem \cite[theorem III.2.4.2]{BlackadarOpAlg} each $C^*$-algebra which is dual as a Banach space is a
$W^*$-algebra, but beware, a $W^*$-algebra may be represented as a non weak${}^*$
...
some $b\in A$, and unitary if $a^*a=aa^*=e_A$. The set $A_{pos}$ of all positive 
elements of $A$ is a closed cone in $A$. If an element $a\in A$ 
is $\langle$~self-adjoint / positive~$\rangle$, then
$\langle$~$\operatorname{sp}_A(a)\subset[-\lVert a\rVert, \lVert a\rVert]$ /
$\operatorname{sp}_A(a)\subset[0,\lVert a\rVert]$~$\rangle$. For a given
...
functional calculus \cite[theorem 4.7.24]{HelBanLocConvAlg}. Loosely speaking,
...
mapping theorem allows one to compute the spectrum of elements given by the continuous
...
$C^*$-algebra. A quotient of a $C^*$-algebra by a closed two-sided ideal is a
...
with involution and multiplication defined by the equalities 
\[
...
$M_n(A)$ that makes it a $C^*$-algebra \cite[theorem 3.4.2]{MurphyCStarAlgsAndOpTh}. Obviously, $M_n(\mathbb{C})$ is isometrically isomorphic as
a ${}^*$-algebra to $\mathcal{B}(\ell_2(\mathbb{N}_n))$. From
\cite[remark 3.4.1]{MurphyCStarAlgsAndOpTh} it follows that the natural
embeddings 
...
then so is $M_n(\phi)$. Finally, we shall mention two isometric isomorphisms
that will be of use:
\[
...
contractive positive approximate identity \cite[theorem 4.7.79]{HelBanLocConvAlg}, and any closed left ideal has a right contractive positive approximate
identity. In some cases even an approximate identity is not enough, so for this
situation there is a procedure to endow a $C^*$-algebra with an identity and
preserve $C^*$-algebraic structure \cite[proposition 4.7.6]{HelBanLocConvAlg}.
...
Banach $A$-module if $X$ is a Banach space endowed with a bilinear operator
...
Banach $A$-module by defining $\langle$~$a\cdot x=0$ / $x\cdot a=0$~$\rangle$
...
Let $X$ be a $\langle$~left / right~$\rangle$ Banach $A$-module and $E$ be a
Banach space. Then $\mathcal{B}(X,E)$ is a $\langle$~right / left~$\rangle$ 
Banach $A$-module with module action defined 
by $\langle$~$(T\cdot a)(x)=T(a\cdot x)$ / $(a\cdot T)(x)=T(x\cdot a)$~$\rangle$ 
for all $a\in A$, $x\in X$. In particular, $X^*$ is 
a $\langle$~right / left~$\rangle$ Banach $A$-module. Similarly, 
$\mathcal{B}(E, X)$ is a $\langle$~left / right~$\rangle$ Banach $A$-module 
with module action defined 
by $\langle$~$(a\cdot T)(x)=a\cdot T(x)$ / $(T\cdot a)(x)=T(x)\cdot a$~$\rangle$ 
for all $a\in A$, $x\in E$.

By $\langle A\text{-}\mathbf{mod}$ / $\mathbf{mod}\text{-}A\rangle$ we shall denote the
category of $\langle$~left / right~$\rangle$ $A$-modules with continuous
$A$-module maps in the role of morphisms. By $\langle$~$A\text{-}\mathbf{mod}_1$ /
$\mathbf{mod}_1\text{-}A$~$\rangle$ we denote its subcategory of
$\langle$~$A\text{-}\mathbf{mod}$ / $\mathbf{mod}\text{-}A$~$\rangle$ with the same objects
and contractive morphisms only. Therefore,
$\langle$~$\operatorname{Hom}_{A\text{-}\mathbf{mod}}(X,Y)={}_A\mathcal{B}(X,Y)$ /
$\operatorname{Hom}_{\mathbf{mod}\text{-}A}(X,Y)=\mathcal{B}_A(X,Y)$~$\rangle$. 
...
As in any category, we can speak of retractions and coretractions in the
...
$\eta\xi=1_X$~$\rangle$ and $\lVert\xi\rVert\lVert\eta\rVert\leq c$. From the
definition it follows that the composition of $\langle$~$c_1$- and $c_2$-retraction
/ $c_1$- and $c_2$-coretraction~$\rangle$ gives a $\langle$~$c_1c_2$-retraction
/ $c_1c_2$-coretraction~$\rangle$. Clearly, the adjoint of
a $\langle$~$c$-retraction / $c$-coretraction~$\rangle$ is a
...
$\lVert\xi\rVert\lVert\eta\rVert\leq c$. In this case we say that $A$-modules $X$
...
this term see \cite[proposition 6.2.2]{HelBanLocConvAlg}. Clearly, $X/X_{ess}$
...
of $A$-modules admits the $\langle$~coproduct / product~$\rangle$ in
$A\text{-}\mathbf{mod}_1$ which in fact is their $\langle$~$\bigoplus_1$-sum /
$\bigoplus_\infty$-sum~$\rangle$. The category $A\text{-}\mathbf{mod}$ admits
$\langle$~coproducts / products~$\rangle$ only for finite families of objects.
Similar statements are valid for $\mathbf{mod}\text{-}A$ and $\mathbf{mod}_1\text{-}A$.
...
$\lVert \phi\projmodtens{A} \psi\rVert\leq\lVert \phi\rVert\lVert \psi\rVert$. 
...
\[
\mathcal{B}(-,E):A\text{-}\mathbf{mod}\to \mathbf{mod}\text{-}A
\qquad\qquad
\mathcal{B}(-,E):\mathbf{mod}\text{-}A\to A\text{-}\mathbf{mod}
\]
\[
\mathcal{B}(E,-):A\text{-}\mathbf{mod}\to A\text{-}\mathbf{mod}
\qquad\qquad
\mathcal{B}(E,-):\mathbf{mod}\text{-}A\to \mathbf{mod}\text{-}A
\]
\[
-\projmodtens{A} Y:\mathbf{mod}\text{-}A\to\mathbf{Ban}
\qquad\qquad
X\projmodtens{A} -:A\text{-}\mathbf{mod}\to\mathbf{Ban}
\]
...
$A\text{-}\mathbf{mod}_1$, $\mathbf{mod}_1\text{-}A$. 

In some cases it is possible to explicitly compute the projective module tensor
product. For example \cite[proposition 6.3.24]{HelBanLocConvAlg}, if $A$ is a 
Banach algebra and $I$ is a closed right ideal of $A_+$ with 
a left $\langle$~contractive / bounded~$\rangle$ approximate identity, and $X$ is 
...
\]
is $\langle$~an isometric isomorphism / a topological isomorphism~$\rangle$ of
...
Most of what has been said here can be generalized to Banach bimodules, but in
this book we shall not exploit them much. In those rare cases when we shall
...
Further, we briefly discuss the ABCs of so-called relative homology introduced and intensively
...
norm at most $c$. A left $A$-module $P$ is called $C$-relatively projective 
...
that $\lVert \psi\rVert\leq cC\lVert \phi\rVert$ and the diagram
\[
\xymatrix{
& {X} \ar[d]^{\xi}\\  % chktex 3
{P} \ar@{-->}[ur]^{\psi} \ar[r]^{\phi} &{Y}}  % chktex 3
\]
is commutative. Such an $A$-morphism $\psi$ is called a lifting of $\phi$, and it is
...
a left inverse bounded linear operator of norm at most $c$. A right 
...
\]
is commutative. Such an $A$-morphism $\psi$ is called an extension of $\phi$, and it
...
Banach module that may prevent it from being relatively projective, injective or flat. 
...
Obviously, any contractive $c$-relatively admissible $\langle$~epimorphism
...
a $C$-retract of a relatively $\langle$~free / cofree~$\rangle$ $A$-module. 
...
example related to Banach algebras. For a given Banach algebra $A$ define an $A$-morphism 
...
We say that $A$ is relatively $\langle$~biprojective / biflat / contractible /
amenable~$\rangle$ if it is $c$-relatively $\langle$~biprojective / biflat /
contractible / amenable~$\rangle$ for some $c\geq 1$. The infimum of the
constants $c$ is called the $\langle$~biprojectivity / biflatness /
contractibility / amenability~$\rangle$ constant.  With slight modifications of
\cite[proposition 7.1.72]{HelBanLocConvAlg} one can show that $A$ is
...
norm not greater than $c$ such that for all $a\in A$ it holds that 
$\langle$~$a\cdot d-d\cdot a=0$ and $a\Pi_A(d)=a$ / 
$\lim_\nu(a\cdot d_\nu-d_\nu\cdot a)=0$ 
and $\lim_\nu a\Pi_A(d_\nu)=a$~$\rangle$. Note that 
$\langle$~such an element $d$ / such a net ${(d_\nu)}_{\nu\in N}$~$\rangle$ is called
$\langle$~a diagonal / an approximate diagonal~$\rangle$. From a homological point
...
\cite[theorem 7.1.60]{HelBanLocConvAlg}. As for flatness, such phenomena are
