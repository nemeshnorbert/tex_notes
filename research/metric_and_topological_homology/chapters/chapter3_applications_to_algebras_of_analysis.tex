...
Now by a result of Hamana  [\cite{HamInjEnvBanMod}, proposition 2] the
...
as a $\bigoplus_\infty$-sum of such algebras [\cite{BerbBaerStarRings},
proposition 1.10.1].
\end{proof}
...
[\cite{HelBanLocConvAlg}, theorem 4.7.79]. As a 
...
a $\mathcal{K}(H)$-module [\cite{HelBanLocConvAlg}, theorem 7.1.27], so from
...
theorem 7.1.60]. In particular $\mathcal{B}(H)$ is a relatively flat
...
$(ii)$ The result follows from [\cite{NemMetProjInjFlatModLp}, theorem 4.5].
\end{proof}
...
$C^*$-algebra, so it is relatively amenable [\cite{HelBanLocConvAlg}, theorem
7.1.87], and therefore every left Banach $c_0(\Lambda)$-module is relatively
flat [\cite{HelBanLocConvAlg}, theorem 7.1.60]; in particular so
...
amenable [\cite{RundeAmenConstFour}, example 2]. Clearly, $\mathbb{C}_s$ is a
...
relatively amenable. Since this algebra is a $C^*$-algebra, it is $1$-relatively 
amenable [\cite{RundeAmenConstFour}, example 2]. Since $L_1(S,\mu)$ is 
...
From [\cite{NemMetProjInjFlatModLp}, theorem 4.5] we know that the number of 
...
presented in full detail in [\cite{DalBanAlgAutCont}, section 3.3].
...
23.2.1(2)]. As $M_s(G)$ is an annihilator $L_1(G)$-module, from
...
proposition  B10]. Therefore, if ${L_\infty(G)}^*$ is topologically injective as a
