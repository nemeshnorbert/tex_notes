...
$\mathscr{L}_1^g$-space. Since $M_s(G)$ is complemented in $M(G)$, $M_s(G)$
is an $\mathscr{L}_1^g$-space too \cite[corollary 23.2.1(2)]{DefFloTensNorOpId}. As $M_s(G)$ is an annihilator $L_1(G)$-module, from
proposition~\ref{MetTopFlatAnnihModCharac} we have that $M_s(G)$ is
a topologically flat $L_1(G)$-module. The $L_1(G)$-module $L_1(G)$ is also
topologically flat by proposition~\ref{LInfIsL1MetrInj}. Since
$M(G)\isom{L_1(G)\text{-}\mathbf{mod}_1}L_1(G)\bigoplus_1 M_s(G)$, $M(G)$ is
a topologically flat $L_1(G)$-module by proposition~\ref{MetTopFlatModCoProd}.
\end{proof}
