...
and $\Vert v^*v\Vert\leq 4\epsilon^2$. Since $u^*v+v^*u$ and $v^*v$ are self-adjoint, then $u^*v+v^*u\leq 8\Vert\sigma\Vert\epsilon e_{A_\#}$ 
...
and faithful $\mathcal{K}(H)$-modules, then from the results of the previous paragraph
...
$(i)$ Since $\ell_p(\Lambda)$ is reflexive for $1<p<+\infty$, then from 
...
an essential $C_0(S)$-module which is tautologically an $L_1$-space, then by
proposition~\ref{MetTopEssL1FlatModAoverAmenBanAlg} this module is metrically
...
a relatively projective $C_0(S)$-module. Since $s$ is the only atom of $\mu$, then
...
an $L_1(G)$-module, then $\rho$ is a coretraction in $\mathbf{mod}-L_1(G)$. Denote
