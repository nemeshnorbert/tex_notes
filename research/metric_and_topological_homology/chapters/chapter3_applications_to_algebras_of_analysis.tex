...
convolution. We shall investigate the main examples of these types. Following the
...
\cite[theorem 4.7.57]{HelBanLocConvAlg}) for any $C^*$-algebra $A$ there
exists a Hilbert space $H$ and an isometric ${}^*$-homomorphism
$\varrho:A\to\mathcal{B}(H)$. For Hilbert spaces that admit such a homomorphism we
may consider the left $A$-module $H_\varrho$ with module action defined as
$a\cdot x=\varrho(a)(x)$. Automatically we get the structure of a right $A$-module
on $H^*$ which by Riesz's theorem is isometrically isomorphic to $H^{cc}$.
This isomorphism allows one to define a right $A$-module structure on $H^{cc}$
by $\overline{x}\cdot a=\overline{\varrho(a^*)(x)}$. For given $x_1,x_2\in H$
...
\end{proposition}
\begin{proof} Without loss of generality we may assume that $\lVert x_0\rVert=1$.
...
Therefore, $H_\varrho$ is a retract of $A_+$ in $A\text{-}\mathbf{mod}_1$. From
...
metrically flat too. Since $H_\varrho^{cc}\isom{\mathbf{mod}_1\text{-}A}H_\varrho^*$,
...
hence flat as a $\mathcal{B}(H)$-module.
\end{proposition}
\begin{proof} From \cite[proposition 0.3.38]{HelBanLocConvAlg} we know that
$\mathcal{N}(H)\isom{\mathbf{Ban}_1}H\projtens H^*$. Let
$\varrho=1_{\mathcal{B}(H)}$. Then we can claim a bit more:
$\mathcal{N}(H)\isom{\mathcal{B}(H)\text{-}\mathbf{mod}_1} H_\varrho\projtens H^*$.
Since $H^*$ is finite dimensional
$H^*\isom{\mathbf{Ban}}\ell_1(\mathbb{N}_n)$ for $n=\dim(H)$ and as a result
$\mathcal{N}(H)\isom{\mathcal{B}(H)\text{-}\mathbf{mod}}
H_\varrho\projtens\ell_1(\mathbb{N}_n)$. By
proposition~\ref{SpatModOverCStarAlgProp} the module $H_\varrho$ is
topologically projective, so from corollary~\ref{MetTopProjTensProdWithl1} we
get that $\mathcal{N}(H)$ is topologically projective as a
...
$f:\operatorname{sp}_A(a)\to\mathbb{R}:t\mapsto t$.
\end{lemma}
...
any $p\in\mathbb{R}_0[t]$ has no constant term, $p(a)\in I$.  Hence, we have 
...
it holds that $\lVert p(a)\rVert=\lVert p|_{\operatorname{sp}_A(a)}\rVert_\infty$, so
$\lVert\operatorname{RPol}_a^0(p)\rVert
=\lVert p|_{\operatorname{sp}_A(a)}\rVert_\infty$. 
Thus, $\operatorname{RPol}_a^0$ is isometric. 
As $\mathbb{R}_0[t]$ is dense in $E$ and $I$ is complete
...
\cite[lemma 2.1]{BleKanFinGenCStarAlgHilbMod} they proved that any
...
\end{enumerate}
\end{theorem}
\begin{proof} $(i)\implies (ii)$ Since $p$ is a self-adjoint idempotent
$\lVert p\rVert=1$, so by proposition~\ref{UnIdeallIsMetTopProj} paragraph $(i)$
the ideal $I$ is metrically projective as an $A$-module.

$(ii)\implies (iii)$ See
proposition~\ref{MetProjIsTopProjAndTopProjIsRelProj}.

$(iii) \implies (i)$ Let ${(e_\nu)}_{\nu\in N}$ be a right contractive
approximate identity of the ideal $I$ \cite[theorem 4.7.79]{HelBanLocConvAlg}.
Since $I$ admits a right approximate identity, it is an essential left
...
$\sigma:I\to A\projtens \ell_1(B_I)$ of $\pi_I$ in $A\text{-}\mathbf{mod}$. For each
...
for all $x\in I$ we 
have $\lVert\sigma(x)\rVert=\sum_{d\in B_I} \lVert\sigma_d(x)\rVert$. 
...
\[
\sum_{d\in S}\lVert \sigma_d\rVert
...
\]
Since $S\in \mathcal{P}_{f}(B_I)$ is arbitrary, the sum 
$\sum_{d\in B_I}\lVert\sigma_d\rVert$ is finite. As a consequence, the 
sum $\sum_{d\in B_I}\lVert\sigma_d\rVert^2$ is finite too. 
...
$\sum_{d\in B_I\setminus S}\lVert\sigma_d\rVert<\epsilon$. Denote its cardinality
by $N$. Consider a positive element 
$b=\sum_{d\in B_I}\lVert\sigma_d\rVert^2 d^*d\in I$. Now we perform 
...
$\lVert b^{1/m}\rVert
=\sup_{t\in\operatorname{sp}_{A_\#}(b)} t^{1/m}\leq\lVert b\rVert^{1/m}$. Then $\limsup_{m\to\infty}\lVert b^{1/m}\rVert\leq 1$. 
Therefore, $\lVert b^{1/m}\rVert\leq 2$ for sufficiently big $m$. Denote
...
\]
Clearly, $\varsigma_d^*\varsigma_d\leq \lVert \varsigma_d\rVert^2 e_{A_\#}\leq
\lVert \sigma_d\rVert^2\lVert b^{1/m}\rVert^2 e_{A_\#}\leq 4\lVert \sigma_d\rVert^2
...
\leq 4\lVert \sigma_d\rVert^2 d^*d+4\lVert \sigma_c\rVert^2 c^*c
\]
...
&=\frac{1}{2}\left(
    ...
    \sum_{c\in S}\sum_{d\in S}c^*\varsigma_c^* \varsigma_d d
\right)\\
&\leq\frac{1}{2}\left(4 N\sum_{d\in S} \lVert \sigma_d\rVert^2 d^*d+
4 N\sum_{c\in S} \lVert \sigma_c\rVert^2 c^*c\right)\\
&=4 N\sum_{d\in S} \lVert \sigma_d\rVert^2 d^*d.
\end{aligned}
\]
Therefore,
\[
...
=\sum_{c\in S}\sum_{d\in S}c^*\varsigma_c^* \varsigma_d d
\leq 4N\sum_{d\in S} \lVert \sigma_d\rVert^2 d^*d
\leq 4N b
\]
Note that
\[
\lVert u\rVert
\leq \sum_{d\in S}\lVert\varsigma_d\rVert\lVert d\rVert
\leq \sum_{d\in S}2\lVert\sigma_d\rVert
\leq 2\lVert\sigma\rVert,
\qquad
\lVert v\rVert
\leq \sum_{d\in B_I\setminus S}\lVert\varsigma_d\rVert\lVert d\rVert
\leq \sum_{d\in B_I\setminus S}2\lVert\sigma_d\rVert
\leq 2\epsilon,
\]
so $\lVert u^*v+v^*u\rVert\leq 8\lVert\sigma\rVert\epsilon$ 
and $\lVert v^*v\rVert\leq 4\epsilon^2$. Since $u^*v+v^*u$ and $v^*v$ are self-adjoint, $u^*v+v^*u\leq 8\lVert\sigma\rVert\epsilon e_{A_\#}$ 
and $v^*v\leq 4\epsilon^2 e_{A_\#}$. Therefore for any $\epsilon>0$ and 
sufficiently big $m$ we have 
\[
b^{2/m}
=u^*u+u^*v+v^*u+v^*v
\leq 4Nb+\epsilon(8\lVert\sigma\rVert+4\epsilon)e_{A_\#}.
\]


In other words, $g_m(b)\geq 0$ for a continuous function
$g_m:\mathbb{R}_+\to\mathbb{R}:t\mapsto
4Nt+\epsilon(8\lVert\sigma\rVert+4\epsilon)-t^{2/m}$. Now choose $\epsilon>0$ such
that $M:=\epsilon(8\lVert\sigma\rVert+4\epsilon)<1$.  By the spectral mapping theorem
\cite[theorem 6.4.2]{HelLectAndExOnFuncAn} we get
$g_m(\operatorname{sp}_{A_\#}(b))
=\operatorname{sp}_{A_\#}(g_m(b))\subset\mathbb{R}_+$.
It is routine to check that $g_m$ has only one extreme point
...
g_m(t)=+\infty$, for sufficiently big $m$ the function $g_m$ has exactly
...
$m$ it holds that $\operatorname{sp}_{A_\#}(b)\subset T_{t_{1,m},t_{2,m}}$, 
where $T_{\alpha, \beta}:=\{t\in\mathbb{R} : 0\leq t \leq \alpha \vee t \geq \beta\}$. 
Since $\lim_{m\to\infty} t_{0,m}=0$, $\lim_{m\to\infty} t_{1,m}=0$ too. 
Since $t^{2/m}\to 1$ as $m\to\infty$ for every $t>0$, the functions $g_m$
converge to $g_\infty:t\mapsto 4Nt+M-1$ uniformly on compact subsets
of $(0,+\infty)$. The only zero of $g_\infty$ is $\theta:=(1-M)/(4N)>0$
and $g_\infty'\equiv 4N>0$, so $\lim_{m\to\infty}t_{2,m}=\theta$. Hence for
every $\delta\in(0,\theta)$ and all sufficiently big $m$ we have
$t_{1,m}<\delta$ and $t_{2,m}>\theta-\delta$, and therefore
$\operatorname{sp}_{A_\#}(b)\subset T_{\delta,\theta-\delta}$. Since $\delta$ is
arbitrary and the spectrum is closed,
$\operatorname{sp}_{A_\#}(b)\subset T_{0,\theta}$.

Consider a continuous function $h:\mathbb{R}_+\to\mathbb{R}:t\mapsto\min(1, t/\theta)$,
then from lemma~\ref{ContFuncCalcOnIdealOfCStarAlg} we get an idempotent
$p=h(b)=\operatorname{RCont}_b^0(h)\in I$ such that $\lVert
p\rVert=\sup_{t\in\operatorname{sp}_{A_\#}(b)}|h(t)|\leq 1$. Therefore, $p$ is a
self-adjoint idempotent. Since $h(t)t=th(t)=t$ for all $t\in
\operatorname{sp}_{A_\#}(b)$, we have $bp=pb=b$. The last equality implies
\[
0=(e_{A_\#}-p)b(e_{A_\#}-p)
=\sum_{d\in B_I}
    {(\lVert\sigma_d\rVert d(e_{A_\#}-p))}^*\lVert\sigma_d\rVert d(e_{A_\#}-p).
\]
Since the right-hand side of this equality is non-negative, $d=dp$ for all
...
for a self-adjoint idempotent $p\in I$.
\end{proof}

It is worth pointing out here that in relative theory there is no such description
...
\cite[section 6]{LykProjOfBanAndCStarAlgsOfContFld} for a nice overview of the
...
\cite[theorem 4.7.79]{HelBanLocConvAlg}. Therefore, $I$ has a right identity
...
\cite[chapter IV,\S\S 2--3]{HelHomolBanTopAlg}.
...
we may restrict the investigation of injective closed ideals to the case of $C^*$-algebras
...
topologically~$\rangle$ injective as an $A$-module iff $I$ is $\langle$~metrically
/ topologically~$\rangle$ injective as an $I$-module.
\end{proposition}
\begin{proof} Note that any closed two-sided ideal $I$ of a $C^*$-algebra $A$ is again
a $C^*$-algebra with a contractive approximate identity \cite[theorem 4.7.79]{HelBanLocConvAlg}. Therefore, $I$ is faithful as a right $I$-module. Now
...
players here. In an attempt to find a purely algebraic definition of
$W^*$-algebras Kaplansky introduced this class of $C^*$-algebras
...
but is strictly larger. Note that for the case of commutative $C^*$-algebras the
property of being an $AW^*$-algebra is equivalent to  $\operatorname{Spec}(A)$
being a Stonean space \cite[theorem 1.7.1]{BerbBaerStarRings}. The main
...
If $A$ is metrically injective as an $A$-module, then it has a norm one left identity
by proposition~\ref{MetTopInjOfId}. But $A$ also has a contractive approximate
identity  \cite[theorem 4.7.79]{HelBanLocConvAlg}, therefore $A$ is unital.
Now by a result of Hamana  \cite[proposition 2]{HamInjEnvBanMod} the
...
The converse was proved by Takesaki in \cite[theorem 2]{TakHanBanThAndJordDecomOfModMap}. Although only two-sided Banach modules were treated there, the
...
as a consequence it cannot contain  $\mathcal{B}(\ell_2(\mathbb{N}_n))$ as a
${}^*$-subalgebra for arbitrarily big $n\in\mathbb{N}$.
\end{proposition}
\begin{proof} By Gelfand-Naimark's theorem \cite[theorem 4.7.57]{HelBanLocConvAlg} there exists a Hilbert space $H$ and an isometric ${}^*$-homomorphism
...
injective $A$-module, $\rho$ has a left inverse $A$-morphism $\tau$.
...
$E$. As any Banach lattice $E$ has the l.u.st.\ property \cite[theorem 17.1]{DiestAbsSumOps}, then so does $A$, because the l.u.st.\ property is inherited by
complemented subspaces.

Assume $A$ contains $\mathcal{B}(\ell_2(\mathbb{N}_n))$ as a ${}^*$-subalgebra for
arbitrarily big $n\in\mathbb{N}$. In fact, this copy of
$\mathcal{B}(\ell_2(\mathbb{N}_n))$ is $1$-complemented in $A$
\cite[lemma 2.1]{LauLoyWillisAmnblOfBanAndCStarAlgsOfLCG}. Therefore, we have
...
\cite[proposition IV.1.4.3]{BlackadarOpAlg} they can be treated as closed
...
\cite[section IV.1.4]{BlackadarOpAlg}. 
...
Hilbert space. Then $\mathcal{B}(H)$ is topologically injective as a
$\mathcal{B}(H)$-module. 
\end{proposition}
\begin{proof} Note that
$\mathcal{B}(H)\isom{\mathbf{mod}_1\text{-}\mathcal{B}(H)}{\mathcal{N}(H)}^*$, and
...
$n\in\mathbb{N}$, then $M_n(C(K))$ is a topologically injective
$M_n(C(K))$-module.
\end{proposition}
...
\cite[corollary 2.4.4]{BroOzaCStarAlgFinDimApprox}, then by
\cite[theorem 3.1]{HaaNucCStarAlgAmen} this algebra is relatively amenable and
even $1$-relatively amenable \cite[example 2]{RundeAmenConstFour}. Since
...
$\bigoplus_\infty \{M_n(\mathbb{C}_s):s\in K \}$ is topologically injective as an
$M_n(C(K))$-module.
...
$M_n(C(K))$ is a retract of the topologically injective $M_n(C(K))$-module
$\bigoplus_\infty \{M_n(\mathbb{C}_s):s\in K \}$ in $\mathbf{mod}\text{-}M_n(C(K))$.
Finally, from proposition~\ref{RetrMetCTopInjIsMetCTopInj} we conclude that
$M_n(C(K))$ is a topologically injective $M_n(C(K))$-module.
\end{proof}
...
Therefore, $M_{n_\lambda}(C(K_\lambda))$ is faithful as an
$M_{n_\lambda}(C(K_\lambda))$-module. It is also topologically injective as an
$M_{n_\lambda}(C(K_\lambda))$-module by proposition~\ref{CKMatrixModTopInj}.
Now the topological injectivity of the $A$-module $A$ follows from paragraph
...
because $K_\lambda$ is a Stonean space \cite[theorem 1.7.1]{BerbBaerStarRings}. Therefore, $M_{n_\lambda}(C(K_\lambda))$ is an $AW^*$-algebra too
\cite[corollary 9.62.1]{BerbBaerStarRings}. Finally, $A$ is an $AW^*$-algebra
as a $\bigoplus_\infty$-sum of such algebras \cite[proposition 1.10.1]{BerbBaerStarRings}.
\end{proof}
...
By considering flatness we finalize this lengthy investigation of closed ideals of
$C^*$-algebras.

\begin{proposition}\label{IdealofCstarAlgisMetTopFlat} Let $I$ be a closed left ideal
of a $C^*$-algebra $A$. Then $I$ is metrically and topologically flat as an
$A$-module.
\end{proposition}
\begin{proof} From \cite[proposition 4.7.78]{HelBanLocConvAlg} it follows that
$I$ has a right contractive approximate identity. It remains to apply
...
complemented in some $L_1$-space \cite[corollary 23.2.1(2)]{DefFloTensNorOpId}. Since $A$ isometrically embeds in its second dual via $\iota_{A}$, we
may regard $A$ as a closed subspace of some $L_1$-space. The latter space is
weakly sequentially complete \cite[corollary III.C.14]{WojBanSpForAnalysts}.
...
noted above, $A$ is finite dimensional, so
...
\cite[theorem III.1.1]{DavCSatrAlgByExmpl}.
...
\end{enumerate}
\end{proposition}
\begin{proof} We may regard $I$ as a closed  ideal of the unitization $A_\#$ of $A$. Since
$I$ is a two-sided ideal, it has a contractive approximate identity
${(e_\nu)}_{\nu\in N}$ such that $0\leq e_\nu\leq e_{A_\#}$
\cite[theorem 4.7.79]{HelBanLocConvAlg}. As a 
corollary $\sup_{\nu\in N}\lVert e_{A_\#}-e_\nu\rVert\leq 1$. Since $I$ has an 
approximate identity, we also have $A_{ess}:=\operatorname{cl}_A(IA)=I$. 
Since $I$ is a two-sided ideal, $A/I$ is a $C^*$-algebra 
\cite[theorem 4.7.81]{HelBanLocConvAlg}.

Assume $A$ is a metrically flat $I$-module. Since 
$\sup_{\nu\in N}\lVert e_{A_\#}-e_\nu\rVert\leq 1$, paragraph $(ii)$ of
proposition~\ref{DualBanModDecomp} tells us that ${(A/A_{ess})}^*={(A/I)}^*$ is 
a retract of $A^*$ in $\mathbf{mod}_1\text{-}I$. Now from
propositions~\ref{MetCTopFlatCharac} and~\ref{RetrMetCTopInjIsMetCTopInj} it
follows that $A/I$ is a metrically flat $I$-module. Since this is an annihilator
module, from proposition~\ref{MetTopFlatAnnihModCharac} it follows that 
...
ideal $I= \{0 \}$, $\operatorname{dim}(A)=1$. Conversely, 
...
In this section we apply the general results on closed ideals obtained above to classical
...
\cite[theorems II.1.6, II.1.8]{TakThOpAlgVol1}) will be of use here. 
...
\begin{enumerate}[label = (\roman*)]
    ...
    flat as a $\mathcal{B}(H)$-module;

    \item $\mathcal{B}(H)$ is metrically or topologically projective or flat as
    a $\mathcal{K}(H)$-module iff $H$ is finite dimensional;

    \item $\mathcal{B}(H)$ is topologically injective as a $\mathcal{B}(H)$- or
    $\mathcal{K}(H)$-module iff $H$ is finite dimensional;

    \item $\mathcal{B}(H)$ is metrically injective as a $\mathcal{B}(H)$- or
    $\mathcal{K}(H)$-module iff $\dim(H)\leq 1$.
\end{enumerate}
\end{proposition}
\begin{proof} $(i)$ Since $\mathcal{B}(H)$ is a unital algebra, it is metrically
and topologically projective as a $\mathcal{B}(H)$-module by
...
proposition~\ref{CStarAlgIsTopFlatOverItsIdeal} the module $\mathcal{B}(H)$ is
neither topologically nor metrically flat as a $\mathcal{K}(H)$-module. Both
...
$\mathcal{B}(\ell_2(\mathbb{N}_n))$ as a ${}^*$-subalgebra for all
$n\in\mathbb{N}$. By proposition~\ref{TopInjIdHaveLUST} we get that
$\mathcal{B}(H)$ is not topologically injective as a $\mathcal{B}(H)$-module. The
...
injective as a $\mathcal{B}(H)$-module. Now from paragraph $(i)$
of proposition~\ref{MetTopInjUnderChangeOfAlg} we get that $\mathcal{B}(H)$ is not
metrically injective as a $\mathcal{K}(H)$-module. If $\dim(H)\leq 1$, both claims
obviously follow from proposition~\ref{MetInjCStarAlgCharac}.
\end{proof}
...
\begin{enumerate}[label = (\roman*)]
    ...
    as a $\mathcal{B}(H)$- or $\mathcal{K}(H)$-module;

    \item $\mathcal{K}(H)$ is metrically or topologically projective as
    a $\mathcal{B}(H)$- or $\mathcal{K}(H)$-module iff $H$ is finite dimensional;

    \item $\mathcal{K}(H)$ is topologically injective as a $\mathcal{B}(H)$- 
    or $\mathcal{K}(H)$-module iff $H$ is finite dimensional;

    \item $\mathcal{K}(H)$ is metrically injective as a $\mathcal{B}(H)$- 
...
$H$. Since $\mathcal{K}(H)$ is a two-sided ideal of $A$, the result follows
...
\begin{enumerate}[label = (\roman*)]
    ...
    a $\mathcal{B}(H)$- or $\mathcal{K}(H)$-module;
    ...
    as a $\mathcal{B}(H)$- or $\mathcal{K}(H)$-module iff $\dim(H)\leq 1$.
\end{enumerate}
\end{proposition}
\begin{proof} Let $A$ be either $\mathcal{B}(H)$ or $\mathcal{K}(H)$.

$(i)$ Note that $\mathcal{N}(H)\isom{\mathbf{mod}_1\text{-}A}{\mathcal{K}(H)}^*$, 
...
$\mathcal{B}(H)\isom{\mathbf{mod}_1\text{-}A}{\mathcal{N}(H)}^*$, so from
proposition~\ref{MetCTopFlatCharac} and paragraph $(iii)$ of
proposition~\ref{KHAndBHModBH} we get that $\mathcal{N}(H)$ is not topologically
flat as an $A$-module. Both results regarding projectivity follow from
...
injective. Since $\mathcal{B}(H)\isom{\mathbf{mod}_1\text{-}A}{\mathcal{N}(H)}^*$,
from proposition~\ref{MetCTopFlatCharac} we get that $\mathcal{N}(H)$ is not
metrically flat as an $A$-module. By proposition~\ref{MetTopProjIsMetTopFlat}, it
is not metrically projective as an $A$-module. If $\dim(H)\leq 1$, then
...
a $\mathcal{K}(H)$-module \cite[theorem 7.1.27]{HelBanLocConvAlg}, so from
proposition 7.1.13 in~\cite{HelBanLocConvAlg} we get that
$\mathcal{N}(H)\isom{\mathcal{K}(H)\text{-}\mathbf{mod}_1}H\projtens H^*$ is also
relatively projective as a $\mathcal{K}(H)$-module. By theorem IV.2.16
...
\cite[proposition 7.1.40]{HelBanLocConvAlg}, 
so $\mathcal{N}(H)\isom{\mathbf{mod}_1\text{-}\mathcal{K}(H)}{\mathcal{K}(H)}^*$
and  $\mathcal{B}(H)\isom{\mathbf{mod}_1\text{-}\mathcal{K}(H)}{\mathcal{N}(H)}^*$ are
relatively injective $\mathcal{K}(H)$-modules. From
\cite[proposition 2.2.8 (i)]{RamsHomPropSemgroupAlg} we know that a Banach
algebra relatively injective over itself as a right module necessarily has a
left identity. Therefore, $\mathcal{K}(H)$ is not a relatively injective
$\mathcal{K}(H)$-module for infinite dimensional $H$. If $H$ is finite
dimensional, then the $\mathcal{K}(H)$-module $\mathcal{K}(H)$ is relatively
injective because $\mathcal{K}(H)=\mathcal{B}(H)$ and $\mathcal{B}(H)$ is
a relatively injective $\mathcal{K}(H)$-module as was shown above. By corollary
5.5.64 from~\cite{DalBanAlgAutCont} the algebra $\mathcal{K}(H)$ is relatively
amenable, so all its left modules are relatively flat \cite[theorem 7.1.60]{HelBanLocConvAlg}. In particular $\mathcal{B}(H)$ is a relatively flat
$\mathcal{K}(H)$-module. From \cite[exercise V.2.20]{HelHomolBanTopAlg} we
know that $\mathcal{B}(H)$ is not relatively projective as
a $\mathcal{K}(H)$-module when $H$ is infinite dimensional. If $H$ is finite
dimensional, then $\mathcal{B}(H)$ is a relatively projective
$\mathcal{K}(H)$-module because $\mathcal{B}(H)=\mathcal{K}(H)$ and
$\mathcal{K}(H)$ is a relatively projective $\mathcal{K}(H)$-module as was shown
above.

$(ii)$ From proposition~\ref{KHAndBHModBH} paragraph $(i)$ and
proposition~\ref{MetProjIsTopProjAndTopProjIsRelProj} it follows that
$\mathcal{B}(H)$ is a relatively projective $\mathcal{B}(H)$-module. From
\cite[propositions 2.3.3, 2.3.4]{RamsHomPropSemgroupAlg} we know that
$\langle$~an essential relatively projective / a faithful relatively
injective~$\rangle$ module over an ideal of a Banach algebra 
is $\langle$~relatively projective / relatively injective~$\rangle$ over 
the algebra itself. Since $\mathcal{K}(H)$ and $\mathcal{N}(H)$ are essential 
and faithful $\mathcal{K}(H)$-modules, from the results of the previous paragraph
...
\cite[proposition 7.1.40]{HelBanLocConvAlg} all aforementioned modules are
relatively flat $\mathcal{B}(H)$-modules. In particular
$\mathcal{B}(H)\isom{\mathbf{mod}_1\text{-}\mathcal{B}(H)}{\mathcal{N}(H)}^*$ is
a relatively injective $\mathcal{B}(H)$-module.
\end{proof}

The results of this section are summarized in the following three tables. Each cell
contains a condition under which the respective module has the respective
property and propositions where this result is proved. We use the ``?'' symbol to indicate
...
ones: flatness of $\mathcal{K}(H)$ as a $\mathcal{B}(H)$- or
$\mathcal{K}(H)$-module, injectivity of $\mathcal{N}(H)$ as a $\mathcal{B}(H)$- or
$\mathcal{K}(H)$-module, projectivity and flatness of the $\mathcal{B}(H)$-module
$\mathcal{B}(H)$. In the remaining cases $H$ needs to be at least finite
dimensional in order for these properties to be equivalent in metric, topological
and relative theory.
...
\]
for $a\in \ell_\infty(\Lambda)$ or $a\in c_0(\Lambda)$ and $z\in\mathbb{C}_\lambda$. 
...
Since $\pi\sigma=1_{\mathbb{C}_\lambda}$, $\mathbb{C}_\lambda$ is a retract
of $A_+$ in $A\text{-}\mathbf{mod}_1$. From propositions~\ref{UnitalAlgIsMetTopProj}
...
is metrically and topologically injective as an $A$-module. Now metric and
topological injectivity of $\mathbb{C}_\lambda$ follow from the isomorphism
$\mathbb{C}_\lambda\isom{\mathbf{mod}_1\text{-}A}\mathbb{C}_\lambda^*$.
\end{proof}
...
\begin{enumerate}[label = (\roman*)]
    ...
    and flat as an $\ell_\infty(\Lambda)$-module;

    \item $\ell_\infty(\Lambda)$ is metrically or topologically projective 
    or flat as a $c_0(\Lambda)$-module iff $\Lambda$ is finite;

    \item $\ell_\infty(\Lambda)$ is metrically and topologically injective as
    an $\ell_\infty(\Lambda)$- and $c_0(\Lambda)$-module.
\end{enumerate}
\end{proposition}
\begin{proof} $(i)$ Since $\ell_\infty(\Lambda)$ is a unital algebra, it is
metrically and topologically projective as an $\ell_\infty(\Lambda)$-module by
proposition~\ref{UnitalAlgIsMetTopProj}. The results on flatness follow from
proposition~\ref{MetTopProjIsMetTopFlat}.
...
a $c_0(\Lambda)$-module. Both claims regarding projectivity follow from
...
it follows that $\ell_\infty(\Lambda)$ is metrically injective as an $A$-module.
...
\begin{enumerate}[label = (\roman*)]
    ...
    an $\ell_\infty(\Lambda)$- or $c_0(\Lambda)$-module;

    \item $c_0(\Lambda)$ is metrically or topologically projective as
    an $\ell_\infty(\Lambda)$- or $c_0(\Lambda)$-module iff $\Lambda$ is finite;
...
$c_0(\Lambda)$ is a two-sided ideal of $A$, the result follows from
...
\begin{enumerate}[label = (\roman*)]
    ...
    flat as an $\ell_\infty(\Lambda)$- or $c_0(\Lambda)$-module;
\end{enumerate}
\end{proposition}
\begin{proof} Let $A$ be either $\ell_\infty(\Lambda)$ or $c_0(\Lambda)$.

$(i)$ Note that $\ell_1(\Lambda)\isom{\mathbf{mod}_1\text{-}A}{c_0(\Lambda)}^*$, so the
...
projective as an $A$-module. Topological projectivity follows from
...
\begin{enumerate}[label = (\roman*)]
    ...
    injective or flat as an $\ell_\infty(\Lambda)$- or $c_0(\Lambda)$-module iff
    $\Lambda$ is finite;

    \item $\ell_p(\Lambda)$ is metrically projective,
    injective or flat as an $\ell_\infty(\Lambda)$- or $c_0(\Lambda)$-module iff
    $\Lambda$ has at most one element;
\end{enumerate}
\end{proposition}
\begin{proof} Let $A$ be either $\ell_\infty(\Lambda)$ 
or $c_0(\Lambda)$. Then $A$ is an $\mathscr{L}_\infty^g$-space.

$(i)$ Since $\ell_p(\Lambda)$ is reflexive for $1<p<+\infty$, from 
corollary~\ref{NoInfDimRefMetTopProjInjFlatModOverMthscrL1OrLInfty} it follows 
that $\ell_p(\Lambda)$ is necessarily finite dimensional if it is 
topologically projective, injective or flat. This is equivalent to $\Lambda$
being finite. If $\Lambda$ is finite, then
$\ell_p(\Lambda)\isom{A\text{-}\mathbf{mod}}\ell_1(\Lambda)$ and
$\ell_p(\Lambda)\isom{\mathbf{mod}\text{-}A}\ell_1(\Lambda)$, so topological
projectivity, injectivity and flatness follow from
proposition~\ref{c0AndlInftyModl1}.

$(ii)$ The result follows from \cite[theorem 4.5]{NemMetProjInjFlatModLp}.
\end{proof}
...
\cite[theorem IV.5.26]{HelHomolBanTopAlg} and admits a contractive approximate
identity, so by \cite[theorem 7.1.60]{HelBanLocConvAlg} all essential
...
\cite[proposition 7.1.40]{HelBanLocConvAlg}. By the same proposition,
$\ell_1(\Lambda)\isom{\mathbf{mod}_1\text{-}c_0(\Lambda)}{c_0(\Lambda)}^*$ and
$\ell_{p^*}(\Lambda)\isom{\mathbf{mod}_1\text{-}c_0(\Lambda)}{\ell_p(\Lambda)}^*$ for
...
has a left identity. Therefore, $c_0(\Lambda)$ is not a relatively injective
$c_0(\Lambda)$-module for infinite $\Lambda$. If $\Lambda$ is finite, then
the $c_0(\Lambda)$-module $c_0(\Lambda)$ is relatively injective because
$c_0(\Lambda)=\ell_\infty(\Lambda)$ and $\ell_\infty(\Lambda)$ is a relatively
injective $c_0(\Lambda)$-module as was shown above. From
\cite[corollary V.2.16(II)]{HelHomolBanTopAlg} we know that
$\ell_\infty(\Lambda)$ is not relatively projective as a $c_0(\Lambda)$-module
provided $\Lambda$ is infinite. If $\Lambda$ is finite, then
$\ell_\infty(\Lambda)$ is a relatively projective $c_0(\Lambda)$-module because
$\ell_\infty(\Lambda)=c_0(\Lambda)$ and $c_0(\Lambda)$ is a relatively projective
$c_0(\Lambda)$-module as was shown above. Finally, $c_0(\Lambda)$ is a
$C^*$-algebra, so it is relatively amenable \cite[theorem 7.1.87]{HelBanLocConvAlg}, and therefore every left Banach $c_0(\Lambda)$-module is relatively
flat \cite[theorem 7.1.60]{HelBanLocConvAlg}; in particular so
is $\ell_\infty(\Lambda)$.
Propositions~\ref{OneDimlInftyc0ModMetTopProjIngFlat},
...
In \cite[theorem 4.4]{NemANoteOnRelInjC0ModC0} it was shown 
that the $\ell_\infty(\Lambda)$-module $c_0(\Lambda)$ is relatively injective. 
From \cite[propositions 2.3.3, 2.3.4]{RamsHomPropSemgroupAlg} we know that
$\langle$~an essential relatively projective / a faithful relatively
...
is essential and faithful, hence by the results of 
the previous paragraph $\ell_p(\Lambda)$ is relatively projective and
injective as an $\ell_\infty(\Lambda)$-module. Since the $c_0(\Lambda)$-module 
$c_0(\Lambda)$ is essential and relatively projective, analogously 
...
flat \cite[proposition 7.1.40]{HelBanLocConvAlg}. As a consequence, 
$\ell_\infty(\Lambda)\isom{\mathbf{mod}_1\text{-}\ell_\infty(\Lambda)} {\ell_1(\Lambda)}^*$ 
is a relatively injective $\ell_\infty(\Lambda)$-module.
Propositions~\ref{OneDimlInftyc0ModMetTopProjIngFlat},
~\ref{MetProjIsTopProjAndTopProjIsRelProj},
~\ref{MetInjIsTopInjAndTopInjIsRelInj}
and~\ref{MetFlatIsTopFlatAndTopFlatIsRelFlat} give the result for modules
...
The results of this section are summarized in the following three tables. Each cell
contains a condition under which the respective module has the respective
property and propositions where this result is proved.
...
topological theories. For example, $\ell_1(\Lambda)$ is projective, injective and
flat as an $\ell_\infty(\Lambda)$- or $c_0(\Lambda)$-module in all three theories.
...
\begin{scriptsize}
    ...
    \hline 
        $\ell_p(\Lambda)$ & 
        \begin{tabular}{@{}c@{}}
            $\operatorname{Card}(\Lambda)\leq 1$ \\ 
...
classical we mean modules $C_0(S)$, $M(S)$ and $L_p(S,\mu)$ for positive measures
$\mu\in M(S)$. The pointwise multiplication plays the role of the module action for
these modules.
...
consisting of measures strictly singular with respect to $\mu$. Then the well-known 
Lebesgue decomposition theorem can be stated 
...
We are obliged to emphasize here that we consider only finite Borel regular positive
measures. This shall simplify many considerations. For example, any atom of a 
regular measure on a locally compact Hausdorff space is a point
\cite[chapter 5, \S 5, exercise 7]{BourbElemMathIntegLivVI}. Since we consider
only finite measures, one cannot say that this section simply generalizes
the results of the previous one. Strictly speaking, these sections are different,
though their methods have much in common.

For a fixed point $s\in S$, by $\mathbb{C}_s$ we denote the left or right Banach
$C_0(S)$-module $\mathbb{C}$ with the module action defined by
\[
...
projective. Now from \cite[proposition 7.1.31]{HelBanLocConvAlg} we know that
...
$C_0(S)$-morphisms. Since $\pi\sigma=1_{\mathbb{C}_s}$, $\mathbb{C}_s$ is a
retract of ${C_0(S)}_+$ in $C_0(S)\text{-}\mathbf{mod}_1$. From
...
$\mathbb{C}_s$ is also a relatively projective $C_0(S)$-module.

$(ii)$ By \cite[theorem 7.1.87]{HelBanLocConvAlg} the algebra $C_0(S)$ is
relatively amenable. Since this algebra is a $C^*$-algebra, it is $1$-relatively 
amenable \cite[example 2]{RundeAmenConstFour}. Clearly, $\mathbb{C}_s$ is a
...
proposition~\ref{MetInjIsTopInjAndTopInjIsRelInj} it is topologically and
relatively injective too. It remains to note that
$\mathbb{C}_s\isom{\mathbf{mod}_1\text{-}C_0(S)}\mathbb{C}_s^*$. 
\end{proof}
...
\begin{enumerate}[label = (\roman*)]
    ...
    \item $C_0(S)$ is metrically injective as a $C_0(S)$-module iff $S$ is 
...
$\operatorname{Spec}(C_0(S))$ is homeomorphic to $S$ \cite[corollary 3.1.6]{HelHomolBanTopAlg}.
...
\cite[chapter IV,\S\S 2--3]{HelHomolBanTopAlg}~$\rangle$ the spectrum of
$C_0(S)$ is $\langle$~compact / compact / paracompact~$\rangle$. 

$(ii)$ The result on metric injectivity is a weakened version of
proposition~\ref{MetInjCStarAlgCharac}. The result on relative injectivity 
follows from~\cite[theorem 4.4]{NemANoteOnRelInjC0ModC0}. It remains to recall
...
that the $C_0(S)$-module $C_0(S)$ is metrically and topologically flat. By
...
that $1\leq p\leq+\infty$ and the $C_0(S)$-module $L_p(S,\mu)$ is relatively 
...
prove. From \cite[chapter 5, \S 5, exercise 7]{BourbElemMathIntegLivVI} we
know that any atom of $\mu$ is a point. Call it $s\in S$. Consider well-defined
linear maps $\pi:L_p(S,\mu)\to\mathbb{C}_s:f\mapsto f(s)$ and
$\sigma:\mathbb{C}_s\to L_p(S,\mu):z\mapsto z\delta_s$. One can easily
check that these maps are $C_0(S)$-morphisms and $\pi\sigma=1_{\mathbb{C}_s}$.
Therefore, $\mathbb{C}_s$ is a retract of $L_p(S,\mu)$ in $C_0(S)\text{-}\mathbf{mod}$.
By assumption, the latter module is relatively projective, so by
\cite[proposition 7.1.6]{HelBanLocConvAlg} the $C_0(S)$-module $\mathbb{C}_s$
is relatively projective. By paragraph $(i)$ of
proposition~\ref{OneDimC0SModMetTopRelProjIngFlat} we see that $s$ is an isolated
point of $S$.
\end{proof}
...
at least relatively projective. Now from \cite[theorem 2]{NemRelProjModLp} 
...
proposition~\ref{MetCTopFlatCharac} we get that
$M(S)\isom{\mathbf{mod}_1\text{-}C_0(S)}{C_0(S)}^*$ is metrically injective. Since
$M(S)\isom{\mathbf{mod}_1\text{-}C_0(S)}L_1(S,\mu)\bigoplus_1 M_s(S,\mu)$
$L_1(S,\mu)$ is a retract of $M(S)$ in $\mathbf{mod}_1\text{-}C_0(S)$. So by
...
$(iii)$ By \cite[theorem 7.1.87]{HelBanLocConvAlg} the algebra $C_0(S)$ is
relatively amenable. Since this algebra is a $C^*$-algebra, it is $1$-relatively 
amenable \cite[example 2]{RundeAmenConstFour}. Since $L_1(S,\mu)$ is 
an essential $C_0(S)$-module which is tautologically an $L_1$-space, by
proposition~\ref{MetTopEssL1FlatModAoverAmenBanAlg} this module is metrically
...
\begin{enumerate}[label = (\roman*)]
    \item $L_p(S,\mu)$ is a relatively injective and flat $C_0(S)$-module;
    it is a relatively projective $C_0(S)$-module iff $\mu$ is purely atomic,
    and all atoms are isolated points;

    \item $L_p(S,\mu)$ is a topologically projective $C_0(S)$-module iff $\mu$
    is purely atomic with finitely many atoms, and all atoms are isolated
    points; it is a topologically injective or flat $C_0(S)$-module iff $\mu$
    is purely atomic with finitely many atoms;

    \item $L_p(S,\mu)$ is a metrically projective $C_0(S)$-module iff $\mu$ is
    purely atomic with at most one atom, which is an isolated point; it is a
    metrically injective or flat $C_0(S)$-module iff $\mu$ is purely atomic
    with at most one atom.
\end{enumerate}
\end{proposition}
\begin{proof} $(i)$ By \cite[theorem 7.1.87]{HelBanLocConvAlg} the algebra
$C_0(S)$ is relatively amenable. Now from \cite[theorem 7.1.60]{HelBanLocConvAlg} it follows that $L_p(S,\mu)$ is relatively flat for all $1<p<+\infty$.
Note that $L_p(S,\mu)\isom{\mathbf{mod}_1\text{-}C_0(S)}{L_{p^*}(S,\mu)}^*$. Then from
\cite[proposition 7.1.42]{HelBanLocConvAlg} we get that $L_p(S,\mu)$ is
relatively injective for any $1<p<+\infty$. Now assume that $L_p(S,\mu)$ is
relatively projective. Then by \cite[theorem 2]{NemRelProjModLp}
...
$S_a^{\mu}$ the set of these atoms. Since $S_a^{\mu}$ is discrete
$C_0(S_a^{\mu})$ is relatively biprojective \cite[theorem 4.5.26]{HelHomolBanTopAlg}. Then $L_p(S,\mu)$ is a relatively projective $C_0(S_a^{\mu})$-module
...
from \cite[proposition 2.3.2(i)]{RamsHomPropSemgroupAlg}
we get that $L_p(S,\mu)$ is relatively projective as a $C_0(S)$-module.

$(ii)$ Assume that $L_p(S,\mu)$ is a topologically projective
or injective or flat $C_0(S)$-module. Since $L_p(S,\mu)$ is reflexive and
$C_0(S)$ is an $\mathscr{L}_\infty^g$-space, $L_p(S,\mu)$ is finite
dimensional by
corollary~\ref{NoInfDimRefMetTopProjInjFlatModOverMthscrL1OrLInfty}. The latter
is equivalent to the measure $\mu$ being purely atomic with finitely many
atoms. If in addition $L_p(S,\mu)$ is topologically projective, then it is
relatively projective by
proposition~\ref{MetProjIsTopProjAndTopProjIsRelProj}, so all atoms of $\mu$
are isolated points by proposition~\ref{AtomsOfRelProjLpMod}. On the other
hand, if $\mu$ is purely atomic with finitely many atoms, then
$L_p(S,\mu)$ is topologically isomorphic 
to $L_1(S,\mu)$ as a left or right $C_0(S)$-module. The latter module is 
topologically injective and flat by 
proposition~\ref{L1C0SModMetTopRelProjInjFlat}, and it is topologically
projective whenever all atoms of $\mu$ are isolated points. Hence, so
is $L_p(S,\mu)$.

$(iii)$ Suppose $L_p(S,\mu)$ is a metrically $\langle$~projective / injective / 
flat~$\rangle$ $C_0(S)$-module. Then by 
proposition $\langle$~\ref{MetProjIsTopProjAndTopProjIsRelProj} / 
\ref{MetInjIsTopInjAndTopInjIsRelInj} / 
\ref{MetFlatIsTopFlatAndTopFlatIsRelFlat}~$\rangle$ this module is topologically
$\langle$~projective / injective / flat~$\rangle$. By the previous paragraph the
measure $\mu$ is purely atomic with finitely many atoms, and in the projective
case all of them are isolated points.
From \cite[theorem 4.5]{NemMetProjInjFlatModLp} we know that the number of 
atoms is at most one. Conversely, if $\mu$ has at most one atom $s\in S$, 
then $L_p(S,\mu)$ is isometrically isomorphic 
to $\mathbb{C}_s$ as a $C_0(S)$-module. By 
proposition~\ref{OneDimC0SModMetTopRelProjIngFlat} this module is metrically 
injective and flat, and it is metrically projective whenever $s$ is an isolated
point.
\end{proof}
...
\end{enumerate}
\end{proposition}
\begin{proof} $(i)$ From \cite[theorem 3]{NemRelProjModLp} it follows 
that $\operatorname{supp}(\mu)$ is pseudocompact and $\mu$ is inner open 
regular. Since $\mu$ is regular and finite, it is 
normal \cite[proposition 9]{NemRelProjModLp}.
    
$(ii)$ Since
$L_\infty(S,\mu)\isom{\mathbf{mod}_1\text{-}C_0(S)}{L_1(S,\mu)}^*$, the result
immediately follows from proposition~\ref{MetCTopFlatCharac} and paragraph
...
algebra is relatively flat \cite[theorem 7.1.60]{HelBanLocConvAlg}. In
...
it is at least relatively projective. For arbitrary $s\in S$ consider the measure
$\mu=\delta_s$ and recall the decomposition
$M(S)\isom{C_0(S)\text{-}\mathbf{mod}_1}L_1(S,\mu)\bigoplus_1 M_s(S,\mu)$. Then
$L_1(S,\mu)$ is a retract of $M(S)$ in $C_0(S)\text{-}\mathbf{mod}_1$. So from
\cite[proposition 7.1.6]{HelBanLocConvAlg} we get that $L_1(S,\mu)$ is
a relatively projective $C_0(S)$-module. Since $s$ is the only atom of $\mu$
from proposition~\ref{L1C0SModMetTopRelProjInjFlat} it follows that $s$ is an
isolated point in $S$. Since $s\in S$ is arbitrary, $S$ is discrete.
Conversely, assume $S$ is discrete. Then $C_0(S)=c_0(S)$, and
$M(S)
\isom{C_0(S)\text{-}\mathbf{mod}_1}{C_0(S)}^*
\isom{C_0(S)\text{-}\mathbf{mod}_1}\ell_1(S)
\isom{C_0(S)\text{-}\mathbf{mod}_1}
...
$(ii)$ Since $M(S)\isom{\mathbf{mod}_1\text{-}C_0(S)}{C_0(S)}^*$, the result
immediately follows from proposition~\ref{MetCTopFlatCharac} and paragraph
$(iii)$ of proposition~\ref{C0SC0SModMetTopRelProjInjFlat}.

$(iii)$ By \cite[theorem 7.1.87]{HelBanLocConvAlg} the algebra $C_0(S)$ is
relatively amenable. Since this algebra is a $C^*$-algebra, it is $1$-relatively 
amenable \cite[example 2]{RundeAmenConstFour}. Note that $M(S)$ is 
an essential $C_0(S)$-module which as a Banach space is an $L_1$-space
\cite[discussion after proposition 2.14]{DalLauSecondDualOfMeasAlg}. Then by
proposition~\ref{MetTopEssL1FlatModAoverAmenBanAlg} this module is metrically
...
The results of this section are summarized in the following three tables. Each cell
contains a condition under which the respective module has the respective
property and propositions where this result is proved. We use the ``?'' symbol to 
indicate open problems, and the ``*'' symbol for partial results. Below is a short 
overview of unsolved problems of this section. We don't have a complete 
description of relatively and topologically injective $C_0(S)$-modules $C_0(S)$. 
It seems quite a challenge because to this day there is no standard category of 
functional analysis where topologically injective objects are fully understood. 
The question of relative projectivity of the $C_0(S)$-module $L_\infty(S,\mu)$ is 
rather old. It seems that even relative projectivity of $L_\infty(S,\mu)$ is a 
rare property. Our conjecture is that $\mu$ must be normal and $S$ must be 
hyper-Stonean. For metric and topological flatness 
of the $C_0(S)$-module $L_\infty(S,\mu)$ we conjecture that $\mu$ must have a 
compact support. 
...
\begin{scriptsize}
    ...
    \hline
        $L_p(S,\mu)$ & 
        \begin{tabular}{@{}c@{}}
            $\mu$\mbox{ is purely atomic with } \\
            \mbox{ at most one atom, which } \\ 
            \mbox{ is an isolated point} \\
            \mbox{\ref{LpC0SModMetTopRelProjIngFlat}} (iii)
        \end{tabular} & 
        \begin{tabular}{@{}c@{}}
            $\mu$\mbox{ is purely atomic } \\
            \mbox{ with at most one atom } \\ 
            \mbox{\ref{LpC0SModMetTopRelProjIngFlat}} (iii)
        \end{tabular} & 
        \begin{tabular}{@{}c@{}}
            $\mu$\mbox{ is purely atomic } \\
    ...
    \hline
        ...
        \begin{tabular}{@{}c@{}}
            $\mu$\mbox{ is purely atomic with } \\ 
            \mbox{ finitely many atoms, all } \\ 
            \mbox{ atoms are isolated points} \\
            \mbox{\ref{LpC0SModMetTopRelProjIngFlat}} (ii)
        \end{tabular} & 
        \begin{tabular}{@{}c@{}}
            $\mu$\mbox{ is purely atomic with } \\ 
            \mbox{ finitely many atoms } \\ 
            \mbox{\ref{LpC0SModMetTopRelProjIngFlat}} (ii)
        \end{tabular} & 
        \begin{tabular}{@{}c@{}}
            $\mu$\mbox{ is purely atomic with } \\ 
...
well known Haar's theorem \cite[section 15.8]{HewRossAbstrHarmAnalVol1} there
...
invariant, that is $m_G(sE)=m_G(E)$ for all $s\in G$ and $E\in \operatorname{Bor}(G)$. It is
called the left Haar measure of the group $G$. If $G$ is compact, we assume that 
$m_G(G)=1$. If $G$ is infinite and discrete, we choose $m_G$ as the counting measure.
For each $s\in G$ the map $m:\operatorname{Bor}(G)\to[0,+\infty]:E\mapsto m_G(Es)$ is also a
...
The group structure of $G$ allows us to introduce the Banach algebra structure on
$L_1(G)$. For given $f,g\in L_1(G)$ we define their convolution as
\[
...
the convolution product becomes a Banach algebra. The Banach algebra $L_1(G)$ has a 
...
The group structure of $G$ allows us to turn the Banach space of complex finite 
...
\]
for all $E\in \operatorname{Bor}(G)$. The Banach space $M(G)$ along with this convolution is 
a unital Banach algebra. The role of the identity is played by Dirac's delta 
measure $\delta_{e_G}$ supported on $e_G$. In fact $M(G)$ is a coproduct 
in $L_1(G)\text{-}\mathbf{mod}_1$ (but not in $M(G)\text{-}\mathbf{mod}_1$) of the 
closed two-sided ideal $M_a(G)$ of measures absolutely continuous with 
respect to $m_G$ and  the subalgebra $M_s(G)$ of measures singular with 
respect to $m_G$. Note that $M_a(G)\isom{M(G)\text{-}\mathbf{mod}_1}L_1(G)$ 
...
over $L_1(G)$ and $M(G)$. The Banach algebra $L_1(G)$ can be 
regarded as a closed two-sided ideal  of $M(G)$ because of the isometric 
...
a left and right $M(G)$-module. Note that for $p=1$ and $\mu\in M_a(G)$ we 
...
a left and right $M(G)$-module too. This special choice of module structure 
nicely interacts with duality. Indeed, we have 
${(L_p(G),\convol_p)}^*\isom{\mathbf{mod}_1\text{-}M(G)}(L_{p^*}(G),\cdot_{p^*})$ 
for all $1\leq p<+\infty$. Finally, the Banach space $C_0(G)$ also becomes a left 
...
of $L_\infty(G)$ 
and ${(C_0(G),\cdot_\infty)}^*\isom{M(G)\text{-}\mathbf{mod}_1}(M(G),\convol)$.
...
considered with the compact open topology. Any character $\gamma\in\widehat{G}$ 
...
Gelfand \cite[theorems 2.7.2, 2.7.5]{KaniBanAlg}. Similarly, for 
...
One of the numerous definitions of an amenable group says that a locally compact 
group $G$ is amenable if there exists an $L_1(G)$-morphism of right 
modules $M:L_\infty(G)\to\mathbb{C}_{e_{\widehat{G}}}$ such 
that $M(\chi_G)=1$ \cite[section VII.2.5]{HelBanLocConvAlg}. We can even 
assume that $M$ is contractive \cite[remark 7.1.54]{HelBanLocConvAlg}.
...
Then $L_1(G)$ is a metrically and topologically flat $L_1(G)$-module, 
...
$L_1(G)$ is a metrically and topologically flat $L_1(G)$-module 
by proposition~\ref{MetTopFlatIdealsInUnitalAlg}. 
Since $L_\infty(G)\isom{\mathbf{mod}_1\text{-}L_1(G)}{L_1(G)}^*$, by 
...
and $\pi^+:{L_1(G)}_+\to\mathbb{C}_\gamma,\, f\oplus_1 w\mapsto f\cdot_{\gamma}1+w$. 
One can easily check 
that $\lVert\pi^+\rVert=\lVert\sigma^+\rVert=1$ 
and $\pi^+\sigma^+=1_{\mathbb{C}_\gamma}$. Therefore, $\mathbb{C}_\gamma$ is a 
retract of ${L_1(G)}_+$ in $L_1(G)\text{-}\mathbf{mod}_1$. From 
...
$(iii)\implies (i)$ Consider the $L_1(G)$-morphism
...
projective, there exists an $L_1(G)$-morphism $\sigma:\mathbb{C}_\gamma\to
...
$\sigma$ is an $L_1(G)$-morphism, for all $s,t\in G$ we have 
\[
f(s^{-1}t)
=L_s(f)(t)
=\lim_\nu L_s(e_\nu\convol \sigma(1))(t)
...
\end{enumerate}
\end{proposition}
\begin{proof} $(i)\implies (ii)$ Since $G$ is amenable, we have a 
...
$L_1(G)$-morphisms of right $L_1(G)$-modules. We shall check this for the operator
...
$L_\infty(G)$ in $\mathbf{mod}_1\text{-}L_1(G)$. From
propositions~\ref{LInfIsL1MetrInj} and~\ref{RetrMetCTopInjIsMetCTopInj} it 
follows that $\mathbb{C}_\gamma$ is metrically injective as an $L_1(G)$-module.
...
an $L_1(G)$-module, $\rho$ is a coretraction in $\mathbf{mod}\text{-}L_1(G)$. Denote
its left inverse morphism by $\pi$; then
$\pi(\overline{\gamma})=\pi(\rho(1))=1$. Consider a bounded linear functional
$M:L_\infty(G)\to\mathbb{C}_{e_{\widehat{G}}}:f\mapsto \pi(f\overline{\gamma})$. For all
...
$\mathbb{C}_\gamma^*\isom{\mathbf{mod}_1\text{-}L_1(G)}\mathbb{C}_\gamma$, so all
equivalences  follow from the three previous paragraphs and
...
$\mu\in M(G)$. In fact, this class of ideals in the case of commutative compact groups $G$
...
$L_1(G)$-modules. Consider the $L_1(G)$-morphism $\phi':L_1(G)\to
L_1(G):x\mapsto\phi(x\convol\mu)$. By Wendel's theorem \cite[theorem 1]{WendLeftCentrzrs}, there exists a measure $\nu\in M(G)$ such that
...
$e\in I\subset L_1(G)$, $\mu=\mu\convol e m_G\in M_a(G)$. 
Set $p=\mu\convol e\in I$; then $\mu=p m_G$.
\end{proof}

We conjecture that the left ideal $L_1(G)\convol \mu$ for an idempotent measure
$\mu\in M(G)$ is a metrically projective $L_1(G)$-module iff $\mu=p m_G$ where $p\in I$
and $\lVert p\rVert=1$.
...
is unital with a unit of norm $1$. By  proposition~\ref{UnIdeallIsMetTopProj} 
we see that $L_1(G)$ is metrically projective as an $L_1(G)$-module.

$(ii)\implies (iii)$ See
proposition~\ref{MetProjIsTopProjAndTopProjIsRelProj}.

$(iii) \implies (i)$ Clearly, $\delta_{e_G}$ is an idempotent measure. Since
$L_1(G)=L_1(G)\convol \delta_{e_G}$ is topologically projective, by
proposition~\ref{CommIdealByIdemMeasL1MetTopProjCharac} 
we have $\delta_{e_G}=f m_G$ for some $f\in L_1(G)$. This is possible only 
if $G$ is discrete.
\end{proof}

Note that the $L_1(G)$-module $L_1(G)$ is relatively projective for any locally
...
\end{proposition}
\begin{proof} 
$(i)\implies (ii)$ We have $M(G)\isom{L_1(G)\text{-}\mathbf{mod}_1}L_1(G)$ for
...
$(iii)\implies (i)$ Note that $M(G)\isom{\mathbf{Ban}_1}
L_1(G)\bigoplus_1 M_s(G)$, with the first summand being the essential part. 
Hence  $M_s(G)\isom{\mathbf{Ban}_1}M(G)/{M(G)}_{ess}$. Since $M(G)$ is 
topologically projective, proposition~\ref{MetTopProjModEssQuotIsProjBanSp} 
shows that $M_s(G)$ is isomorphic to $\ell_1(\Lambda)$ for some $\Lambda$.

Assume that $G$ is not discrete. By~\cite[section 19.26]{HewRossAbstrHarmAnalVol1} 
there exists a continuous probability measure $\mu\in M(G)$ singular with 
respect to $m_G$. Every measure absolutely continuous with respect to $\mu$ is 
singular with respect to $m_G$, so $L_1(G,\mu)$ is a closed subspace 
of $M_s(G)$. Since $\mu$ is continuous, the Banach space $L_1([0,1])$ 
isometrically embeds 
in $L_1(G,\mu)$~\cite[theorem 4.4.14]{DalDasLauStrBanSpContFuncAsDualSp}, and 
hence in $\ell_1(\Lambda)$. This is 
impossible~\cite[corollary 4.4.18]{DalDasLauStrBanSpContFuncAsDualSp}, so $G$ is
discrete.

$(iv)\implies (i)$ Note that $M(G)\isom{L_1(G)\text{-}\mathbf{mod}_1}
...
\end{proposition}
\begin{proof} We regard $M(G)$ as a left module over its closed two-sided ideal
$M_a(G)\isom{M(G)\text{-}\mathbf{mod}_1}L_1(G)$. Since $M_a(G)$ is an ideal, we
have $L_1(G)M(G)\subset M_a(G)$ and hence ${M(G)}_{ess}\subset M_a(G)$; since
$L_1(G)$ has a contractive approximate identity, it is an essential left
$L_1(G)$-module and hence $M_a(G)\subset{M(G)}_{ess}$. Therefore
${M(G)}_{ess}=M_a(G)$, and this module is topologically flat by
proposition~\ref{LInfIsL1MetrInj}.

By the Lebesgue decomposition theorem
$M(G)\isom{\mathbf{Ban}_1}M_a(G)\bigoplus_1 M_s(G)$, so the Banach space
$M(G)/{M(G)}_{ess}$ is isometrically isomorphic to $M_s(G)$. Since $M(G)$ is an
$L_1$-space, it is a fortiori an $\mathscr{L}_1^g$-space, and $M_s(G)$ is
$1$-complemented in $M(G)$, so $M_s(G)$ is an $\mathscr{L}_1^g$-space too
\cite[corollary 23.2.1(2)]{DefFloTensNorOpId}.

The algebra $L_1(G)$ has a two-sided contractive approximate identity, so
paragraph $(iii)$ of proposition~\ref{TopFlatModCharac} applies to the
$L_1(G)$-module $M(G)$ and shows that it is topologically flat.
\end{proof}
...
compact group, and $X$ be $\langle$~an essential / a faithful / an essential~$\rangle$
$L_1(G)$-module. Then
...
\end{enumerate}
\end{proposition}
\begin{proof} Recall that $L_1(G)\isom{L_1(G)\text{-}\mathbf{mod}_1}M_a(G)$ is a
...
essential and the $L_1(G)$-modules $C_0(G)$, $M(G)$, $L_p(G)$ for $1\leq p\leq
...
compact group. Then $M(G)$ is a metrically and topologically projective
$M(G)$-module. As a consequence, it is a metrically and topologically flat
$M(G)$-module.
\end{proposition} 
\begin{proof} Since $M(G)$ is a unital algebra, metric and topological
...
In this section we shall show that many modules of harmonic analysis fail to
...
contains a copy of $\ell_\infty(\mathbb{N})$ \cite[corollary 1.1.4]{RosOnRelDisjFamOfMeas}, and consequently a copy of $c_0(\mathbb{N})$. The Banach space
$L_1(G)$ is weakly sequentially complete \cite[corollary III.C.14]{WojBanSpForAnalysts}, so by corollary 5.2.11 in~\cite{KalAlbTopicsBanSpTh} it cannot contain
...
its complemented subspace $M_a(G)\isom{\mathbf{Ban}_1}L_1(G)$. By the previous
argument this is impossible. So $M(G)$ is not topologically injective as a Banach
...
${L_\infty(G)}^*\isom{\mathbf{Ban}_1}{L_1(G)}^{**}$ \cite[proposition B10]{DefFloTensNorOpId}. Therefore, if ${L_\infty(G)}^*$ is topologically injective as a
Banach space, then so is its retract $L_1(G)$. By the previous arguments this is
impossible, so ${L_\infty(G)}^*$ is not a topologically injective Banach space.

$(ii)$ If $C_0(G)$ is a retract of an $L_1$-space, then
$M(G)\isom{\mathbf{Ban}_1}{C_0(G)}^*$ is a retract of an $L_\infty$-space, 
so it must be a topologically injective Banach space. This contradicts 
paragraph $(i)$, so $C_0(G)$ is not a retract of an $L_1$-space. 
Note that $\ell_\infty(\mathbb{N})$ embeds in $L_\infty(G)$, hence so 
does $c_0(\mathbb{N})$. Therefore, if $L_\infty(G)$ is
a retract of an $L_1$-space, then there would exist an $L_1$-space containing a
copy of $c_0(\mathbb{N})$. This is impossible as already shown in paragraph
$(i)$.
\end{proof}
...
\begin{enumerate}[label = (\roman*)]
    ...
    projective, injective or flat $A$-modules.
\end{enumerate}
\end{proposition}
...
$(iii)$ Note that ${C_0(G)}^*\isom{\mathbf{mod}_1\text{-}A}M(G)$. Now the result 
...
$A$-module. Since $L_p(G)$ is finite dimensional, by paragraphs $(i)$ and
...
1 from~\cite{LyubIsomEmdbFinDimLp} for Banach spaces over the field $\mathbb{C}$: if
...
\end{proposition}
\begin{proof}
$(i)$ Since $G$ is finite, $C_0(G)=L_\infty(G)$. The result follows from
proposition~\ref{LInfIsL1MetrInj}.

$(ii)$ If $G$ is trivial, that is, $G= \{e_G \}$, then $L_p(G)=C_0(G)=L_1(G)$ and
the result follows from theorem~\ref{L1ModL1MetTopProjCharac}. If $G$ is non-trivial, then we recall
...
${C_0(G)}^*\isom{\mathbf{mod}_1\text{-}L_1(G)}M(G)\isom{\mathbf{mod}_1\text{-}L_1(G)}L_1(G)$,
${L_p(G)}^*\isom{\mathbf{mod}_1\text{-}L_1(G)}L_{p^*}(G)$ for $1\leq p^*<\infty$.
\end{proof}

It is worth mentioning here that if we considered all Banach spaces over the
...
\isom{L_1(\mathbb{Z}_2)\text{-}\mathbf{mod}_1}
...
for $1\leq p\leq \infty$ are topologically projective, injective and flat.
\end{proposition} 
\begin{proof}
...
Therefore, $i$ is an isomorphism in $L_1(G)\text{-}\mathbf{mod}$ and $\mathbf{mod}\text{-}L_1(G)$.
Similarly, one can show that $(L_\infty(G),\cdot_\infty)$ and $(L_p(G),\cdot_p)$
for $1<p\leq+\infty$ are isomorphic in $L_1(G)\text{-}\mathbf{mod}$ and
$\mathbf{mod}\text{-}L_1(G)$. Finally, one can easily check that $(L_1(G),\convol)$ and
$(L_\infty(G),\cdot_\infty)$ are isomorphic in $L_1(G)\text{-}\mathbf{mod}$ and
$\mathbf{mod}\text{-}L_1(G)$ via the 
...
Now we can summarize the results on homological properties of modules of harmonic
analysis in three tables. Each cell of the table contains a condition under
which the respective module has the respective property and propositions where this
result is proved. We shall mention that the results for modules $L_p(G)$ are valid for both
module actions $\convol_p$ and $\cdot_p$. The characterization and proofs for
homologically trivial modules $\mathbb{C}_\gamma$ in the case of relative theory are
the same as in
propositions~\ref{OneDimL1ModMetTopProjCharac}
and~\ref{OneDimL1ModMetTopInjFlatCharac}. As usual, we use ${}^{*}$ to
indicate that only a necessary condition is known. As we showed above, even
topological theory is too restrictive for $L_1(G)$ to be projective as an
...
confirms the important role of Banach geometry in metric and topological Banach
homology.
