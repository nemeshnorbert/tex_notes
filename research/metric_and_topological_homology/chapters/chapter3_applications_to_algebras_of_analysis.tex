...
\begin{enumerate}[label = (\roman*)]
    ...
    \item $L_p(S,\mu)$ is a metrically projective or injective or flat 
    $C_0(S)$-module iff $\mu$ is purely atomic with at most one atom,
...
is equivalent to the measure $\mu$ being purely atomic with finitely many atoms, and
all atoms being isolated points. On the other hand, if $\mu$ is purely atomic 
with finitely many atoms, all of which are isolated points, then
$L_p(S,\mu)$ is topologically isomorphic 
to $L_1(S,\mu)$ as a left or right $C_0(S)$-module. The latter module is 
topologically projective, injective and flat by 
proposition~\ref{L1C0SModMetTopRelProjInjFlat}. Hence, so is $L_p(S,\mu)$.
...
$\langle$~projective / injective / flat~$\rangle$. By the previous paragraph we get
that the measure $\mu$ is purely atomic with all atoms being isolated points.
From [\cite{NemMetProjInjFlatModLp}, theorem 4.5] we know that the number of 
atoms is at most one. Conversely, if $\mu$ has at most one atom $s\in S$ which 
is an isolated point, then $L_p(S,\mu)$ is isometrically isomorphic 
