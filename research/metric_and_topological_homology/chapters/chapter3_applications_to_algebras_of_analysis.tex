...
it holds that $\Vert p(a)\Vert=\Vert p|_{\operatorname{sp}_A(a)}\Vert_\infty$, so
...
$\Vert b^{1/m}\Vert
=\sup_{t\in\operatorname{sp}_{A_\#}(b)} t^{1/m}\leq\Vert b\Vert^{1/m}$. Then $\limsup_{m\to\infty}\Vert b^{1/m}\Vert\leq 1$. 
...
and $\Vert v^*v\Vert\leq 4\epsilon^2$. Since $u^*v+v^*u$ and $v^*v$ are self-adjoint, $u^*v+v^*u\leq 8\Vert\sigma\Vert\epsilon e_{A_\#}$ 
...
that $M:=\epsilon(8\Vert\sigma\Vert+4\epsilon)<1$.  By the spectral mapping theorem
