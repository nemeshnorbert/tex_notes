...
\end{proposition}
\begin{proof} Put $E:=P/P_{ess}$ and let $q:P\to E$ be the natural quotient
morphism. Since $a\cdot x\in P_{ess}$ for all $a\in A$ and $x\in P$, the
$A$-module $E$ is annihilator. Endow $\ell_1(B_E)$ with the annihilator
$A$-module structure and consider the strict coisometry $\pi:\ell_1(B_E)\to E$
well-defined by the equality $\pi(\delta_y)=y$ for all $y\in B_E$. Since $E$
and $\ell_1(B_E)$ are annihilator modules, $\pi$ is also an $A$-module map.
...
projective, there exists an $A$-morphism $\psi:P\to\ell_1(B_E)$ such
...
Since $\ell_1(B_E)$ is an annihilator module, $\psi(a\cdot x)=a\cdot\psi(x)=0$
...
unique operator $\overline{\psi}:E\to\ell_1(B_E)$ satisfying
$\overline{\psi}q=\psi$. For all $x\in P$ and $y\in P_{ess}$ we have
$\lVert\overline{\psi}(q(x))\rVert
=\lVert\psi(x+y)\rVert\leq\lVert\psi\rVert\lVert x+y\rVert$, and
therefore $\lVert\overline{\psi}\rVert\leq\lVert\psi\rVert$.
From $\pi\overline{\psi}q=\pi\psi=q$ and surjectivity of $q$ we
get $\pi\overline{\psi}=1_E$. Thus, the Banach space $E$ is
a $\langle$~$1$-retract / $C$-retract~$\rangle$ of $\ell_1(B_E)$.

It remains to note that, regarding Banach spaces as modules over the zero
algebra, we have
$\ell_1(B_E)\isom{\mathbf{Ban}_1}\mathbb{C}\projtens\ell_1(B_E)$, so by
proposition~\ref{MetCTopFreeMod} the Banach space $\ell_1(B_E)$ is
$\langle$~metrically / $1$-topologically~$\rangle$ projective. By
proposition~\ref{RetrMetCTopProjIsMetCTopProj} the Banach space $E$ is
$\langle$~metrically / $C$-topologically~$\rangle$ projective. Now
from $\langle$~\cite[proposition 3.2]{HelMetrFrQMod} / results
of~\cite{KotheTopProjBanSp}~$\rangle$ we get that $E$ is
...
$c$-topologically injective~$\rangle$ operator $\xi:Y\to X$ of Banach spaces
and an arbitrary bounded linear operator $\phi:Y\to J_{ann}$. Endow $X$ and $Y$
with the annihilator right $A$-module structure; then $\xi$ is an
...
that $\psi\xi=j\phi$ and $\langle$~$\lVert\psi\rVert\leq\lVert j\phi\rVert$ /
...
and $j\widetilde{\psi}\xi=\psi\xi=j\phi$ gives $\widetilde{\psi}\xi=\phi$
