...
In what follows, $A$ denotes a not necessarily unital Banach algebra. We
immediately start from the most important definitions in this book.

\begin{definition}[{\cite[definition 1.4]{HelMetrFrQMod};~\cite[definition 2.4]{WhiteInjmoduAlg}}]\label{MetCTopProjMod} 
An $A$-module $P$ is 
...
$\langle$~$\lVert\psi\rVert\leq\lVert\phi\rVert$ / 
$\lVert \psi\rVert\leq c C\lVert\phi\rVert$~$\rangle$. We say that an $A$-module 
$P$ is topologically projective if it is $C$-topologically projective for 
some $C\geq 1$.
\end{definition}

The task of constructing an $A$-morphism $\psi$ for given $A$-morphisms $\phi$ 
...
$\langle$~$\operatorname{Hom}_{A\text{-}\mathbf{mod}_1}(P,-)
:A\text{-}\mathbf{mod}_1\to\mathbf{Ban}_1$
/
$\operatorname{Hom}_{A\text{-}\mathbf{mod}}(P,-)
:A\text{-}\mathbf{mod}\to\mathbf{Ban}$~$\rangle$
...
In the category $\langle$~$A\text{-}\mathbf{mod}_1$ / $A\text{-}\mathbf{mod}$~$\rangle$ there
...
\begin{proposition}[{\cite[lemma 2.6]{WhiteInjmoduAlg}}]\label{MetCTopFreeMod} Let $\Lambda$ be an arbitrary 
...
Clearly, $\lVert y_\lambda\rVert\leq \lVert\phi\rVert$. Since $\xi$ is 
strictly $\langle$~coisometric / $c$-topologically surjective~$\rangle$, 
there exists $x_\lambda\in X$ such that $\xi(x_\lambda)=y_\lambda$
and $\lVert x_\lambda\rVert\leq K\lVert y_\lambda\rVert$ for $\langle$~$K=1$ / 
...
and $\lVert\psi\rVert\leq K\lVert\phi\rVert$. Thus, for a given $\phi$ and $\xi$
we have constructed an $A$-morphism $\psi$ such that $\xi\psi=\phi$ and
$\langle$~$\lVert\psi\rVert\leq \lVert\phi\rVert$ / $\lVert\psi\rVert\leq
c\lVert\phi\rVert$~$\rangle$. Hence, the $A$-module $A_\bullet\projtens
\ell_1(\Lambda)$ is $\langle$~metrically / $1$-topologically~$\rangle$
projective.
\end{proof}


\begin{proposition}\label{UnitalAlgIsMetTopProj} The left $A$-module $A_\bullet$
is $\langle$~metrically / $1$-topologically~$\rangle$ projective.
\end{proposition} 
\begin{proof} Consider the set $\Lambda=\mathbb{N}_1$. 
By proposition~\ref{MetCTopFreeMod} the 
$A$-module $A_\bullet\projtens\ell_1(\Lambda)$ is metrically 
and $1$-topologically projective. Now it remains to note that 
$A_\bullet\projtens\ell_1(\Lambda)
\isom{A\text{-}\mathbf{mod}_1}
A_\bullet\projtens \mathbb{C}
\isom{A\text{-}\mathbf{mod}_1}
A_\bullet$.
\end{proof}

\begin{proposition}[{\cite[lemma 2.7]{WhiteInjmoduAlg}}]\label{RetrMetCTopProjIsMetCTopProj} Any 
...
$\zeta\eta=1_{P}$ and $\lVert\zeta\rVert\lVert\eta\rVert\leq c$ 
for $\langle$~$c=1$ / $c\geq 1$~$\rangle$. Consider an arbitrary
strictly $\langle$~coisometric / $c'$-topologically surjective~$\rangle$ 
$A$-morphism $\xi:X\to Y$ and an arbitrary $A$-morphism $\phi:P\to Y$. 
Consider the $A$-morphism $\phi'=\phi\zeta$. Since $P'$ is 
$\langle$~metrically / $C$-topologically~$\rangle$ projective, there 
exists an $A$-morphism $\psi':P'\to X$ such that $\phi'=\xi\psi'$ 
and $\lVert\psi'\rVert \leq K\lVert\phi'\rVert$ 
for $\langle$~$K=1$ / $K=c' C$~$\rangle$. Now it is routine to check that 
for the $A$-morphism $\psi=\psi'\eta$ 
holds $\xi\psi=\phi$ and $\lVert\psi\rVert \leq cK\lVert\phi\rVert$. 
Thus, for a given $\phi$ and $\xi$ we have
constructed an $A$-morphism $\psi$ such that $\xi\psi=\phi$  and
$\langle$~$\lVert\psi\rVert\leq\lVert\phi\rVert$ / 
$\lVert \psi\rVert\leq c c' C\lVert\phi\rVert$~$\rangle$. Hence, $P$ 
...
\begin{proposition}[{\cite[proposition 2.10]{WhiteInjmoduAlg}}]\label{MetCTopProjModViaCanonicMorph} 
An $A$-module $P$ is
$\langle$~metrically / $C$-topologically~$\rangle$ projective iff $\pi_P^+$ is 
a $\langle$~$1$-retraction / $C$-retraction~$\rangle$ in $A\text{-}\mathbf{mod}$.
\end{proposition}
\begin{proof}
Suppose $P$ is 
$\langle$~metrically / $C$-topologically~$\rangle$ projective. Then consider 
the strictly $\langle$~coisometric / $1$-topologically surjective~$\rangle$
$A$-morphism $\pi_P^+$. Consider the lifting problem with $\phi=1_P$ 
and $\xi=\pi_P^+$. Then from the $\langle$~metric / $C$-topological~$\rangle$ 
projectivity of $P$ we get an $A$-morphism $\sigma^+$ such 
that $\pi_P^+\sigma^+=1_P$
and $\lVert\sigma^+\rVert\leq K$ for $\langle$~$K=1$ / $K=C$~$\rangle$.
Since $\lVert\pi_P^+\rVert\lVert \sigma^+\rVert\leq K$, 
we conclude that $\pi_P^+$ is a
$\langle$~$1$-retraction / $C$-retraction~$\rangle$ in $A\text{-}\mathbf{mod}$.
...
\begin{proposition}[{\cite[proposition 2.9]{WhiteInjmoduAlg}}]\label{MetProjIsTopProjAndTopProjIsRelProj} Every metrically
...
In other words, $P$ is a $C$-retract of the module $A_+\projtens E$ for some Banach
...
regarded as the category of left Banach modules over the zero algebra. As a
result, we get the definition of a $\langle$~metrically / topologically~$\rangle$
projective Banach space. All the results mentioned above hold for this type of
projectivity. Both types of projective objects have been described by now.
...
set $\Lambda$. Using a result of Grothendieck from~\cite{GrothMetrProjFlatBanSp},
...
\cite[proposition 3.2]{HelMetrFrQMod}. Thus, the zoo of projective Banach
spaces is wide but uniform.
...
$\pi_P:A\projtens\ell_1(B_P)\to P,\, a\projtens\delta_x\mapsto a\cdot x$ 
is a $\langle$~$1$-retraction / $C$-retraction~$\rangle$ in $A\text{-}\mathbf{mod}$.
\end{proposition} 
\begin{proof}
...
\[
\sigma^+(P)
=\sigma^+(\operatorname{cl}_P(AP))
\subset \operatorname{cl}_{A_+\projtens\ell_1(B_P)}(A\cdot\sigma^+(P))\subset
\]
\[
\operatorname{cl}_{A_+\projtens\ell_1(B_P)}(A\cdot(A_+\projtens\ell_1(B_P)))
=A\projtens\ell_1(B_P).
\] 
So we have a well-defined corestriction $\sigma:P\to A\projtens\ell_1(B_P)$
...
the natural projection $p_\lambda:P\to P_\lambda$. By
...
of $C$-retractions $\pi_\lambda:A_+\projtens\ell_1(S_\lambda)\to P_\lambda$, where $S_\lambda=B_{P_\lambda}$. 
It follows that
$\bigoplus_1 \{\pi_\lambda:\lambda\in\Lambda \}$ is a $C$-retraction in
$A\text{-}\mathbf{mod}$. As a result, $P$ is a $C$-retract of 
\[
\bigoplus\nolimits_1\left \{A_+\projtens
\ell_1(S_\lambda):\lambda\in\Lambda\right \} \isom{A\text{-}\mathbf{mod}_1}
\bigoplus\nolimits_1\left \{\bigoplus\nolimits_1 \{A_+:s\in S_\lambda
\}:\lambda\in\Lambda\right \}
\]
\[
\isom{A\text{-}\mathbf{mod}_1} \bigoplus\nolimits_1 \{A_+:s\in S \}
\isom{A\text{-}\mathbf{mod}_1} A_+ \projtens \ell_1(S)
\]
in $A\text{-}\mathbf{mod}$ where $S=\bigsqcup_{\lambda\in\Lambda}S_\lambda$. Clearly,
...
The property of being a metrically, topologically or relatively projective module 
puts some restrictions on the Banach geometric structure of the module.

\begin{proposition}[{\cite[proposition 2.1.1]{RamsHomPropSemgroupAlg}}]\label{MetTopRelProjModCompIdealPartCompl} Let $P$ 
...
in $P$.
\end{proposition}
\begin{proof} Since $A$ is $1$-complemented in $A_+$, $I$ 
...
Consider the operator $R=r\projtens 1_P$. Then $R|_{I\projtens P}=1_{I\projtens P}$ 
and $\lVert R\rVert\leq\lVert r\rVert$. Since $P$ 
is $\langle$~metrically / $C$-topologically / $C$-relatively~$\rangle$ 
projective, consider the $A$-morphism
$\varpi_P:A_+\projtens P\to P,\, a\projtens x\mapsto a\cdot x$. It is strictly
coisometric, and $x\mapsto e_{A_+}\projtens x$ is a bounded linear right inverse
for it of norm $1$, so $\varpi_P$ is in addition a $1$-relatively admissible
epimorphism. Lifting $1_P$ along $\varpi_P$ therefore gives an $A$-morphism
$\sigma^+:P\to A_+\projtens P$ with $\varpi_P\sigma^+=1_P$ of norm
$\langle$~at most $1$ / at most $C$ / at most $C$~$\rangle$. Now consider a 
bounded linear operator $p=\varpi_P R \sigma^+$. Clearly,
$\operatorname{Im}(p)=\varpi_P(R(\sigma^+(P)))
\subset\varpi_P(R(A_+\projtens P))
\subset \varpi_P(I\projtens P)
\subset \operatorname{cl}_P(I P)$.
Since $I$ is a right ideal of $A$ and $\sigma^+$ is an $A$-morphism, 
\[
...
$p(x)
=\varpi_P(R(\sigma^+(x)))
=\varpi_P(1_{I\projtens P}(\sigma^+(x)))
=\varpi_P(\sigma^+(x))
=x$. 
Thus, $p$ is a projection onto $\operatorname{cl}_P(I P)$ with norm at 
most $\lVert \sigma^+\rVert\lVert r\rVert$. Hence, $\operatorname{cl}_P(I P)$ 
...
be a $\langle$~metrically / $C$-topologically / $C$-relatively~$\rangle$ 
projective $A$-module. Then $P_{ess}$ 
is $\langle$~$1$-complemented / $C$-complemented / $C$-complemented~$\rangle$ 
in $P$.
\end{corollary}
...
\begin{proposition}\label{MetTopProjModEssQuotIsProjBanSp} Let $P$ be a
$\langle$~metrically / $C$-topologically~$\rangle$ projective $A$-module. Then
$P/P_{ess}$ is a $\langle$~metrically / $C$-topologically~$\rangle$ projective
Banach space. As a consequence,
$\langle$~$P/P_{ess}\isom{\mathbf{Ban}_1}\ell_1(\Lambda)$ /
$P/P_{ess}\isom{\mathbf{Ban}}\ell_1(\Lambda)$~$\rangle$ for some set $\Lambda$.
\end{proposition}
\begin{proof} Put $Q:=P/P_{ess}$ and let $q:P\to Q$ be the natural quotient
morphism. Since $a\cdot x\in P_{ess}$ for all $a\in A$ and $x\in P$, the
$A$-module $Q$ is annihilator. Endow $\ell_1(B_Q)$ with the annihilator
$A$-module structure and consider the strict coisometry $\pi:\ell_1(B_Q)\to Q$
well-defined by the equality $\pi(\delta_y)=y$ for all $y\in B_Q$. Since $Q$
and $\ell_1(B_Q)$ are annihilator modules, $\pi$ is also an $A$-module map.
Recall that strict coisometries are exactly the contractive strictly
$1$-topologically surjective operators, so $\pi$ belongs to the class of
morphisms considered in definition~\ref{MetCTopProjMod} for both types of
projectivity. Since $P$ is $\langle$~metrically / $C$-topologically~$\rangle$
projective, there exists an $A$-morphism $\psi:P\to\ell_1(B_Q)$ such
that $\pi\psi=q$ and $\langle$~$\lVert\psi\rVert\leq\lVert q\rVert$ /
$\lVert\psi\rVert\leq C\lVert q\rVert$~$\rangle$. As $\lVert q\rVert\leq 1$,
the norm of $\psi$ is $\langle$~at most $1$ / at most $C$~$\rangle$.

Since $\ell_1(B_Q)$ is an annihilator module, $\psi(a\cdot x)=a\cdot\psi(x)=0$
for all $a\in A$ and $x\in P$. By linearity and continuity $\psi$ vanishes on
$P_{ess}=\operatorname{cl}_P(AP)=\operatorname{Ker}(q)$, so there exists a
unique operator $\overline{\psi}:Q\to\ell_1(B_Q)$ satisfying
$\overline{\psi}q=\psi$. For all $x\in P$ and $n\in P_{ess}$ we have
$\lVert\overline{\psi}(q(x))\rVert
=\lVert\psi(x+n)\rVert\leq\lVert\psi\rVert\lVert x+n\rVert$, and
therefore $\lVert\overline{\psi}\rVert\leq\lVert\psi\rVert$.
From $\pi\overline{\psi}q=\pi\psi=q$ and surjectivity of $q$ we
get $\pi\overline{\psi}=1_Q$. Thus, the Banach space $Q$ is
a $\langle$~$1$-retract / $C$-retract~$\rangle$ of $\ell_1(B_Q)$.

It remains to note that, regarding Banach spaces as modules over the zero
algebra, we have
$\ell_1(B_Q)\isom{\mathbf{Ban}_1}\mathbb{C}\projtens\ell_1(B_Q)$, so by
proposition~\ref{MetCTopFreeMod} the Banach space $\ell_1(B_Q)$ is
$\langle$~metrically / $1$-topologically~$\rangle$ projective. By
proposition~\ref{RetrMetCTopProjIsMetCTopProj} the Banach space $Q$ is
$\langle$~metrically / $C$-topologically~$\rangle$ projective. Now
from $\langle$~\cite[proposition 3.2]{HelMetrFrQMod} / results
of~\cite{KotheTopProjBanSp}~$\rangle$ we get that $Q$ is
$\langle$~isometrically / topologically~$\rangle$ isomorphic
to $\ell_1(\Lambda)$ for some set $\Lambda$.
\end{proof}
...
idempotent theorem \cite[section 3.5]{KaniBanAlg}: every semisimple
...
\begin{enumerate}[label = (\roman*)]
    ...
    idempotent~$\rangle$ $p\in I$, then $I$ is a $\langle$~metrically / 
    $\lVert p\rVert$-topologically~$\rangle$ projective $A$-module;

    \item if $I$ is commutative, semisimple and $\operatorname{Spec}(I)$ is
    compact, then $I$ is a topologically projective $A$-module.
\end{enumerate}
\end{proposition}
\begin{proof} 
$(i)$ For $A$-module maps $\pi:A_\times\to I:x\mapsto xp$ and 
$\sigma:I\to A_\times:x\mapsto x$ we clearly have $\pi\sigma=1_I$. 
Therefore, $I$ is a $\langle$~$1$-retract / $\lVert p\rVert$-retract~$\rangle$ 
...
From \cite[exercise 2.3.7]{DalesIntroBanAlgOpHarmAnal} we know that there
...
an $A$-module. To prove the main result of this section we need two preparatory
lemmas.

\begin{lemma}\label{ImgOfAMorphFromBiIdToA} Let $I$ be a closed two-sided ideal of a
Banach algebra $A$ which is essential as a left $I$-module and let $\phi:I\to A$
be an $A$-morphism. Then $\operatorname{Im}(\phi)\subset I$.
\end{lemma}
\begin{proof} Since $I$ is a right ideal, $\phi(ab)=a\phi(b)\in I$ for all
$a,b\in I$. So $\phi(I\cdot I)\subset I$. Since $I$ is an essential left
$I$-module, $I=\operatorname{cl}_A(\operatorname{span}(I\cdot I))$ and
$\operatorname{Im}(\phi)
\subset\operatorname{cl}_A(\operatorname{span}\phi(I\cdot I))
\subset\operatorname{cl}_A(\operatorname{span}I)
=I$.
\end{proof}
...
\begin{enumerate}[label = (\roman*)]
    ...
    $\psi:I\to I$ of right $I$-modules such that $\lVert\psi\rVert\leq
    k\lVert\phi\rVert$ and $\phi(x)y=x\psi(y)$ for all $x,y\in I$. Then $I$ has a
...
we have $\lVert\sigma(x)\rVert=\sum_{d\in B_I} \lVert\sigma_d(x)\rVert$ for
all $x\in I$. Since $\sigma$ is a right inverse morphism of $\pi_I$
$x=\pi_I(\sigma(x))=\sum_{d\in B_I}\sigma_d(x)d$ for all $x\in I$. 

Assume $(i)$ holds. From the assumption, for each $d\in B_I$ there exists a morphism
...
for all $S\in\mathcal{P}_{f}(B_I)$ it holds that
\[
\sum_{d\in S}\lVert \tau_d(d)\rVert
=\sum_{d\in S}\lim_{\nu}\lVert e_\nu \tau_d(d) \rVert
=\lim_{\nu}\sum_{d\in S}\lVert e_\nu \tau_d(d)\rVert
=\lim_{\nu}\sum_{d\in S}\lVert \sigma_d(e_\nu)d \rVert
\]
\[
\leq\liminf_{\nu}\sum_{d\in S}\lVert\sigma_d(e_\nu)\rVert\lVert d\rVert 
\leq\liminf_{\nu}\sum_{d\in S}\lVert\sigma_d(e_\nu)\rVert
\leq\liminf_{\nu}\sum_{d\in B_I}\lVert\sigma_d(e_\nu)\rVert
\]
\[
=\liminf_{\nu}\lVert\sigma(e_\nu)\rVert
\leq\lVert\sigma\rVert\liminf_{\nu}\lVert e_\nu\rVert
\leq D\lVert\sigma\rVert 
\]
...
approximate identity, so $p$ is the identity of $I$ with $\lVert
p\rVert=\lim_\nu\lVert e_\nu\rVert$. Therefore, the norm of $p$ is $\langle$~at most
$1$ / at most $c$~$\rangle$.

Assume $(ii)$ holds. From the assumption, for each $d\in B_I$ there exists a
morphism of right $I$-modules $\tau_d:I\to I$ such that
$\sigma_d(x)d=x\tau_d(d)$ for all $x\in I$ and 
$\lVert\tau_d\rVert\leq k\lVert\sigma_d\rVert$. 
...
\[
\lVert\sigma_d(x)\rVert
=\lVert\sigma_d(\lim_\nu x e_\nu)\rVert
=\lim_\nu\lVert x\sigma_d(e_\nu)\rVert
\leq\lVert x\rVert\liminf_\nu\lVert\sigma_d(e_\nu)\rVert
\]
so $\lVert\sigma_d\rVert\leq \liminf_\nu\lVert\sigma_d(e_\nu)\rVert$. 
Then for all $S\in\mathcal{P}_{f}(B_I)$ it holds that
\[
\sum_{d\in S}\lVert \tau_d(d)\rVert
\leq \sum_{d\in S}\lVert \tau_d\rVert\lVert d\rVert
\leq k\sum_{d\in S}\lVert \sigma_d\rVert
...
\leq k\liminf_{\nu}\sum_{d\in B_I}\lVert \sigma_d(e_\nu) \rVert
=k\liminf_{\nu}\lVert\sigma(e_\nu)\rVert
\leq k\lVert\sigma\rVert\liminf_{\nu}\lVert e_\nu\rVert
\leq kD\lVert\sigma\rVert
\]
Since $S\in \mathcal{P}_{f}(B_I)$ is arbitrary, we have a well-defined element
$p=\sum_{d\in B_I}\tau_d(d)$ with norm $\langle$~at most $1$ / at most
...
of a commutative Banach algebra $A$ and let $I$ have a $\langle$~contractive /
...
projective as an $A$-module. Since $A$ is commutative, for any $A$-morphism
$\phi:I\to A$ and $x,y\in I$ we have $\phi(x)y=x\phi(y)$. Since $I$ has a 
bounded approximate identity and $A$ is commutative, we can apply
...
\cite[theorem IV.3.6]{HelHomolBanTopAlg}. 

Note that the existence of some bounded approximate identity is not necessary 
for a closed ideal of a commutative Banach algebra to be even 
topologically projective. Indeed, consider the Banach algebra  
$A_0(\mathbb{D})$ --- the ideal of the disk algebra consisting of functions 
vanishing at zero. By a combination of propositions 4.3.5 and 4.3.13 
paragraph $(iii)$ from~\cite{DalBanAlgAutCont} we get that $A_0(\mathbb{D})$ 
has no bounded approximate identities. On the other hand, 
from \cite[example IV.2.2]{HelBanLocConvAlg} we know that
$A_0(\mathbb{D})\isom{A_0(\mathbb{D})\text{-}\mathbf{mod}} {A_0(\mathbb{D})}_+$, so
$A_0(\mathbb{D})$ is topologically projective by
proposition~\ref{UnitalAlgIsMetTopProj}.

The next proposition is an obvious adaptation of a purely algebraic argument for
projective cyclic modules. It is almost identical to \cite[proposition 2.11]{WhiteInjmoduAlg}.

\begin{proposition}\label{MetTopProjCycModCharac} Let $I$ be a closed left ideal in
$A_\bullet $. Assume the natural projection $\pi:A_\bullet\to A_\bullet/I$ is
strictly $\langle$~coisometric / $c$-topologically surjective~$\rangle$. 
Then the following holds:

\begin{enumerate}[label = (\roman*)]
    \item If $A_\bullet /I$ is $\langle$~metrically / $C$-topologically~$\rangle$
    projective as an $A$-module, then there exists an idempotent $p\in I$ such that
    $I=A_\bullet p$ and $\lVert e_{A_\bullet}-p\rVert$ is $\langle$~at most $1$ / at most
    $cC$~$\rangle$;

    \item If there exists an idempotent $p\in I$ such that $I=A_\bullet  p$ and
    $\lVert e_{A_\bullet }-p\rVert$ is $\langle$~at most $1$ / at most
    $C$~$\rangle$, then $A_\bullet/I$ is $\langle$~metrically /
    $C$-topologically~$\rangle$ projective.
\end{enumerate}
\end{proposition}
\begin{proof} $(i)$ Since the natural quotient map $\pi$ is strictly 
$\langle$~coisometric / $c$-topologically surjective~$\rangle$ and $A_\bullet /I$
is $\langle$~metrically / $C$-topologically~$\rangle$ projective, $\pi$ has
a right inverse $A$-morphism $\sigma$ with norm $\langle$~at most $1$ / at most
$cC$~$\rangle$. We set $e_{A_\bullet }-p=(\sigma\pi)(e_{A_\bullet })$. Then
$(\sigma\pi)(a)=a(e_{A_\bullet }-p)$. By construction, $\pi\sigma=1_{A_\bullet /I}$,
so  
\[
e_{A_\bullet }-p
=(\sigma\pi)(e_{A_\bullet })
=(\sigma\pi)(\sigma\pi)(e_{A_\bullet })
=(\sigma\pi)(e_{A_\bullet }-p)
=(e_{A_\bullet }-p)(\sigma\pi)(e_{A_\bullet })
={(e_{A_\bullet }-p)}^2
\]
This equality shows that $p^2=p$. Therefore, $A_\bullet
p=\operatorname{Ker}(\sigma\pi)$ because $(\sigma\pi)(a)=a-ap$. Since $\sigma$
is injective, this is equivalent to $A_\bullet p=\operatorname{Ker}(\pi)$ which
equals $I$. Finally, note that 
$\lVert e_{A_\bullet }-p\rVert
=\lVert(\sigma\pi)(e_{A_\bullet})\rVert
\leq\lVert\sigma\rVert\lVert\pi\rVert\lVert e_{A_\bullet }\rVert
=\lVert\sigma\rVert$.

$(ii)$ Since $p^2=p$, we have a well-defined closed left ideal $I=A_\bullet p$ and
an $A$-module map $\sigma:A_\bullet /I\to A_\bullet:a+I\mapsto a-ap$. It is easy
to check that $\pi\sigma=1_{A_\bullet/I}$ 
and $\lVert\sigma\rVert\leq\lVert e_{A_\bullet }-p\rVert$. 
This means that $\pi:A_\bullet \to A_\bullet /I$ is a
$\langle$~$1$-retraction / $C$-retraction~$\rangle$. From
propositions~\ref{UnitalAlgIsMetTopProj} 
and~\ref{RetrMetCTopProjIsMetCTopProj} it follows that $A_\bullet /I$ is
$\langle$~metrically / $C$-topologically~$\rangle$ projective.
\end{proof} 
...
assumptions. For example, if a closed left ideal $I$ is complemented as a Banach space in
$A_\times$, then almost the same criterion as in the previous proposition holds in
relative theory \cite[proposition 7.1.29]{HelBanLocConvAlg}. There are other
...
iff $A_\times\isom{A\text{-}\mathbf{mod}}I\bigoplus_1 I'$ for some closed left 
ideal $I'$ of $A$. For details see \cite[chapter IV, \S 4]{HelHomolBanTopAlg}.
...
\begin{definition}[{\cite[definition 4.3]{HelMetrFrQMod};~\cite[definition 3.4]{WhiteInjmoduAlg}}]\label{MetCTopInjMod} 
An $A$-module $J$ is 
...
$\langle$~$\operatorname{Hom}_{\mathbf{mod}_1\text{-}A}(-,J)
:\mathbf{mod}_1\text{-}A\to\mathbf{Ban}_1$
/
$\operatorname{Hom}_{\mathbf{mod}\text{-}A}(-,J)
:\mathbf{mod}\text{-}A\to\mathbf{Ban}$~$\rangle$
maps $\langle$~isometric / $c$-topologically injective~$\rangle$
$A$-morphisms into strictly $\langle$~coisometric / 
$c C$-topologically surjective~$\rangle$ operators. 

In the category $\langle$~$\mathbf{mod}_1\text{-}A$ / $\mathbf{mod}\text{-}A$~$\rangle$ there
is a special class of $\langle$~metrically / $1$-topologically~$\rangle$
...
\begin{proposition}[{\cite[lemma 3.6]{WhiteInjmoduAlg}}]\label{MetCTopCofreeMod} Let $\Lambda$ be an arbitrary 
set, then the right $A$-modules $\mathcal{B}(A_+, \ell_\infty(\Lambda))$ 
...
$\langle$~an isometric / a $c$-topologically injective~$\rangle$ 
$A$-morphism $\xi:Y\to X$. Fix an arbitrary $\lambda\in\Lambda$ and define 
a bounded linear functional 
$h_\lambda:Y\to\mathbb{C},\, y\mapsto \phi(y)(e_{A_\bullet})(\lambda)$. 
Clearly, $\lVert h_\lambda\rVert\leq\lVert \phi\rVert$.
Denote $X_0=\operatorname{Im}(\xi)$ and consider 
the $A$-morphism $\eta=\xi|^{X_0}$. Since $\xi$ is 
$\langle$~isometric / $c$-topologically injective~$\rangle$, $X_0$ is
...
By the Hahn-Banach theorem we can extend $f_\lambda$ to some bounded linear functional
$g_\lambda:X\to\mathbb{C}$ with norm  
$\lVert g_\lambda\rVert
=\lVert f_\lambda\rVert
\leq\lVert h_\lambda\rVert\lVert\zeta\rVert
\leq K\lVert \phi\rVert$. 
Consider the $A$-morphism 
$\psi
...
$\lVert\psi\rVert\leq K\lVert\phi\rVert$. Thus, for a given $\phi$ 
and $\xi$ we have constructed an $A$-morphism $\psi$ such that $\psi\xi=\phi$ 
and $\langle$~$\lVert\psi\rVert\leq\lVert\phi\rVert$ /
$\lVert\psi\rVert\leq c\lVert\phi\rVert$~$\rangle$. Hence, the 
...
$A_\bullet^*$ is metrically and $1$-topologically injective.
\end{proposition}
\begin{proof} Consider the set $\Lambda=\mathbb{N}_1$. 
By proposition~\ref{MetCTopCofreeMod} the 
$A$-module $\mathcal{B}(A_\bullet, \ell_\infty(\Lambda))$ is metrically 
and $1$-topologically injective. Now it remains to note that 
$\mathcal{B}(A_\bullet, \ell_\infty(\Lambda))
\isom{\mathbf{mod}_1\text{-}A}
\mathcal{B}(A_\bullet, \mathbb{C})
\isom{\mathbf{mod}_1\text{-}A}
A_\bullet^*$.
\end{proof}

\begin{proposition}[{\cite[lemma 3.7]{WhiteInjmoduAlg}}]\label{RetrMetCTopInjIsMetCTopInj} Any 
$\langle$~$1$-retract / $c$-retract~$\rangle$ of a 
...
$\zeta\eta=1_{J}$ and $\lVert\zeta\rVert\lVert\eta\rVert\leq c$ 
for $\langle$~$c=1$ / $c\geq 1$~$\rangle$. Consider 
an arbitrary $\langle$~isometric / $c'$-topologically injective~$\rangle$ 
$A$-morphism $\xi:Y\to X$ and an arbitrary $A$-morphism $\phi:Y\to J$. 
Consider the $A$-morphism $\phi'=\eta\phi$. Since $J'$ is 
$\langle$~metrically / $C$-topologically~$\rangle$ injective, there 
exists an $A$-morphism $\psi':X\to J'$ such that $\phi'=\psi'\xi$ 
and $\lVert\psi'\rVert \leq K\lVert\phi'\rVert$ 
for $\langle$~$K=1$ / $K=c' C$~$\rangle$. Now it is routine to check that 
for the $A$-morphism $\psi=\zeta\psi'$ 
holds $\psi\xi=\phi$ and $\lVert\psi\rVert \leq cK\lVert\phi\rVert$. 
Thus, for a given $\phi$ and $\xi$ we have
constructed an $A$-morphism $\psi$ such that $\psi\xi=\phi$  and
$\langle$~$\lVert\psi\rVert\leq\lVert\phi\rVert$ / 
$\lVert \psi\rVert\leq c' c C\lVert\phi\rVert$~$\rangle$. Hence, $J$ 
...
\begin{proposition}[{\cite[proposition 3.10]{WhiteInjmoduAlg}}]\label{MetCTopInjModViaCanonicMorph} An $A$-module $J$ is
$\langle$~metrically / $C$-topologically~$\rangle$ injective iff $\rho_J^+$ is 
a $\langle$~$1$-coretraction / $C$-coretraction~$\rangle$ in $\mathbf{mod}\text{-}A$.
\end{proposition}
\begin{proof}
Suppose $J$ is 
$\langle$~metrically / $C$-topologically~$\rangle$ injective. Then consider 
$\langle$~an isometric / a $1$-topologically injective~$\rangle$
$A$-morphism $\rho_J^+$. Consider the extension problem with $\phi=1_J$ 
and $\xi=\rho_J^+$. Then from $\langle$~metric / $C$-topological~$\rangle$ 
injectivity of $J$ we get an $A$-morphism $\tau^+$ such 
that $\tau^+\rho_J^+=1_J$
and $\lVert\tau^+\rVert\leq K$ for $\langle$~$K=1$ / $K=C$~$\rangle$. 
Since $\lVert\rho_J^+\rVert\lVert \tau^+\rVert\leq K$, 
we conclude that $\rho_J^+$ is 
a $\langle$~$1$-coretraction / $C$-coretraction~$\rangle$ in $\mathbf{mod}\text{-}A$.

Conversely, assume that $\rho_J^+$ is 
a $\langle$~$1$-coretraction / $C$-coretraction~$\rangle$. In other words, $J$ is 
...
\begin{proposition}[{\cite[proposition 3.9]{WhiteInjmoduAlg}}]\label{MetInjIsTopInjAndTopInjIsRelInj} Every metrically
...
category of right Banach modules over the zero algebra, we may speak of
...
isometrically isomorphic to a $C(K)$-space for some extremally disconnected compact
Hausdorff space $K$ \cite[theorem 3.11.6]{LaceyIsomThOfClassicBanSp}. Usually
...
\cite[chapter 40]{JohnLinHandbookGeomBanSp}.
...
left inverse morphism $\tau^+$, with norm $\langle$~at most $1$ / at most
...
faithful and $a\in A$ is arbitrary, $\tau^+(T)=0$. Let $p:A_+\to A$ be
the natural projection from $A_+$ onto $A$, then define bounded linear operators
...
Note that $\lVert\tau \rVert\leq\lVert\tau^+\rVert\lVert j\rVert\leq\lVert\tau^+\rVert$.
...
for all $x\in J$. Since $\tau \rho_J=1_J$, $\rho_J$ 
is a  $\langle$~$1$-coretraction / $C$-coretraction~$\rangle$ 
in $\mathbf{mod}\text{-}A$.

Conversely, assume $\rho_J$ is a $\langle$~$1$-coretraction /
$C$-coretraction~$\rangle$. In other words, $\rho_J$ has a left inverse 
morphism $\tau $ with norm $\langle$~at most $1$ / at most $C$~$\rangle$. 
Define $i:A\to A_+$ to be the natural embedding of $A$ into $A_+$ and 
define the $A$-morphism $q=\mathcal{B}(i,\ell_\infty(B_{J^*}))$. 
Obviously, $\rho_J=q\rho_J^+$. Consider the $A$-morphism $\tau^+=\tau q$. 
Note that $\lVert\tau^+\rVert\leq\lVert\tau \rVert\lVert q\rVert\leq \lVert\tau \rVert$.
Therefore, $\tau^+$ has norm $\langle$~at most $1$ / at most $C$~$\rangle$. 
...
\begin{enumerate}[label = (\roman*)]
    ...
    $J_\lambda$ is $C$-topologically injective.
\end{enumerate}
\end{proposition}
...
the natural projection $p_\lambda:J\to J_\lambda$. By
...
$C$-coretraction in $\mathbf{mod}\text{-}A$. As a result, $J$ is a $C$-retract of 
\[
\bigoplus\nolimits_\infty \{
...
\]
in $\mathbf{mod}\text{-}A$, where $S=\bigsqcup_{\lambda\in\Lambda}S_\lambda$. Clearly,
...
is $\langle$~metrically / $C$-topologically~$\rangle$ injective as a $1$-retract 
of the $\langle$~metrically / $C$-topologically~$\rangle$ injective 
...
The property of being a metrically, topologically or relatively injective module 
puts some restrictions on the Banach geometric structure of the module.

\begin{proposition}[{\cite[proposition 2.2.1]{RamsHomPropSemgroupAlg}}]\label{MetTopRelInjModCompIdealAnnihCompl} Let $J$ 
be a $\langle$~metrically / $C$-topologically / $C$-relatively~$\rangle$ 
...
a bounded linear operator $p:J\to J,\, x\mapsto x-\tau^+(\rho_J^+(x)r)$. Clearly,
$\lVert p\rVert\leq 1+\lVert \tau^+\rVert\lVert r\rVert$. 
Since $\operatorname{Im}(r)\subset I$, for all $x\in J^{\perp I}$ we 
have $\rho_J^+(x)r=0$. Hence, $p(x)=x$ for all $x\in J^{\perp I}$. Since $I$ is 
a right ideal, one can check that for all $x\in J$ and $a\in I$ it holds that 
...
a projection onto $J^{\perp I}$ with norm at 
most $1+\lVert \tau^+\rVert\lVert r\rVert$. Hence, $J^{\perp I}$ 
is $\langle$~$2$-complemented / $(1+cC)$-complemented / 
$(1+cC)$-complemented~$\rangle$ in $J$.
\end{proof}
...
\begin{proposition}\label{MetTopInjModAnnihPartIsInjBanSp} Let $J$ be a
$\langle$~metrically / $C$-topologically~$\rangle$ injective right $A$-module.
Then $J_{ann}$ is a $\langle$~metrically / $C$-topologically~$\rangle$ injective
Banach space. $\langle$~As a consequence,
$J_{ann}\isom{\mathbf{Ban}_1}C(K)$ for some Stonean space $K$ /~$\rangle$.
\end{proposition}
\begin{proof} Consider an arbitrary $\langle$~isometric /
$c$-topologically injective~$\rangle$ operator $\iota:Y\to X$ of Banach spaces
and an arbitrary bounded linear operator $\phi:Y\to J_{ann}$. Endow $X$ and $Y$
with the annihilator right $A$-module structure; then $\iota$ is an
$A$-morphism. Let $j:J_{ann}\to J$ be the natural isometric embedding. Since
$J_{ann}\cdot A= \{0 \}$, for all $y\in Y$ and $a\in A$ we
have $(j\phi)(y\cdot a)=(j\phi)(0)=0=(j\phi)(y)\cdot a$, so $j\phi$ is an
$A$-morphism too. Since $J$ is a $\langle$~metrically /
$C$-topologically~$\rangle$ injective $A$-module, by
definition~\ref{MetCTopInjMod} there exists an $A$-morphism $\psi:X\to J$ such
that $\psi\iota=j\phi$ and $\langle$~$\lVert\psi\rVert\leq\lVert j\phi\rVert$ /
$\lVert\psi\rVert\leq cC\lVert j\phi\rVert$~$\rangle$. As $j$ is isometric, the
norm of $\psi$ is $\langle$~at most $\lVert\phi\rVert$ / at most
$cC\lVert\phi\rVert$~$\rangle$.

Since $X$ is an annihilator module, $\psi(x)\cdot a=\psi(x\cdot a)=\psi(0)=0$
for all $x\in X$ and $a\in A$, that is, $\psi(X)\subset J_{ann}$. Therefore,
$\psi$ admits a corestriction $\widetilde{\psi}:X\to J_{ann}$ of the same norm,
and $j\widetilde{\psi}\iota=\psi\iota=j\phi$ gives $\widetilde{\psi}\iota=\phi$
because $j$ is injective. Thus, $J_{ann}$ solves every $\langle$~metric /
$C$-topological~$\rangle$ extension problem for Banach spaces, that is, it is a
$\langle$~metrically / $C$-topologically~$\rangle$ injective Banach space.
$\langle$~From \cite[theorem 3.11.6]{LaceyIsomThOfClassicBanSp} the Banach
space $J_{ann}$ is isometrically isomorphic to $C(K)$ for some Stonean
space $K$. /~$\rangle$
\end{proof}
...
The first definition of metrically flat modules was given by Helemskii 
...
terminology. Unfortunately, White's definition was erroneous, although the results 
were correct. So we take it upon ourselves to fix the definition.

\begin{definition}[{\cite[definition I]{HelMetrFlatNorMod};~\cite[definition 4.4]{WhiteInjmoduAlg}}]\label{MetCTopFlatMod} 
...
$\langle$~$-\projmodtens{A}F:\mathbf{mod}_1\text{-}A\to\mathbf{Ban}_1$ /
$-\projmodtens{A}F:\mathbf{mod}\text{-}A\to\mathbf{Ban}$~$\rangle$ maps
...
\begin{proposition}[{\cite[lemma 4.10]{WhiteInjmoduAlg}}]\label{MetCTopFlatCharac}
An $A$-module $F$ is
...
Since the operators ${(\xi\projmodtens{A} 1_F)}^*$ and $\mathcal{B}_A(\xi,F^*)$ 
are equivalent in $\mathbf{Ban}_1$ via the universal property of the projective module 
tensor product, we get that $\xi\projmodtens{A} 1_F$ 
...
considering the respective duals.
...
\begin{enumerate}[label = (\roman*)]
    ...
    $\xi^{**}\psi=\iota_Y\phi$ and $\langle$~$\lVert\psi\rVert=\lVert\phi\rVert$ /
    $\lVert\psi\rVert\leq cC\lVert\phi\rVert$~$\rangle$;
...
\cite[exercise 4.4.6]{HelLectAndExOnFuncAn} we know that $\xi^*$ is
...
surjective~$\rangle$ by \cite[exercise 4.4.7]{HelLectAndExOnFuncAn}. Note that
${(\xi^*\projmodtens{A}1_F)}^*$ and ${}_A\mathcal{B}(F,\xi^{**})$ are equivalent
in $\mathbf{Ban}_1$ thanks to the law of adjoint associativity. So
${}_A\mathcal{B}(F,\xi^{**})$ is strictly $\langle$~coisometric / 
...
$\langle$~$\lVert\psi\rVert=\lVert\iota_Y\phi\rVert=\lVert\phi\rVert$ /
$\lVert\psi\rVert\leq cC\lVert\iota_Y\phi\rVert=cC\lVert\phi\rVert$~$\rangle$.

$(ii)\implies (iii)$ Set $\xi=\pi_F^+$ and $\phi=1_F$. Since $\xi$ is
strictly $\langle$~coisometric/ $1$-topologically surjective~$\rangle$,
from the assumption we get an $A$-morphism $\sigma:F\to
{(A_+\projtens\ell_1(B_F))}^{**}$ such that 
${(\pi_F^+)}^{**}\sigma
=\iota_F 1_F
=\iota_F$ 
and $\langle$~$\lVert\sigma\rVert\leq \lVert\phi\rVert=1$ /
$\lVert\sigma\rVert\leq 1\cdot C\lVert\phi\rVert=C$~$\rangle$.

$(iii)\implies (i)$ Let $\sigma$ be a right inverse $A$-morphism for
${(\pi_F^+)}^{**}$ with norm $\langle$~at most $1$ / at most $C$~$\rangle$.
Consider the $A$-morphism $\tau=\sigma^*\iota_{{(A_+\projtens\ell_1(B_F))}^*}$.
...
the category of left Banach modules over the zero algebra; then we get the 
definition of a $\langle$~metrically / topologically~$\rangle$ flat Banach space. 
...
\cite[corollary 23.5(1)]{DefFloTensNorOpId}: a Banach space is topologically
flat iff it is an $\mathscr{L}_1^g$-space.

\begin{proposition}\label{MetTopFlatModCoProd} Let
${(F_\lambda)}_{\lambda\in\Lambda}$ be a family of $A$-modules. Then 
...
$\Lambda$ be an arbitrary set. Then $\mathcal{B}(P,\ell_\infty(\Lambda))$ is an
$\langle$~metrically / $C$-topologically~$\rangle$ injective $A$-module. In
particular, $P^*$ is an $\langle$~metrically / $C$-topologically~$\rangle$
injective $A$-module.
\end{proposition}
...
$C$-coretraction~$\rangle$ from $\mathcal{B}(P,\ell_\infty(\Lambda))$ into a
...
The property of being a metrically, topologically or relatively flat module 
puts some restrictions on the Banach geometric structure of the module.

\begin{proposition}[{\cite[corollary 2.2.2]{RamsHomPropSemgroupAlg}}]\label{MetTopRelFlatModCompIdealPartCompl} Let $F$ 
be a $\langle$~metrically / $C$-topologically / $C$-relatively~$\rangle$ 
flat $A$-module, and let $I$ 
be a $\langle$~$1$-complemented / $c$-complemented / $c$-complemented~$\rangle$
...
recall that ${(F^*)}^{\perp I}={\operatorname{cl}_F(I F)}^\perp$. 
\end{proof}

\begin{corollary}\label{MetTopRelFlatModPartCompl} Let $F$ 
be a $\langle$~metrically / $C$-topologically / $C$-relatively~$\rangle$ 
flat $A$-module. Then $F_{ess}$ 
...
\begin{proposition}\label{MetTopFlatModEssQuotIsFlatBanSp} Let $F$ be a
$\langle$~metrically / $C$-topologically~$\rangle$ flat $A$-module. Then
$F/F_{ess}$ is a $\langle$~metrically / $C$-topologically~$\rangle$ flat Banach
space, that is, $\langle$~$F/F_{ess}\isom{\mathbf{Ban}_1}L_1(\Omega,\mu)$ for
some measure space $(\Omega, \Sigma, \mu)$ / $F/F_{ess}$ is an
$\mathscr{L}_{1,C}^g$-space~$\rangle$.
\end{proposition}
\begin{proof} By proposition~\ref{MetCTopFlatCharac} the right $A$-module $F^*$
is $\langle$~metrically / $C$-topologically~$\rangle$ injective. Then from
proposition~\ref{MetTopInjModAnnihPartIsInjBanSp} it follows that the Banach
space ${(F^*)}_{ann}$ is $\langle$~metrically / $C$-topologically~$\rangle$
injective too. Recall that
${(F^*)}_{ann}={(F^*)}^{\perp A}={(F_{ess})}^\perp$ and that
${(F/F_{ess})}^*\isom{\mathbf{Ban}_1}{(F_{ess})}^\perp$. Therefore, the dual
of the Banach space $F/F_{ess}$ is $\langle$~metrically /
$C$-topologically~$\rangle$ injective, and it remains to apply
proposition~\ref{MetCTopFlatCharac} once more, this time to Banach spaces
regarded as modules over the zero algebra. Now the description of
$\langle$~metrically / $C$-topologically~$\rangle$ flat Banach spaces is given
by $\langle$~\cite[theorem 1]{GrothMetrProjFlatBanSp} /
\cite[corollary 23.5(1)]{DefFloTensNorOpId}~$\rangle$.
\end{proof}
...
metrically or topologically flat. The proofs are somewhat similar to the ones used
...
of $A_\times $ and suppose $I$ has a right $\langle$~contractive / $c$-bounded~$\rangle$
approximate identity ${(e_\nu)}_{\nu\in N}$. Then $I$ is $\langle$~metrically /
$c$-topologically~$\rangle$ flat.
\end{proposition}
...
$|f(a e_\nu)|
\leq\lVert f\rVert\lVert a\rVert\lVert e_\nu\rVert
\leq c\lVert f\rVert\lVert a\rVert$, 
i.e. ${(f(ae_\nu))}_{\nu\in N}$ is a bounded net of complex numbers. 
Therefore, we have a well-defined limit $\lim_{\mathfrak{U}}f(ae_\nu)$ along 
the ultrafilter $\mathfrak{U}$. Now it is routine to check that 
$\sigma:I^*\to A_\times^*:f\mapsto (a\mapsto \lim_{\mathfrak{U}}f(ae_\nu))$ 
is an $A$-morphism with norm $\langle$~at most $1$ / at most $c$~$\rangle$. 
Let $\rho:I\to A_\times$ be the natural embedding; then for 
all $f\in I^*$ and $a\in I$ we have
\[
\rho^*(\sigma(f))(a)
=\sigma(f)(\rho(a))
=\sigma(f)(a)
...
of $A_\times$ \cite[proposition 7.1.45]{HelBanLocConvAlg}. Now we are able to
give an example of a metrically flat module which is not even topologically
projective. Clearly, the algebra $c_0(\mathbb{N})$ has no identity, but it admits a
contractive approximate identity. By
theorem~\ref{GoodCommIdealMetTopProjIsUnital} the $\ell_\infty(\mathbb{N})$-module
$c_0(\mathbb{N})$ is not topologically
...
\cite[proposition 4.11]{WhiteInjmoduAlg}. The case of topological flatness was
solved by Helemskii in \cite[theorem VII.1.20]{HelHomolBanTopAlg}.

\begin{proposition}\label{MetTopFlatCycModCharac} Let $I$ be a closed left proper ideal
of $A_\bullet $. Then the following are equivalent:

\begin{enumerate}[label = (\roman*)]
    \item $A_\bullet /I$ is a $\langle$~metrically / $C$-topologically~$\rangle$
    flat $A$-module;

    \item $I$ has a right bounded approximate identity ${(e_\nu)}_{\nu\in N}$
    with $\sup_{\nu\in N}\lVert e_{A_\bullet }-e_\nu\rVert$ $\langle$~at most $1$ /
    at most $C$~$\rangle$
\end{enumerate}
\end{proposition}
\begin{proof} $(i)\implies (ii)$  Since $A_\bullet /I$ is $\langle$~metrically
/ $C$-topologically~$\rangle$ flat, by proposition~\ref{MetCTopFlatCharac} 
the right $A$-module ${(A_\bullet /I)}^*$ is $\langle$~metrically /
$C$-topologically~$\rangle$ injective. Let $\pi:A_\bullet \to A_\bullet /I$ be the
natural quotient map; then $\pi^*:{(A_\bullet /I)}^*\to A_\bullet ^*$ is an
isometry. Since ${(A_\bullet /I)}^*$ is $\langle$~metrically /
$C$-topologically~$\rangle$ injective, $\pi^*$ is a coretraction, i.e.\
there exists a $\langle$~strictly coisometric / topologically
surjective~$\rangle$ $A$-morphism $\tau:A_\bullet ^*\to {(A_\bullet /I)}^*$ of
norm $\langle$~at most $1$ / at most $C$~$\rangle$ such that
$\tau\pi^*=1_{{(A_\bullet /I)}^*}$. Consider $p\in A_\bullet ^{**}$ such that
$\iota_{A_\bullet }(e_{A_\bullet })-p
=\tau^*(\pi^{**}(\iota_{A_\bullet }(e_{A_\bullet })))$. 
Fix $f\in I^\perp$. Since $I^\perp=\pi^*({(A_\bullet /I)}^*)$, 
there exists a functional $g\in {(A_\bullet /I)}^*$ such 
that $f=\pi^*(g)$. Thus,
\[
(\iota_{A_\bullet }(e_{A_\bullet })-p)(f)
=\tau^*(\pi^{**}(\iota_{A_\bullet }(e_{A_\bullet })))(\pi^*(g))
=\pi^{**}(\iota_{A_\bullet }(e_{A_\bullet }))(\tau(\pi^*(g)))
\]
\[
=\pi^{**}(\iota_{A_\bullet }(e_{A_\bullet }))(g)
=\iota_{A_\bullet }(e_{A_\bullet })(\pi^*(g))
=\iota_{A_\bullet }(e_{A_\bullet })(f).
\]
Therefore, $p(f)=0$ for all $f\in I^\perp$, i.e. $p\in I^{\perp\perp}$. Recall
that $I^{\perp\perp}$ is the weak${}^*$ closure of $I$ in $A_\bullet ^{**}$, so we can
choose a net ${(e_\nu'')}_{\nu\in N''}\subset I$ such that
${(\iota_{A_\bullet }(e_\nu''))}_{\nu\in N''}$ converges to $p$ in the weak${}^*$ topology.
Clearly, ${(\iota_{A_\bullet }(e_{A_\bullet }-e_\nu''))}_{\nu\in N''}$ converges to
$\iota_{A_\bullet }(e_{A_\bullet })-p$ in the same topology. By
\cite[lemma 1.1]{PosAndApproxIdinBanAlg} there exists a net in the convex hull
$\operatorname{conv}{
    (\iota_{A_\bullet }(e_{A_\bullet }-e_\nu''))
}_{\nu\in N''}
=\iota_{A_\bullet }(e_{A_\bullet })
-\operatorname{conv}{(\iota_{A_\bullet}(e_\nu''))}_{\nu\in N''}$ 
that weak${}^*$ converges to $\iota_{A_\bullet }(e_{A_\bullet })-p$ and 
all elements of this net are bounded in norm 
by $\lVert \iota_{A_\bullet }(e_{A_\bullet})-p\rVert$. Denote this net by 
${(\iota_{A_\bullet }(e_{A_\bullet })-\iota_{A_\bullet }(e_\nu'))}_{\nu\in N'}$; 
then ${(\iota_{A_\bullet }(e_\nu'))}_{\nu\in N'}$ weak${}^*$ converges to $p$. 
For any $a\in I$ and $f\in A_\bullet ^*$ we have
\[
\lim_{\nu}f(ae_\nu')
=\lim_{\nu}\iota_{A_\bullet }(e_\nu')(f\cdot a)
=p(f\cdot a)
=\iota_{A_\bullet }(e_{A_\bullet })(f\cdot a)
-\tau^*(\pi^{**}(\iota_{A_\bullet }(e_{A_\bullet })))(f\cdot a)
\]
\[
=f(a)-\iota_{A_\bullet }(e_{A_\bullet })(\pi^*(\tau(f\cdot a)))
=f(a)-\pi^*(\tau(f)\cdot a)(e_{A_\bullet })
=f(a)-\tau(f)(\pi(a))
=f(a)
\]
hence ${(e_\nu')}_{\nu\in N'}$ is a weak right bounded approximate identity for
$I$. By \cite[proposition 33.2]{AppIdAndFactorInBanAlg} there is a net
${(e_\nu)}_{\nu\in N}\subset\operatorname{conv}{(e_\nu')}_{\nu\in N'}$ which is
a right bounded approximate identity for $I$. For any $\nu\in N$ we have
$e_{A_\bullet }-e_\nu\in\operatorname{conv}{(e_{A_\bullet }-e_\nu')}_{\nu\in N'}$,
so taking into account the norm bound 
on ${(\iota_{A_\bullet }(e_{A_\bullet}-e_\nu'))}_{\nu\in N'}$ we get 
\[
\sup_{\nu\in N}\lVert e_{A_\bullet }-e_\nu\rVert
\leq\lVert \iota_{A_\bullet }(e_{A_\bullet })-p\rVert
\leq\lVert\tau^*(\pi^{**}(\iota_{A_\bullet }(e_{A_\bullet })))\rVert
\leq\lVert\tau^*\rVert\lVert\pi^{**}\rVert\lVert\iota_{A_\bullet }(e_{A_\bullet })\rVert
=\lVert\tau\rVert
\]
...
ultrafilter dominating $\mathfrak{F}$. For a fixed $f\in A_\bullet ^*$ 
and $a\in A_\bullet $ we have 
$|f(a-a e_\nu)|=|f(a(e_{A_\bullet }-e_\nu))|
\leq\lVert f\rVert\lVert a\rVert\lVert e_{A_\bullet }-e_\nu\rVert
\leq D\lVert f\rVert\lVert a\rVert$,
...
$I$ and $\mathfrak{U}$ contains the section filter, for all $a\in I$ we have
$\lim_{\mathfrak{U}}f(a-ae_\nu)=\lim_{\nu}f(a-ae_\nu)=0$. Therefore, for each
$f\in A_\bullet ^*$ we have a well-defined map $\tau(f):A_\bullet /I\to
\mathbb{C},\, a+I\mapsto \lim_{\mathfrak{U}} f(a-ae_\nu)$. Clearly, this is a
linear functional and from the inequalities above we see that its norm is bounded 
by $D\lVert f\rVert$. Now it is routine to check 
that $\tau:A_\bullet ^*\to {(A_\bullet /I)}^*,\, f\mapsto \tau(f)$ is 
an $A$-morphism with norm $\langle$~at most $1$ / at most $C$~$\rangle$. 
For all $g\in{(A_\bullet /I)}^*$ and $a+I\in A_\bullet /I$ we have
\[
\tau(\pi^*(g))(a+I)
...
=g(a+I)
\]
i.e. $\tau:A_\bullet ^*\to {(A_\bullet /I)}^*$ is a retraction. The right
$A$-module $A_\bullet ^*$ is $\langle$~metrically / $1$-topologically~$\rangle$
injective by proposition~\ref{DualOfUnitalAlgIsMetTopInj}, hence its
$\langle$~$1$-retract / $C$-retract~$\rangle$ ${(A_\bullet /I)}^*$ is
$\langle$~metrically / $C$-topologically~$\rangle$ injective. 
Proposition~\ref{MetCTopFlatCharac} gives that the
module $A_\bullet /I$ is $\langle$~metrically / $C$-topologically~$\rangle$ flat.
\end{proof}

It is worth mentioning that every operator algebra $A$ (not necessarily self
adjoint) with a contractive approximate identity has a contractive approximate
identity ${(e_\nu)}_{\nu\in N}$ such 
that $\sup_{\nu\in N}\lVert e_{A_\#}-e_\nu\rVert\leq 1$ and 
even $\sup_{\nu\in N}\lVert e_{A_\#}-2e_\nu\rVert\leq 1$. Here $A_\#$ is 
a unitization of $A$ as an operator algebra. For details
see~\cite{PosAndApproxIdinBanAlg},~\cite{BleContrAppIdInOpAlg}.

Again we shall compare our results on metric and topological flatness of cyclic
modules with their relative counterparts. Helemskii and Sheinberg showed
\cite[theorem VII.1.20]{HelHomolBanTopAlg} that a cyclic module $A/I$ is relatively
flat if $I$ admits a right bounded approximate identity. In the case when $I^\perp$
...
modules is a much stronger property due to a specific restriction on the norms of
...
$\langle$~left / right~$\rangle$ annihilator $A$-module. Then $\mathbb{C}$ is
a $1$-retract of $X$ in $\langle$~$A\text{-}\mathbf{mod}_1$ / $\mathbf{mod}_1\text{-}A$~$\rangle$.
\end{proposition}
\begin{proof} Take any $x_0\in X$ with $\lVert x_0\rVert=1$ and using the Hahn-Banach
theorem choose $f_0\in X^*$ such that $\lVert f_0\rVert=f_0(x_0)=1$. Consider
...
$\langle$~$A\text{-}\mathbf{mod}_1$ / $\mathbf{mod}_1\text{-}A$~$\rangle$. The argument for
right modules is the same.
\end{proof}
...
$\mathbb{C}\isom{A\text{-}\mathbf{mod}_1} A_+/A$. If we regard $\mathbb{C}$ as a right
annihilator $A$-module, we also have
$\mathbb{C}\isom{\mathbf{mod}_1\text{-}A}{(A_+/A)}^*$. 

\begin{proposition}\label{MetTopProjModCCharac} The annihilator $A$-module
...
$C$-topological~$\rangle$ projectivity of $A_+/A$ is equivalent to the existence of 
...
\begin{enumerate}[label = (\roman*)]
    \item $P$ is a $\langle$~metrically / $C$-topologically~$\rangle$ projective
    $A$-module;
...
projective as a $1$-retract of the $\langle$~metrically / $C$-topologically~$\rangle$
projective module $P$. Proposition~\ref{MetTopProjModCCharac} gives that
$\langle$~$A= \{0 \}$ / $A$ has a right identity of norm at most $C-1$~$\rangle$.
By corollary~\ref{MetTopProjTensProdWithl1} the annihilator $A$-module
$\mathbb{C}\projtens\ell_1(B_P)\isom{A\text{-}\mathbf{mod}_1}\ell_1(B_P)$ is
$\langle$~metrically / $C$-topologically~$\rangle$ projective. Consider the strict
coisometry $\pi:\ell_1(B_P)\to P$ well-defined by the equality $\pi(\delta_x)=x$ 
for all $x\in B_P$. Since $P$ and $\ell_1(B_P)$ are annihilator modules, 
$\pi$ is also an $A$-module map. Since $P$ is 
$\langle$~metrically / $C$-topologically~$\rangle$
projective, the $A$-morphism $\pi$ has a right inverse morphism $\sigma$ of
norm  $\langle$~at most $1$ / at most $C$~$\rangle$. Therefore, $P$ is a
$\langle$~$1$-retract / $C$-retract~$\rangle$ of the $\langle$~metrically /
...
$\mathbb{C}\projtens\ell_1(\Lambda)\isom{A\text{-}\mathbf{mod}_1}\ell_1(\Lambda)$ is
...
proposition~\ref{MetTopFlatCycModCharac} this is equivalent to the existence of a 
right bounded approximate identity ${(e_\nu)}_{\nu\in N}$ in $A$ with
$\langle$~$\sup_{\nu\in N}\lVert e_{A_+}-e_\nu\rVert\leq 1$ / 
$\sup_{\nu\in N}\lVert e_{A_+}-e_\nu\rVert\leq C$~$\rangle$. 
It remains to note that the latter inequality holds 
iff $\langle$~$e_\nu=0$ and therefore $A= \{0 \}$ /
${(e_\nu)}_{\nu\in N}$ is a right $(C-1)$-bounded approximate identity~$\rangle$.
\end{proof}
...
\begin{enumerate}[label = (\roman*)]
    \item $J$ is a $\langle$~metrically / $C$-topologically~$\rangle$ injective
    $A$-module;
...
$\langle$~metrically / $C$-topologically~$\rangle$ injective as a $1$-retract of the
...
injective. Consider the isometry $\rho:J\to\ell_\infty(B_{J^*})$ 
well-defined by $\rho(x)(f)=f(x)$ for all $x\in J$ and $f\in B_{J^*}$. 
Since $J$ and $\ell_\infty(B_{J^*})$ are  annihilator modules, 
$\rho$ is also an $A$-module map. 
Since $J$ is $\langle$~metrically / $C$-topologically~$\rangle$ injective, 
the $A$-morphism $\rho$ has a left inverse $A$-morphism $\tau$ with 
norm $\langle$~at most $1$ / at most $C$~$\rangle$.
Therefore, $J$ is a $\langle$~$1$-retract / $C$-retract~$\rangle$ of the
$\langle$~metrically / $1$-topologically~$\rangle$ injective Banach space
$\ell_\infty(B_{J^*})$. By 
proposition~\ref{RetrMetCTopInjIsMetCTopInj} the Banach space $J$ is
$\langle$~metrically / $C$-topologically~$\rangle$ injective. $\langle$~From
\cite[theorem 3.11.6]{LaceyIsomThOfClassicBanSp} the Banach space $J$ is
...
injective Banach space and there is an isometric 
...
$\langle$~$\mathbf{mod}_1\text{-}A$ / $\mathbf{mod}\text{-}A$~$\rangle$. By 
proposition~\ref{RetrMetCTopInjIsMetCTopInj} the $A$-module $J$ 
is $\langle$~metrically / $C$-topologically~$\rangle$ injective.
\end{proof}
...
\begin{enumerate}[label = (\roman*)]
    \item $F$ is a $\langle$~metrically / $C$-topologically~$\rangle$ flat
    $A$-module;
...
\end{enumerate}
\end{proposition}
\begin{proof} By $\langle$~\cite[theorem 1]{GrothMetrProjFlatBanSp} /
...
We are obliged to compare these results with similar ones in relative theory. From
$\langle$~\cite[proposition 2.1.7]{RamsHomPropSemgroupAlg} /
\cite[proposition 2.1.10]{RamsHomPropSemgroupAlg}~$\rangle$ we know that an
...
In metric and topological theory, in contrast with the relative one, homological
...
forbid the existence of certain homologically excellent algebras. One of the most
...
particular of all annihilator) left Banach modules over it. In sharp contrast,
...
such Banach algebra that all $A$-modules are $\langle$~metrically /
topologically~$\rangle$ projective.
\end{proposition}
\begin{proof} Consider any infinite dimensional $\mathscr{L}_\infty^g$-space $X$
(say $\ell_\infty(\mathbb{N})$) as an annihilator $A$-module. From the remark right
after \cite[corollary 23.3]{DefFloTensNorOpId} we know that $X$ is not an
...
modules over certain Banach algebras have a geometric structure similar to that of those
...
algebra which is as a Banach space isometrically isomorphic to $L_1(\Theta,\nu)$
...
a singleton atom adjoined. Then $A_+\isom{\mathbf{Ban}_1} L_1(\Theta',\nu')$.

$(i)$ Since $P$ is a $\langle$~metrically / topologically~$\rangle$ projective
$A$-module, by proposition~\ref{MetCTopProjModViaCanonicMorph} it is a
retract of $A_+\projtens \ell_1(B_P)$ in $\langle$~$A\text{-}\mathbf{mod}_1$ /
$A\text{-}\mathbf{mod}$~$\rangle$. Let $\mu_c$ be the counting measure on $B_P$. Then
by Grothendieck's theorem \cite[theorem 2.7.5]{HelLectAndExOnFuncAn}
\[
...
an $L_1$-space is again an $L_1$-space \cite[theorem 6.17.3]{LaceyIsomThOfClassicBanSp}.

$(ii)$ Since $J$ is a $\langle$~metrically / topologically~$\rangle$ injective
$A$-module, by proposition~\ref{MetCTopInjModViaCanonicMorph} it is a
retract of $\mathcal{B}(A_+,\ell_\infty(B_{J^*}))$ in
$\langle$~$\mathbf{mod}_1\text{-}A$ / $\mathbf{mod}\text{-}A$~$\rangle$. Let $\mu_c$ be the
counting measure on $B_{J^*}$. Then by Grothendieck's theorem
\cite[theorem 2.7.5]{HelLectAndExOnFuncAn}
\[
\mathcal{B}(A_+,\ell_\infty(B_{J^*}))
\isom{\mathbf{Ban}_1}
{(A_+\projtens \ell_1(B_{J^*}))}^*
\isom{\mathbf{Ban}_1}
{(L_1(\Theta',\nu')\projtens L_1(B_{J^*},\mu_c))}^*
\]
\[
\isom{\mathbf{Ban}_1}{L_1(\Theta'\times B_{J^*},\nu'\times \mu_c)}^*
\isom{\mathbf{Ban}_1}L_\infty(\Theta'\times B_{J^*},\nu'\times \mu_c)
\]
Therefore, $J$ is $\langle$~$1$-complemented / complemented~$\rangle$ in some
$L_\infty$-space. Since any $L_\infty$-space is a $\langle$~metrically /
topologically~$\rangle$ injective Banach space, so is $J$. It remains to
recall that every metrically injective Banach space is a $C(K)$-space for some
Stonean space $K$ \cite[theorem 3.11.6]{LaceyIsomThOfClassicBanSp}.

$(iii)$  By $\langle$~\cite[theorem 1]{GrothMetrProjFlatBanSp} / the remark after
\cite[corollary 23.5(1)]{DefFloTensNorOpId}~$\rangle$ the Banach space $F^*$
...
Banach algebra which is topologically isomorphic as a Banach space to some
...
\end{proposition}
\begin{proof} If $A$ is an $\mathscr{L}_1^g$-space, then so is $A_+$. 

Let $P$ be a topologically projective $A$-module. Then by
proposition~\ref{MetCTopProjModViaCanonicMorph} it is a retract 
of $A_+\projtens \ell_1(B_P)$ in $A\text{-}\mathbf{mod}$ and a fortiori 
in $\mathbf{Ban}$. Since
$\ell_1(B_P)$ is an $\mathscr{L}_1^g$-space, so is
$A_+\projtens\ell_1(B_P)$ as a projective tensor product of
$\mathscr{L}_1^g$-spaces \cite[exercise 23.17(c)]{DefFloTensNorOpId}.
Therefore, $P$ is an $\mathscr{L}_1^g$-space as a complemented subspace of
an $\mathscr{L}_1^g$-space \cite[corollary 23.2.1(2)]{DefFloTensNorOpId}.
...
\isom{\mathbf{mod}_1\text{-}A}
{(A_+\projtens\ell_1(B_{J^*}))}^*$
in $\mathbf{mod}\text{-}A$ and a fortiori in $\mathbf{Ban}$. As we showed in the
...
$\mathscr{L}_\infty^g$-space \cite[corollary 23.2.1(1)]{DefFloTensNorOpId}. It
remains to recall that any complemented subspace of an $\mathscr{L}_\infty^g$-space
is again an $\mathscr{L}_\infty^g$-space \cite[corollary 23.2.1(2)]{DefFloTensNorOpId}.

Finally, let $F$ be a topologically flat $A$-module. Then $F^*$ is a topologically
injective $A$-module by proposition~\ref{MetCTopFlatCharac}. From the previous
paragraph it follows that $F^*$ is an $\mathscr{L}_\infty^g$-space. By
\cite[corollary 23.5(1)]{DefFloTensNorOpId} we get that $F$ is an
...
measure space and $\Lambda$ be an arbitrary set. Then the Banach space
$\bigoplus_0 \{L_1(\Omega,\mu):\lambda\in\Lambda \}$ and all its duals have the
Dunford-Pettis property. Moreover, $\bigoplus_1
\{L_\infty(\Omega,\mu):\lambda\in\Lambda \}$ and $\bigoplus_\infty
\{L_1(\Omega,\mu):\lambda\in\Lambda \}$ have this property.
\end{proposition}
\begin{proof} Consider the one point compactification $\alpha\Lambda:=\Lambda\cup
\{\Lambda \}$ of the set $\Lambda$ with the discrete topology. From
\cite[corollary 7]{BourgOnTheDPP} we know 
...
in $C(\alpha\Lambda)$ via the projection 
$P:C(\alpha\Lambda)\to C(\alpha\Lambda):x\mapsto(\lambda\mapsto x(\lambda)-x( \{\Lambda \}))$, 
then $E:=c_0(\Lambda, L_1(\Omega,\mu))$ is complemented 
in $F$. Similarly, any $n$-th dual of $E$ is complemented in the $n$-th dual of $F$, 
because we can take the $n$-th adjoint of $P$ in the role of projection. 
...
\cite[proposition 13.44]{FabHabBanSpTh}. 
...
From \cite[proposition B10]{DefFloTensNorOpId} we know that any $L_1$-space is
...
$E^{**}$ via the projection $\bigoplus_\infty  \{Q:\lambda\in\Lambda \}$. Since
$E^{**}$ has the Dunford-Pettis property, by \cite[proposition 13.44]{FabHabBanSpTh} so does its complemented 
...
subspaces \cite[proposition 13.44]{FabHabBanSpTh}.
\end{proof}
...
\end{proposition}
\begin{proof} Since $E$ is an $\mathscr{L}_\infty^g$-space, $E^*$ is an
$\mathscr{L}_{1}^g$-space \cite[corollary 23.2.1(1)]{DefFloTensNorOpId}. Then
from \cite[corollary 23.2.1(3)]{DefFloTensNorOpId} it follows that $E^{***}$
is a retract of an $L_1$-space. Recall that $E^*$ is complemented in $E^{***}$ via
the Dixmier projection, so $E^*$ is complemented in some $L_1$-space too. Thus, we
...
the projection $\bigoplus_\infty \{P:\lambda\in\Lambda \}$. The 
...
$\langle$~$L_1$-space / $L_\infty$-space~$\rangle$ \cite[corollary 23.2.1(3)]{DefFloTensNorOpId}. Since any $\langle$~$L_1$-space /
$L_\infty$-space~$\rangle$ admits the Dunford-Pettis
property~\cite{GrothApllFaiblCompSpCK}, so does $E^{**}$ as a complemented
subspace \cite[proposition 13.44]{FabHabBanSpTh}. It remains to recall that a
Banach space has the Dunford-Pettis property whenever so does its dual.
\end{proof}

\begin{theorem}\label{TopProjInjFlatModOverMthscrL1OrLInftySpHaveDPP} Let $A$ be
a Banach algebra which is an $\mathscr{L}_1^g$-space or an
$\mathscr{L}_\infty^g$-space as a Banach space. Then any topologically projective,
...
Assume $A$ is an $\mathscr{L}_\infty^g$-space. Then so is $A_+$. Let $J$ be a
...
\] 
in $\mathbf{mod}\text{-}A$ and a fortiori in $\mathbf{Ban}$. By
proposition~\ref{ProdOfDualsOfMthscrLInftySpHaveDPP} this space has the
Dunford-Pettis property. As $J$ is a retract 
of $\mathcal{B}(A_+,\ell_\infty(B_{J^*}))$, $J$ also has 
the Dunford-Pettis property \cite[proposition 13.44]{FabHabBanSpTh}. 

If $F$ is a topologically flat $A$-module, then $F^*$ is a topologically
injective $A$-module by proposition~\ref{MetCTopFlatCharac}. By the previous
...
by proposition~\ref{MetTopProjIsMetTopFlat}. From the previous paragraph it follows
...
$\mathscr{L}_\infty^g$-space as a Banach space. Then there are no topologically
projective, injective or flat infinite dimensional reflexive $A$-modules. A
fortiori, there are no metrically projective, injective or flat infinite
...
property. On the other hand, there are no infinite dimensional reflexive Banach
spaces with the Dunford-Pettis property. Thus, we get the desired result
regarding topological injectivity. Since the dual of a reflexive module is reflexive,
...
examples. The first one is about the convolution algebra $L_1(G)$ on a locally
compact group $G$ with the left Haar measure. It is an $\mathscr{L}_1^g$-space. In
\cite[\S6]{DalPolHomolPropGrAlg} and~\cite{RachInjModAndAmenGr} it was proved
...
\cite[proposition 3.12.1]{PierAmenLCA}, so in case $G$ is compact $L_p(G)$ is
relatively projective, injective and flat for all $1<p<+\infty$.  The second
example is about the $\mathscr{L}_\infty^g$-spaces $c_0(\Lambda)$ and
$\ell_\infty(\Lambda)$ for an infinite set $\Lambda$. In
proposition~\ref{c0AndlInftyModsRelTh} we shall show that $\ell_p(\Lambda)$ for
$1<p<\infty$ is relatively projective, injective and flat as a $c_0(\Lambda)$- or
$\ell_\infty(\Lambda)$-module. 

We finalize this lengthy section by a short remark on the l.u.st.\ property of
...
in $\mathbf{mod}\text{-}A$ and a fortiori in $\mathbf{Ban}$. If $A$ has 
the l.u.st.\ property, then $A^{**}$ is complemented
in some Banach lattice $L$ \cite[theorem 17.5]{DiestAbsSumOps}. As a
consequence, $A_+^{***}$ is complemented in the Banach lattice
$M:=L^*\bigoplus_\infty\mathbb{C}$. Note that $A_+^*$ is contractively
...
via the projection $\bigoplus_\infty \{P:\lambda\in B_{J^*} \}$. Since $J$ is a 
retract of $E$, $J$ is also complemented in $K$. Any Banach 
lattice $K$ has the l.u.st.\ property \cite[theorem 17.1]{DiestAbsSumOps}. 
...
by proposition~\ref{MetCTopFlatCharac}. By the previous paragraph $F^*$ has the
...
proposition~\ref{MetTopProjIsMetTopFlat}. So $P$ has the l.u.st.\ property by the
previous paragraph.
\end{proof}
...
x)-a'\cdot\phi(x))=\phi(aa'\cdot x)-aa'\cdot\phi(x)=0$. Since $Y$ is a faithful
...
subalgebra of $A$ and $P$ be an $A$-module which is essential as an $I$-module.
Then
...
$\widetilde{\pi}_P$ has a right inverse morphism in $\langle$~$I\text{-}\mathbf{mod}_1$
/ $I\text{-}\mathbf{mod}$~$\rangle$, say $\widetilde{\sigma}$, of norm $\langle$~at most
...
$\langle$~$A\text{-}\mathbf{mod}_1$ / $A\text{-}\mathbf{mod}$~$\rangle$. So by
...
has a right inverse morphism $\sigma$ in $\langle$~$A\text{-}\mathbf{mod}_1$ /
$A\text{-}\mathbf{mod}$~$\rangle$ with norm $\langle$~at most $1$ / at most
$C$~$\rangle$. Obviously $\sigma$ is a right inverse for $\pi_P$ in
$\langle$~$I\text{-}\mathbf{mod}_1$ / $I\text{-}\mathbf{mod}$~$\rangle$ too. By $i:I\to A$ we
denote the natural embedding, and by $r:A\to I$ its $\langle$~contractive /
bounded~$\rangle$ left inverse. By assumption $\lVert r\rVert\leq c$. Consider a
...
\subset\operatorname{cl}_{A\projtens \ell_1(B_P)}(I\cdot (A\projtens \ell_1(B_P)))
...
in $\langle$~$I\text{-}\mathbf{mod}_1$ / $I\text{-}\mathbf{mod}$~$\rangle$, so by
...
subalgebra of $A$ and $J$ be a right $A$-module which is faithful as an $I$-module.
Then
...
morphism in $\langle$~$\mathbf{mod}_1\text{-}I$ / $\mathbf{mod}\text{-}I$~$\rangle$, say
...
$\widetilde{\rho}_J=q\rho_J$. Consider an $I$-morphism $\tau =\widetilde{\tau} q$.
By paragraph $(ii)$ of proposition~\ref{MorphCoincide} it is also an
$A$-morphism. Note that 
$\lVert\tau \rVert
\leq\lVert\widetilde{\tau}\rVert\lVert q\rVert
\leq\lVert\widetilde{\tau}\rVert$, 
...
an $A$-module, then by proposition~\ref{NonDegenMetTopInjCharac} the $A$-morphism
$\rho_J$ has a left inverse in $\langle$~$\mathbf{mod}_1\text{-}A$ /
$\mathbf{mod}\text{-}A$~$\rangle$, say $\tau $, of norm $\langle$~at most $1$ / at most
$C$~$\rangle$. Assume we are given an operator $T\in
...
$I$-module and $a\in I$ is arbitrary, $\tau (T)=0$. Since $I$ is weakly 
$\langle$~$1$-complemented /$c$-complemented~$\rangle$ in $A$, $i^*$ has a right 
...
and $T\in\mathcal{B}(I,\ell_\infty(B_{J^*}))$. Since $r$ is a right inverse 
...
Note that 
$\lVert\widetilde{\tau}\rVert\leq\lVert\tau \rVert\lVert j\rVert\leq \lVert
\tau\rVert\lVert r\rVert$, so $\widetilde{\tau}$ has 
norm $\langle$~at most $1$ / at most $cC$~$\rangle$.
In the same way one can show that, for all $x\in J$, 
it holds that $(\rho_J(x)-j(\widetilde{\rho}_J(x)))|_I=0$,
...
$\widetilde{\rho}_J$ is a  $\langle$~$1$-coretraction /
$cC$-coretraction~$\rangle$ in $\langle$~$\mathbf{mod}_1\text{-}I$ /
$\mathbf{mod}\text{-}I$~$\rangle$, so by proposition~\ref{NonDegenMetTopInjCharac} the
...
subalgebra of $A$ and $F$ be an $A$-module which is essential as an $I$-module.
Then
...
\end{enumerate}
\end{proposition}
\begin{proof} Note that the dual of an essential module is faithful. Now the result
...
$i_\lambda:A_\lambda\to A$ allows one to regard $A_\lambda$ as a contractively 
...
an $A$-module. It remains to apply the proposition
...
injective / flat~$\rangle$ as an $A$-module. Fix an arbitrary $\lambda\in\Lambda$.
Clearly, we may regard $X_\lambda$ as an $A$-module and even more $X_\lambda$ is a
$1$-retract of $X$ in $\langle$~$A\text{-}\mathbf{mod}_1$ / $\mathbf{mod}_1\text{-}A$ /
$A\text{-}\mathbf{mod}_1$~$\rangle$. By proposition
...
an $A$-module. It remains to apply paragraph $(ii)$ of proposition
...
Based on the results obtained above, we collect more interesting facts on metric and
...
$A$ be its subalgebra with a two-sided bounded approximate identity
${(e_\nu)}_{\nu\in N}$ and let $X$ be a left $B$-module. 
Denote $c_1=\sup_{\nu\in N}\lVert e_\nu\rVert$, 
$c_2=\sup_{\nu\in N}\lVert e_B-e_\nu\rVert$ and $X_{ess}=\operatorname{cl}_X(AX)$.
Then 
...
\begin{enumerate}[label = (\roman*)]
    ...
    $c_1$-retract / $c_2$-retract~$\rangle$ of the $A$-module $X^*$;
...
have 
$|f(x-e_\nu\cdot x)|
\leq\lVert f\rVert\lVert e_B - e_\nu\rVert\lVert x\rVert
\leq c_2\lVert f\rVert\lVert x\rVert$, i.e. ${(f(x-e_\nu\cdot x))}_{\nu\in N}$ is a
bounded net of complex numbers. Therefore, we have a well-defined limit
$\lim_{\mathfrak{U}}f(x-e_\nu\cdot x)$ along the ultrafilter $\mathfrak{U}$. Since
${(e_\nu)}_{\nu\in N}$ is a two-sided approximate identity for $A$ and
$\mathfrak{U}$ contains the section filter, for all $x\in X_{ess}$ we have
$\lim_{\mathfrak{U}}f(x-e_\nu\cdot x)=\lim_{\nu}f(x-e_\nu\cdot x)=0$. Therefore,
for each $f\in X ^*$ we have a well-defined map 
$\tau(f):
X /X_{ess}\to \mathbb{C},\, 
x+X_{ess}\mapsto \lim_{\mathfrak{U}} f(x-e_\nu\cdot x)$. 
Clearly this is a linear functional, and from the inequalities above we see 
its norm is bounded by $c_2\lVert f\rVert$. Now it is routine to check 
...
h\oplus{}_\infty g\mapsto \sigma(h)+\pi^*(g)
\]
are isomorphisms of right $A$-modules with $\lVert\xi \rVert\leq c_2$ 
and $\lVert \eta\rVert\leq c_1+1$. Hence, $X^*$ is $c_2(c_1+1)$-isomorphic 
in $\mathbf{mod}\text{-}A$ to $X_{ess}^*\bigoplus_\infty {(X/X_{ess})}^*$.

$(ii)$ Both results immediately follow from the equalities
$\rho^*\sigma=1_{X_{ess}^*}$, $\tau \pi^*=1_{{(X/X_{ess})}^*}$ and the estimates
$\lVert \rho^*\rVert\lVert \sigma\rVert\leq c_1$, 
$\lVert\tau\rVert\lVert \pi^*\rVert\leq c_2$.

$(iii)$ Now consider the case when $X$ is an $\mathscr{L}_{1,C}^g$-space. Then $X^*$
is an $\mathscr{L}_{\infty,C}^g$-space \cite[corollary 23.2.1(1)]{DefFloTensNorOpId}. As $\langle$~$X_{ess}^*$ / ${(X/X_{ess})}^*$~$\rangle$ is
$\langle$~$c_1$-complemented / $c_2$-complemented~$\rangle$ in $X^*$, it is an
$\langle$~$\mathscr{L}_{\infty,c_1C}^g$-space /
$\mathscr{L}_{\infty,c_2C}^g$-space~$\rangle$ by \cite[corollary 23.2.1(1)]{DefFloTensNorOpId}. Again we apply \cite[corollary 23.2.1(1)]{DefFloTensNorOpId} to conclude that $\langle$~$X_{ess}$  / $X/X_{ess}$~$\rangle$ is an
$\langle$~$\mathscr{L}_{1,c_1C}^g$-space /
$\mathscr{L}_{1,c_2C}^g$-space~$\rangle$.
\end{proof}

The following proposition is an analog 
of \cite[proposition 2.1.11]{RamsHomPropSemgroupAlg}.

\begin{proposition}\label{TopFlatModCharac} Let $A$ be a Banach algebra with
a two-sided $c$-bounded approximate identity, and $F$ be a left $A$-module. Then
...
\begin{enumerate}[label = (\roman*)]
    ...
    an $\mathscr{L}_{1,C_2}^g$-space, then $F$ is a ${(1+c)}^2\max(C_1,
    C_2,1+c)$-topologically flat $A$-module.
...
\end{enumerate}
\end{proposition}
\begin{proof} We regard $A$ as a closed subalgebra of the unital Banach algebra
$B:=A_+$. Then $F$ is a unital left $B$-module. Using the notation of
...
propositions~\ref{RetrMetCTopInjIsMetCTopInj} and~\ref{MetCTopFlatCharac} the modules
...
$(ii)$ Again, by proposition~\ref{MetCTopFlatCharac} the right $A$-module
$F_{ess}^*$ is $C_1$-topologically injective. The module $F/F_{ess}$ is an
annihilator $A$-module and $A$ has a right $c$-bounded approximate identity, so
by proposition~\ref{MetTopFlatAnnihModCharac} it is
$\max(C_2,1+c)$-topologically flat, and therefore ${(F/F_{ess})}^*$ is
$\max(C_2,1+c)$-topologically injective by
proposition~\ref{MetCTopFlatCharac}. So from
proposition~\ref{MetTopInjModProd} their product is
$\max(C_1,C_2,1+c)$-topologically injective. By proposition~\ref{DualBanModDecomp}
this product is ${(1+c)}^2$-isomorphic to $F^*$ in $\mathbf{mod}\text{-}A$. Therefore,
$F^*$ is a ${(1+c)}^2\max(C_1, C_2,1+c)$-topologically injective $A$-module. Now the
...
\cite[proposition 0.3.15]{HelHomolBanTopAlg}. As a consequence,
...
\]
\[
\geq\sup \{c^{-1}|f(x)|:x\in B_F \}=c^{-1}\lVert f\rVert.
\]
...
\cite[corollary 23.5(1)]{DefFloTensNorOpId}~$\rangle$ the Banach space $F^*$
is $\langle$~metrically / $C$-topologically~$\rangle$ injective. Since the operator
$\pi_F^*$ is $\langle$~isometric / $c$-topologically injective~$\rangle$
...
Fix $h\in {(A\projtens\ell_1(B_F))}^*$ and $x\in F$. Consider a bilinear
functional $M_{h,x}:A\times A\to\mathbb{C}:(a,b)\mapsto R(h\cdot a)(b\cdot x)$.
Clearly, $\lVert M_{h,x}\rVert\leq\lVert R\rVert\lVert h\rVert\lVert x\rVert$. By
the universal property of the projective tensor product we have a bounded linear
functional 
...
$m_{h,a\cdot x}(u)=m_{h,x}(u\cdot a)$. It is easily checked for elementary tensors.
...
have $|m_{h,x}(d_\nu)|\leq c\lVert R\rVert\lVert h\rVert\lVert x\rVert$, 
i.e. ${(m_{h,x}(d_\nu))}_{\nu\in N}$ is a
bounded net of complex numbers. Therefore, we have a well-defined limit
...
the norm estimates for $m_{h,x}$ it follows that $\tau$ is bounded 
with $\lVert\tau\rVert\leq c\lVert R\rVert$. For all $a\in A$, $x\in F$ 
...
So $\tau\pi_F^*=1_{F^*}$. This means that $F^*$ is a $\langle$~$1$-retract /
 ...
 $\langle$~$\mathbf{mod}_1\text{-}A$ / $\mathbf{mod}\text{-}A$~$\rangle$. Let $i:A\to A_+$ be
 the natural isometric embedding. It is a morphism of left $A$-modules
 and $\pi_F^+(i\projtens 1_{\ell_1(B_F)})=\pi_F$, so
 ${(i\projtens 1_{\ell_1(B_F)})}^*$ is a contractive morphism of right
 $A$-modules and
 $\tau{(i\projtens 1_{\ell_1(B_F)})}^*{(\pi_F^+)}^*=\tau\pi_F^*=1_{F^*}$.
 Hence $F^*$ is also a $\langle$~$1$-retract / $c^2 C$-retract~$\rangle$ of
 ${(A_+\projtens\ell_1(B_F))}^*$. The latter
...
${(1+c)}^2C\max(c^3,(1+c))$-topologically flat $A$-module.
\end{theorem}
...
$A$-module $F/F_{ess}$ is an $\mathscr{L}_{1,(1+c)C}^g$-space. From
proposition~\ref{MetTopEssL1FlatModAoverAmenBanAlg} we get that the essential
$A$-module $F_{ess}$ is $c^3 C$-topologically flat. Now the result follows from
...
\cite[theorem 7.1.60]{HelBanLocConvAlg}. Even topological theory is so
...
\begin{enumerate}[label = (\roman*)]
    \item $F$ is a topologically flat $A$-module; 
...
Finally, we are able to give an example of a relatively flat, but not topologically
...
isomorphic to a Hilbert space \cite[p.52]{RosProjTransInvSbspLpG}. By
\cite[proposition 1.4.7]{KaniBanAlg} we have that $I$ is a two-sided ideal of
$A$, as is any translation invariant subspace of $A$. By \cite[section 23.3]{DefFloTensNorOpId} this ideal is not an $\mathscr{L}_1^g$-space. So from
proposition~\ref{TopFlatModAoverAmenL1BanAlgCharac} we get that $I$ is not
topologically flat as an $A$-module. We claim that it is still relatively flat. Since
$\mathbb{T}$ is a compact group, it is amenable \cite[proposition 3.12.1]{PierAmenLCA}. Thus, $A$ is relatively amenable \cite[proposition VII.1.86]{HelBanLocConvAlg}, so all closed left ideals of $A$ are relatively flat
...
The results of this section are needed for the study of metric and topological
injectivity of $C^*$-algebras.

\begin{proposition}\label{MetTopInjOfId} Let $I$ be a closed right ideal of a Banach
algebra $A$. Assume $I$ is a $\langle$~metrically / $C$-topologically~$\rangle$
...
of $A$ in $\mathbf{mod}\text{-}A$.
\end{proposition}
\begin{proof} Consider the isometric embedding $\rho^+:I\to A_+$ of $I$ into $A_+$.
Clearly, this is an $A$-morphism. Since $I$ is $\langle$~metrically /
$C$-topologically~$\rangle$ injective, $\rho^+$ has a left inverse
...
$\lVert p\rVert
\leq\lVert\tau^+\rVert\lVert e_{A_+}\rVert
\leq\lVert\tau^+\rVert$. Consider the maps
$\rho:I\to A:x\mapsto x$ and $\tau:A\to I:x\mapsto p x$. Clearly, they are
morphisms of right $A$-modules and $\tau\rho=1_I$. Hence, $I$ is a
$\langle$~$1$-retract / $C$-retract~$\rangle$ of $A$ in $\mathbf{mod}\text{-}A$.
\end{proof}
